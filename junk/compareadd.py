...
sap_lookup = {
    site["name"][-4:]: (site["name"], site["address"]) for _, site in sites_df.iterrows() if site["name"][-4:].isdigit()
}
...
if __name__ == "__main__":
    ...
    with Pool(processes=cpu_count()) as pool:
        tmo_results = list(
            tqdm(
                pool.imap(compare_tmo_address, tmo_df.to_dict(orient="records")), total=len(tmo_df), desc="TMO Progress"
            )
        )
    ...
    with Pool(processes=cpu_count()) as pool:
        site_results = list(
            tqdm(pool.imap(compare_site_address_wrapper, site_args), total=len(site_args), desc="Site Progress")
        )
    ...
    with open("address_comparison_results.txt", "w", encoding="utf-8") as f:
        f.write(f"Comparison run at: {datetime.now(UTC).isoformat()}\n\n")
        ...
        for sap_id, tmo_addr, site_name, site_addr, match_type in tmo_results:
            if match_type == "unmatched":
                ...
                f.write(
                    f"✅ Matched TMO ({match_type}): SAP ID {sap_id},"
                    f" TMO: {tmo_addr} <--> Mist: {site_name} \n {site_addr}\n"
                )

        for site_name, site_addr, _sap_id, _tmo_addr, match_type in site_results:
            if match_type == "unmatched":
