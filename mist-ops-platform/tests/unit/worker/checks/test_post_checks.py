"""Tests proving post-checks fetch the org device inventory once per run.

Issue #1886. Before this fix, PostCheckService called list_all_entities
...
def _make_service(device_ids: list[str]) -> PostCheckService:
    """Build a PostCheckService whose Mist client returns one inventory page."""
    ...
    return PostCheckService(MagicMock(), mist)  # WHY: the DB session is unused by these checks.
...
class TestServiceHealthResultsUseTheSharedIndex:
    ...
    def test_connected_devices_pass_and_disconnected_devices_fail(self) -> None:
        ...
        service = PostCheckService(MagicMock(), mist)  # WHY: the DB session is unused here.
        ...
        assert by_name["health:dev-a"].passed is True  # WHY: connected must pass.
        assert by_name["health:dev-b"].passed is False  # WHY: disconnected must fail.

    def test_a_device_missing_from_inventory_fails_as_unknown(self) -> None:
        mist = MagicMock()  # WHY: stand in for the real Mist client.
        ...
        by_name = {r.name: r for r in results}  # WHY: index results for a readable assertion.
        assert by_name["health:dev-x"].passed is False  # WHY: a missing device must fail.
        # WHY: message names the unknown state.
        assert "unknown" in by_name["health:dev-x"].message
...
class TestInventoryFetchFailureFailsEveryTarget:
    ...
    def test_an_api_exception_fails_every_target_with_the_error(self) -> None:
        ...
        by_name = {r.name: r for r in results}  # WHY: index results for a readable assertion.
        assert by_name["health:dev-a"].passed is False  # WHY: no device can pass on failure.
        assert by_name["health:dev-b"].passed is False  # WHY: no device can pass on failure.
