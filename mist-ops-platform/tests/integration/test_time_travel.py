...
class TestTimeTravelIntegration:
    ...
    def test_sha256_dedup(self, sample_configs: list[dict]) -> None:
        ...
        assert hashlib.sha256(blob_a.encode()).hexdigest() == (
            hashlib.sha256(blob_b.encode()).hexdigest()
        )
    ...
    def test_different_blobs_different_hash(
        ...
        hash_a = hashlib.sha256(
            json.dumps(sample_configs[0]["config_blob"], sort_keys=True).encode()
        ).hexdigest()
        hash_b = hashlib.sha256(
            json.dumps(sample_configs[1]["config_blob"], sort_keys=True).encode()
        ).hexdigest()
    ...
    def test_time_travel_selects_latest_before_ts(
        ...
        candidates = [
            c for c in sample_configs if c["captured_at"] <= query_ts
        ]
