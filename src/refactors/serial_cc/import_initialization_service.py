...
class ImportInitializationService:
    ...
    def _run_dependency_upgrade(manager: Any, skip_deps: bool) -> None:
        ...
        if skip_deps:  # --skip-deps short-circuits all dependency tooling
            logger.info("Dependency checking and installation skipped (--skip-deps flag)")  # Trace the skip
    ...
    def _import_package_group(manager: Any, packages: dict[str, Any], required: bool, skip_deps: bool) -> None:
        ...
        for module_name, package_spec in packages.items():  # Small groups import sequentially
            ...
            if result:  # Import succeeded
                logger.info("  [OK] %s: Available", module_name)  # Trace availability
            elif required:  # Required import failed - this is an error
                logger.error("  [FAIL] %s: Failed to import", module_name)  # Trace required failure
            else:  # Optional import failed - non-fatal warning
                logger.warning("  [WARN] %s: Not available", module_name)  # Trace optional absence
    ...
    def _log_summary(manager: Any, elapsed_time: float) -> None:
        ...
        logger.info("Import initialization completed in %.2f seconds", elapsed_time)  # Trace elapsed time
        logger.info("Required dependencies: %s/%s successful", successful_required, total_required)  # Required summary
        logger.info("Optional dependencies: %s/%s available", optional_imported, len(manager.optional_packages))  # Opt

        if manager.installed_packages:  # Some packages were installed during this run
            logger.info("Newly installed packages: %s", ", ".join(manager.installed_packages))  # List installs

        if manager.failed_imports:  # Some imports failed
            logger.error("Failed imports: %s", ", ".join(manager.failed_imports))  # List failures
    ...
    def execute(cls, manager: Any, skip_deps: bool = False) -> tuple[bool, dict[str, Any]]:
        ...
        if manager._initialization_complete:  # Idempotent - return cached results on repeat calls
            logger.debug("Import initialization already completed, returning cached results")  # Trace cache hit
        ...
        logger.info("Initializing global import management system...")  # Trace workflow start

        cls._run_dependency_upgrade(manager, skip_deps)  # Phase 1: uv toolchain check/install/upgrade

        logger.info("Importing required dependencies...")  # Trace required-import phase
        cls._import_package_group(manager, manager.required_packages, True, skip_deps)  # Phase 2: required imports

        logger.info("Importing optional dependencies...")  # Trace optional-import phase
