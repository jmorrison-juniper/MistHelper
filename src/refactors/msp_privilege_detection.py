...
def _extract_msp_name(response: Any) -> str | None:
    ...
    logger.debug("_extract_msp_name: entry")  # BEFORE: trace helper entry
    ...
    if not isinstance(data, dict):  # Need a dict payload to read the name field
        logger.debug("_extract_msp_name: response.data is not a dict, returning None")  # AFTER: no-data trace
    ...
    logger.debug("_extract_msp_name: exit -- name=%s", "<set>" if result else "None")  # AFTER: success trace
...
def _fetch_msp_name(msp_id: str, session: Any) -> str | None:
    ...
    logger.info("_fetch_msp_name: entry (msp_id=%s...)", msp_id[:8])  # BEFORE: trace fetch entry
    if session is None:  # No active session to query with
        logger.debug("_fetch_msp_name: no session, returning None")  # AFTER: no-session trace
    ...
    try:  # API call and payload parsing may fail. Degrade to None on any error
        ...
        logger.debug("_fetch_msp_name: exit -- result=%s", "<set>" if result else "None")  # AFTER: success trace
...
def _msp_resolve_name(msp_id: str, priv: dict[str, Any], session: Any) -> str:
    ...
    logger.debug("_msp_resolve_name: entry (msp_id=%s...)", msp_id[:8])  # BEFORE: trace resolve entry
    ...
    if not msp_name or msp_name == "Unknown":  # Name absent or placeholder.
        ...
        logger.debug("_msp_resolve_name: fell back to fetch or short label -- %s", resolved)  # AFTER: fallback trace
        return resolved  # Return the fallback name
    logger.debug("_msp_resolve_name: exit -- using grant-provided name")  # AFTER: happy-path trace
...
def _msp_parse_one_privilege(priv: Any, session: Any) -> dict[str, Any] | None:
    ...
    logger.debug("_msp_parse_one_privilege: entry")  # BEFORE: trace parse entry
    if not (isinstance(priv, dict) and priv.get("msp_id")):  # Only MSP-scoped dict grants qualify.
        logger.debug("_msp_parse_one_privilege: not an MSP grant, returning None")  # AFTER: non-msp trace
    ...
    if not msp_id or not isinstance(msp_id, str):  # Guard against missing/invalid IDs.
        logger.debug("_msp_parse_one_privilege: msp_id missing or non-str, returning None")  # AFTER: bad-id trace
    ...
    logger.debug("_msp_parse_one_privilege: exit -- built normalized MSP record")  # AFTER: success trace
...
def _msp_extract_from_user_data(user_data: dict[str, Any], session: Any) -> list[dict[str, Any]]:
    ...
    logger.debug("_msp_extract_from_user_data: entry")  # BEFORE: trace extract entry
    privileges = user_data.get("privileges", [])  # Pull the list of privilege grants.
    logger.debug("MSP detection: parsing %s privilege entries", len(privileges))  # Log how many grants we'll scan.
    ...
    logger.debug("_msp_extract_from_user_data: exit -- found %d MSP grant(s)", len(detected_msps))  # AFTER trace
...
def _msp_fetch_user_data(session: Any) -> dict[str, Any] | None:
    ...
    logger.info("_msp_fetch_user_data: entry")  # BEFORE: trace API call
    ...
    if not response or not hasattr(response, "data"):  # No usable payload came back.
        logger.warning("getSelf returned no data - cannot detect MSP privileges")  # Warn we cannot determine access.
    ...
    if not isinstance(user_data, dict):  # The body should be a JSON object.
        logger.warning("getSelf returned unexpected type: %s", type(user_data))  # Warn about the malformed shape.
        return None  # Cannot parse privileges from this.
    logger.debug("_msp_fetch_user_data: exit -- returning validated user_data dict")  # AFTER: success trace
...
def detect_msp_privileges(session: Any) -> list[dict[str, Any]]:
    ...
    if not session:  # No usable session was supplied.
        logger.warning("Cannot detect MSP privileges - no active session")  # Warn that detection cannot proceed.
    ...
    try:  # API or parsing failures must degrade to "no MSP access" rather than crash the session.
        ...
        if detected_msps:  # At least one MSP grant was found.
            logger.info("User has MSP-level access to %s MSP(s)", len(detected_msps))  # Report the count.
        else:  # No MSP grants present in the payload.
            logger.debug("No MSP privileges detected for current user")  # Note the absence at debug level.
        logger.debug(
