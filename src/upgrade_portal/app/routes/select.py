...
def read_site_locks(org_id: str, site_ids: list[str]) -> dict[str, str | None]:
    ...
    except Exception as error:  # Keep broad because a read-only page must survive any lock store or seam fault.
        logger.error(
            "select: the lock store read failed with %s, so every site shows an unknown state",
            type(error).__name__,
        )  # Log the exception type without a token or address.
...
def address_index(cloud_session: Any, site_id: str) -> dict[str, str]:
    ...
    except Exception as error:  # Keep broad because an address-only read must not hide the inventory page.
        logger.error("select: the device statistics read failed with %s", type(error).__name__)
        return {}
...
def session_lock_record(site_id: str) -> lock.LockRecord | None:
    ...
    try:  # The session read needs a request, and a damaged field must not hide a page.
        stored = stored_lock_records().get(site_id)  # None means this browser took no lock on that site.
    except Exception as error:  # Keep broad because a damaged session must not hide a read-only banner.
        logger.error(
            "select: the session lock index read for site %s failed with %s",
            site_id,
            type(error).__name__,
        )
...
def lock_cooldown_seconds(org_id: str, site_id: str) -> int:
    ...
    except Exception as error:  # Keep broad because a cooldown read must not hide the page.
        logger.error("select: the lock cooldown read for site %s failed with %s", site_id, type(error).__name__)
...
def takeover_word(holder: str) -> str:
    ...
    except Exception as error:  # Keep broad because a page without identity context must still render safely.
        logger.error("select: the takeover owner read failed with %s", type(error).__name__)
...
def lock_banner_context(org_id: str, site_id: str) -> dict[str, Any]:
    ...
    except Exception as error:  # Keep broad because a banner must show unknown instead of hiding the page.
        logger.error("select: the lock seam for site %s failed with %s", site_id, type(error).__name__)
...
def read_banner_lock_record(org_id: str, site_id: str) -> lock.LockRecord | None:
    ...
    except Exception as error:  # Keep broad because a run-link read must not hide the lock banner.
        logger.error("select: the lock run read for site %s failed with %s", site_id, type(error).__name__)
