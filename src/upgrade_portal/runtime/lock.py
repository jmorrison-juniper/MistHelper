...
def lock_renewal_max_seconds() -> int:
    ...
    try:  # The environment holds text and can hold a mistake.
        ...
        _LOGGER.warning("lock: %s must be a number, so the portal uses %s", LOCK_RENEWAL_MAX_SECONDS_VARIABLE, DEFAULT_LOCK_RENEWAL_MAX_SECONDS)
    ...
    _LOGGER.warning("lock: %s must be at least %s, so the portal uses %s", LOCK_RENEWAL_MAX_SECONDS_VARIABLE, LOCK_TTL_SECONDS, DEFAULT_LOCK_RENEWAL_MAX_SECONDS)
