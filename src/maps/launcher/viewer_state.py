...
class MapViewerState:
    ...
    serializer: Any = None  # PlotlyMapDataSerializer used by site-switch + refresh callbacks (wave E2)
    all_sites: list[dict[str, Any]] = field(default_factory=list)  # All org sites for URL/site-switch (wave E2)
    all_maps: list[dict[str, Any]] = field(default_factory=list)  # All site maps for URL/site-switch (wave E2)
    available_sites: list[dict[str, Any]] = field(default_factory=list)  # Sites available to selectors (wave E2)
    figure_builder: Any = None  # PlotlyMapFigureBuilder reused by URL/site-switch figure rebuilds (wave E2)
    heatmap_renderer: Any = None  # PlotlyCoverageHeatmapRenderer for RF coverage rebuilds (wave E2)
