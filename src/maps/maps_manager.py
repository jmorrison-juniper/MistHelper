...
from src.maps._container_detection import is_running_in_container  # Detects Docker/container runtimes to gate GUI viewer launches.
...
from src.maps.plotly_map_callback_manager import PlotlyMapCallbackManager  # Registers Dash callbacks for the interactive viewer.
...
class MapsManager:  # WHY: declare MapsManager class
    ...
    @staticmethod
    def _match_site_by_index(sites_sorted: list, selection: str) -> "dict | None":  # WHY: declare private helper _match_site_by_index
    ...
    @staticmethod
    def _match_site_by_name(sites_sorted: list, selection: str) -> "dict | None":  # WHY: declare private helper _match_site_by_name
        ...
        if len(matches) > 1:  # WHY: branch on condition
            print(f"\n! Multiple matches found ({len(matches)}). Please be more specific.")  # WHY: surface user-facing message
            return None  # WHY: return computed result
        print("\n! No matching site found")  # WHY: surface user-facing message
        return None  # WHY: return computed result

    def _resolve_site_selection(self, sites_sorted: list, selection: str) -> "dict | None":  # WHY: declare private helper _resolve_site_selection
    ...
    def select_site(self):  # WHY: declare public method select_site
        ...
        try:
            selection = InputUtils.safe_input("Enter site index or name: ", context="select_site").strip()  # WHY: compute selection
            ...
            logging.info("MapsManager site selection: %s (%s)", self.current_site_name, self.current_site_id)  # WHY: action-log before operation
    ...
    @staticmethod
    def _print_map_selection_list(maps: list, site_name: str) -> None:  # WHY: declare private helper _print_map_selection_list
    ...
    @staticmethod
    def _parse_map_selection(selection: str, maps: list) -> "str | None":  # WHY: declare private helper _parse_map_selection
    ...
    def _prompt_map_choice(self, maps: list, site_name: str) -> "str | None":  # WHY: declare private helper _prompt_map_choice
    ...
    def _fetch_site_maps_or_none(self, site_id: str, site_name: str) -> "list | None":  # WHY: declare private helper _fetch_site_maps_or_none
    ...
    def _select_map_with_list(self, site_id: str, site_name: str) -> "tuple[str | None, list]":  # WHY: declare private helper _select_map_with_list
    ...
    def _select_map_from_site(self, site_id, site_name, return_all_maps=False):  # WHY: declare private helper _select_map_from_site
    ...
    def _backup_map_geometry(self, api_session, site_id, map_id, map_name, backup_reason="manual"):  # WHY: declare private helper _backup_map_geometry
    ...
    @staticmethod
    def _print_menu_bulk_and_analytics_section() -> None:  # WHY: declare private helper _print_menu_bulk_and_analytics_section
    ...
    def _dispatch_menu_choice(self, choice: "str | None", dispatch: dict) -> bool:  # WHY: declare private helper _dispatch_menu_choice
    ...
    @staticmethod
    def _render_site_maps_table(site_name: str, maps: list) -> None:  # WHY: declare private helper _render_site_maps_table
    ...
    def _run_export_site_maps(self, site_id: str, site_name: str) -> None:  # WHY: declare private helper _run_export_site_maps
    ...
    def _flatten_map_for_export(self, site: dict, map_item: dict) -> dict:  # WHY: declare private helper _flatten_map_for_export
    ...
    def _fetch_site_maps_safe(self, site_id: str, err_context: str) -> list:  # WHY: declare private helper _fetch_site_maps_safe
    ...
    def _collect_flat_map_rows(self, sites: list, desc: str) -> list:  # WHY: declare private helper _collect_flat_map_rows
    ...
    def export_all_site_maps(self):  # WHY: declare public method export_all_site_maps
        ...
        try:
            ...
            if not all_maps_data:  # WHY: guard against missing precondition
                ...
                return  # WHY: return early
            write_data_with_format_selection(all_maps_data, "SiteMaps_Export", api_function_name="listSiteMaps")  # WHY: assign computed value
            print(f"\n{'-' * 80}")  # WHY: surface user-facing message
            print(f"Export completed: {len(all_maps_data)} maps exported")  # WHY: surface user-facing message
            print(f"{'-' * 80}")  # WHY: surface user-facing message
            logging.info("Exported %s maps from %s sites", len(all_maps_data), len(sites))  # WHY: action-log before operation
        except Exception as e:  # WHY: handle expected error
    ...
    def export_maps_with_images(self):  # WHY: declare public method export_maps_with_images
        ...
        try:
            ...
            if not maps_with_images:  # WHY: guard against missing precondition
                ...
                return  # WHY: return early
            write_data_with_format_selection(maps_with_images, "SiteMaps_WithImages", api_function_name="listSiteMaps")  # WHY: assign computed value
            print(f"\n{'-' * 80}")  # WHY: surface user-facing message
    ...
    def _determine_image_extension(self, image_url: str) -> str:  # WHY: declare private helper _determine_image_extension
    ...
    def _download_single_map_image(self, map_item: dict, download_dir: str) -> bool:  # WHY: declare private helper _download_single_map_image
        ...
        try:
            ...
            if response.status_code == 200:  # WHY: branch on condition
                ...
                return True  # WHY: return computed result
            logging.warning("Failed to download %s: HTTP %s", map_name, response.status_code)  # WHY: surface non-fatal issue
        except Exception as e:  # WHY: handle expected error
    ...
    def _fetch_maps_with_images(self, site_id: str, site_name: str) -> "list | None":  # WHY: declare private helper _fetch_maps_with_images
    ...
    def _resolve_site_maps_for_download(self, site_id: str) -> tuple[str, list] | None:  # WHY: declare private helper _resolve_site_maps_for_download
    ...
    def _download_maps_batch(self, maps_with_images: list, download_dir: str) -> int:  # WHY: declare private helper _download_maps_batch
        ...
        logging.info("Starting batch download of %d map images to %s", len(maps_with_images), download_dir)  # WHY: action-log before operation
    ...
    @staticmethod
    def _print_download_images_summary(downloaded: int, total: int, download_dir: str) -> None:  # WHY: declare private helper _print_download_images_summary
    ...
    def _run_download_site_map_images(self, site_id: str) -> None:  # WHY: declare private helper _run_download_site_map_images
    ...
    def _print_map_optional_fields(self, map_details: dict) -> None:  # WHY: declare private helper _print_map_optional_fields
    ...
    def _fetch_map_detail(self, site_id: str, map_id: str) -> dict | None:  # WHY: declare private helper _fetch_map_detail
    ...
    def _run_view_map_details(self, site_id: str, site_name: str) -> None:  # WHY: declare private helper _run_view_map_details
    ...
    def _prompt_map_name(self) -> str | None:  # WHY: declare private helper _prompt_map_name
        ...
        try:
            map_name = InputUtils.safe_input("Enter map name: ", context="_prompt_map_name").strip()  # WHY: compute map_name
        except EOFError:  # WHY: handle expected error
    ...
    def _prompt_map_type(self) -> str:  # WHY: declare private helper _prompt_map_type
        ...
        type_choice = InputUtils.safe_input("Select type (1-3, default=1): ", context="_prompt_map_type").strip() or "1"  # WHY: compute type_choice
        type_map = {"1": "image", "2": "google", "3": "baidu"}  # WHY: compute type_map
        return type_map.get(type_choice, "image")  # WHY: return computed result

    def _prompt_image_dimensions(self) -> tuple[int, int, float]:  # WHY: declare private helper _prompt_image_dimensions
    ...
    def _build_and_create_map(self, site_id: str, map_name: str, map_type: str) -> None:  # WHY: declare private helper _build_and_create_map
        ...
        try:
            ...
            response = mistapi.api.v1.sites.maps.createSiteMap(self.apisession, site_id=site_id, body=map_payload)  # WHY: compute response
            if response.status_code in [200, 201]:  # WHY: branch on condition
                created_map = response.data  # WHY: compute created_map
                print(f"\n{'-' * 80}")  # WHY: surface user-facing message
                ...
                logging.error("Map creation failed: %s - %s", response.status_code, response.data)  # WHY: surface fatal issue
    ...
    def create_site_map(self):  # WHY: declare public method create_site_map
        ...
        print("-" * 80)  # WHY: surface user-facing message
        print("\n! Note: This creates a map placeholder. Upload image separately (Menu 7)")  # WHY: surface user-facing message
    ...
    @staticmethod
    def _build_map_without_image_record(site: dict, map_item: dict, org_id: str) -> dict:  # WHY: declare private helper _build_map_without_image_record
    ...
    def _scan_site_missing_image_maps(self, site: dict) -> tuple[list, int]:  # WHY: declare private helper _scan_site_missing_image_maps
        ...
        rows = [self._build_map_without_image_record(site, m, self.org_id) for m in maps if "url" not in m]  # WHY: compute rows
        return rows, len(maps)  # Rows for maps missing images + total map count

    def _collect_maps_missing_images(self, sites: list) -> tuple[list, int]:  # WHY: declare private helper _collect_maps_missing_images
    ...
    @staticmethod
    def _render_maps_without_images_table(rows: list) -> None:  # WHY: declare private helper _render_maps_without_images_table
    ...
    def maps_without_images_report(self):  # WHY: declare public method maps_without_images_report
        ...
        try:
            ...
            write_data_with_format_selection(rows, "MapsWithoutImages_Report", api_function_name="listSiteMaps")  # WHY: assign computed value
    ...
    def _collect_property_input(self, prompt, current_value, value_type=str):  # WHY: declare private helper _collect_property_input
        """Collect a single property update from user with type validation."""
        raw = InputUtils.safe_input(f"{prompt} [{current_value}]: ", context="_collect_property_input").strip()  # WHY: compute raw
    ...
    @staticmethod
    def _print_map_update_preview(update_payload: dict) -> None:  # WHY: declare private helper _print_map_update_preview
    ...
    def _apply_map_update(self, site_id: str, map_id: str, update_payload: dict) -> None:  # WHY: declare private helper _apply_map_update
    ...
    def _confirm_and_apply_map_update(self, site_id: str, map_id: str, update_payload: dict) -> None:  # WHY: declare private helper _confirm_and_apply_map_update
    ...
    @staticmethod
    def _render_current_map_properties(current_map: dict) -> None:  # WHY: declare private helper _render_current_map_properties
    ...
    def _load_map_for_update(self, site_id: str, site_name: str) -> "tuple[str, dict] | None":  # WHY: declare private helper _load_map_for_update
        ...
        if not map_id:  # WHY: guard against missing precondition
            return None  # WHY: return computed result
        resp = mistapi.api.v1.sites.maps.getSiteMap(self.apisession, site_id=site_id, map_id=map_id)  # WHY: compute resp
        if resp.status_code != 200:  # WHY: branch on condition
    ...
    @staticmethod
    def _print_update_map_properties_header() -> None:  # WHY: declare private helper _print_update_map_properties_header
        """Print the banner shown by the update-map-properties action."""
        print("\n" + "-" * 80)  # Top rule
        print("UPDATE MAP PROPERTIES")  # Title
        print("-" * 80)  # Bottom rule

    def _run_update_map_properties(self, site_id: str, site_name: str) -> None:  # WHY: declare private helper _run_update_map_properties
    ...
    def _fetch_map_for_delete(self, site_id: str, map_id: str) -> dict | None:  # WHY: declare private helper _fetch_map_for_delete
    ...
    @staticmethod
    def _render_map_delete_preview(current_map: dict, map_id: str) -> None:  # WHY: declare private helper _render_map_delete_preview
    ...
    def _confirm_map_delete(map_id: str) -> bool:  # WHY: declare private helper _confirm_map_delete
        ...
        confirmation = InputUtils.safe_input("Confirmation: ", context="delete_site_map").strip()  # WHY: compute confirmation
    ...
    def _perform_map_delete(self, site_id: str, map_id: str) -> None:  # WHY: declare private helper _perform_map_delete
        ...
        resp = mistapi.api.v1.sites.maps.deleteSiteMap(self.apisession, site_id=site_id, map_id=map_id)  # WHY: compute resp
    ...
    def _run_delete_site_map(self, site_id: str, site_name: str) -> None:  # WHY: declare private helper _run_delete_site_map
    ...
    @staticmethod
    def _validate_image_file_path(file_path: str) -> str | None:  # WHY: declare private helper _validate_image_file_path
    ...
    def _prompt_and_validate_image_path(self) -> str | None:  # WHY: declare private helper _prompt_and_validate_image_path
    ...
    def _prompt_yes_no(prompt: str, context: str) -> bool:  # WHY: declare private helper _prompt_yes_no
        ...
        return InputUtils.safe_input(prompt, context=context).strip().lower() in ["yes", "y"]  # WHY: return computed result

    def _check_upload_size(self, file_path: str) -> tuple[float, bool]:  # WHY: declare private helper _check_upload_size
    ...
    def _perform_image_upload(self, site_id: str, map_id: str, file_path: str) -> None:  # WHY: declare private helper _perform_image_upload
    ...
    def _confirm_image_upload(self, site_id: str, map_id: str, file_path: str) -> None:  # WHY: declare private helper _confirm_image_upload
    ...
    def _run_upload_map_image(self, site_id: str, site_name: str) -> None:  # WHY: declare private helper _run_upload_map_image
    ...
    def _get_devices_on_map(self, site_id: str, map_id: str) -> list | None:  # WHY: declare private helper _get_devices_on_map
        ...
        devices_response = mistapi.api.v1.sites.devices.listSiteDevices(self.apisession, site_id=site_id, type="all")  # WHY: compute devices_response
        if devices_response.status_code != 200:  # WHY: branch on condition
            print(f"\n! Failed to fetch devices: HTTP {devices_response.status_code}")  # WHY: surface user-facing message
            return None  # WHY: return computed result
    ...
    def _export_map_devices_csv(self, devices: list, site_id: str, site_name: str) -> None:  # WHY: declare private helper _export_map_devices_csv
        ...
        write_data_with_format_selection(devices_data, filename, api_function_name="listSiteDevices")  # WHY: assign computed value
    ...
    def _run_view_devices_flow(self, site_id: str, site_name: str) -> None:  # WHY: declare private helper _run_view_devices_flow
    ...
    def _maybe_export_map_devices(self, devices_on_map: list, site_id: str, site_name: str) -> None:  # WHY: declare private helper _maybe_export_map_devices
        """Prompt for and optionally perform CSV export of the device listing."""
        answer = InputUtils.safe_input(
            "\nExport to CSV? (yes/no): ",
            context="view_devices_on_map",
        ).strip().lower()  # Normalize the user's reply for comparison.
    ...
    def _fetch_site_maps_with_images(self, site_id: str) -> "list | None":  # WHY: declare private helper _fetch_site_maps_with_images
    ...
    def _download_all_site_map_images(self, site: dict, base_dir: str) -> tuple[int, int]:  # WHY: declare private helper _download_all_site_map_images
        ...
        try:
            ...
            downloaded = sum(1 for m in maps_with_images if self._download_single_map_image(m, site_dir))  # WHY: compute downloaded
    ...
    @staticmethod
    def _print_bulk_download_summary(total_downloaded: int, total_maps: int, base_dir: str) -> None:  # WHY: declare private helper _print_bulk_download_summary
    ...
    def _install_visualization_packages(self) -> None:  # WHY: declare private helper _install_visualization_packages
        ...
        if _import_manager is None:  # WHY: branch on condition
            logging.debug("import_manager not available (standalone mode) - skipping package installation checks")  # WHY: action-log after operation
            return  # WHY: return early
        ...
        for package_name, package_spec in required.items():  # WHY: iterate collection
            logging.debug("Checking required package: %s (%s)", package_name, package_spec)  # WHY: action-log after operation
            _import_manager.import_module_safely(  # WHY: advance computation
        ...
        for package_name, package_spec in optional.items():  # WHY: iterate collection
            try:
                logging.debug("Checking optional package: %s (%s)", package_name, package_spec)  # WHY: action-log after operation
                _import_manager.import_module_safely(  # WHY: advance computation
                    package_name, package_spec=package_spec, required=False, skip_deps=False, skip_upgrade=True
                )
                ...
                logging.debug("Optional package %s unavailable: %s", package_name, pkg_error)  # WHY: action-log after operation
    ...
    def _check_visualization_packages(self) -> "bool | None":  # WHY: declare private helper _check_visualization_packages
    ...
    @staticmethod
    def _log_and_print_map_summary(map_data: dict, map_id: str) -> None:  # WHY: declare private helper _log_and_print_map_summary
        ...
        print(f"Dimensions: {map_data.get('width', 1000)}x{map_data.get('height', 1000)} pixels")  # WHY: surface user-facing message
    ...
    def _warn_if_map_unscaled(map_data: dict) -> None:  # WHY: declare private helper _warn_if_map_unscaled
        ...
        if map_ppm:  # WHY: branch on condition
            return  # WHY: return early
        map_name = map_data.get("name", "Unnamed")  # WHY: compute map_name
        logging.warning("MAP NOT SCALED: Map '%s' has PPM=0 - image has not been scaled in Mist Portal", map_name)  # WHY: surface non-fatal issue
        print("\n" + "!" * 60)  # WHY: surface user-facing message
        print("! WARNING: This map image has NOT been scaled!")  # WHY: surface user-facing message
        print("! RF coverage heatmap and location features will not work correctly.")  # WHY: surface user-facing message
    ...
    def _fetch_map_details(self, site_id: str, map_id: str) -> "dict | None":  # WHY: declare private helper _fetch_map_details
        ...
        logging.info("Fetching map details - site_id: %s, map_id: %s", site_id, map_id)  # WHY: action-log before operation
        map_response = mistapi.api.v1.sites.maps.getSiteMap(self.apisession, site_id=site_id, map_id=map_id)  # WHY: compute map_response
    ...
    def _fetch_devices_on_map(self, site_id: str, map_id: str) -> list:  # WHY: declare private helper _fetch_devices_on_map
        ...
        devices_response = mistapi.api.v1.sites.stats.listSiteDevicesStats(self.apisession, site_id=site_id, limit=1000)  # WHY: compute devices_response
        logging.debug("listSiteDevicesStats API response: HTTP %s", devices_response.status_code)  # WHY: action-log after operation
        if devices_response.status_code != 200:  # WHY: branch on condition
    ...
    def _fetch_zones_on_map(self, site_id: str, map_id: str) -> list:  # WHY: declare private helper _fetch_zones_on_map
        ...
        try:
            zones_response = mistapi.api.v1.sites.zones.listSiteZones(self.apisession, site_id=site_id)  # WHY: compute zones_response
            if zones_response.status_code == 200:  # WHY: branch on condition
                ...
                logging.info("Total zones at site: %s, Zones on this map: %s", len(all_zones), len(zones_on_map))  # WHY: action-log before operation
                logging.debug("Zones on map: %s", zones_on_map)  # WHY: action-log after operation
                return zones_on_map  # WHY: return computed result
            logging.warning("Failed to fetch zones - HTTP %s", zones_response.status_code)  # WHY: surface non-fatal issue
            return []  # WHY: return computed result
    ...
    def _filter_clients_for_map(self, all_clients: list, map_id: str) -> list:  # WHY: declare private helper _filter_clients_for_map
    ...
    def _fetch_all_wireless_clients(self, site_id: str) -> "list | None":  # WHY: declare private helper _fetch_all_wireless_clients
    ...
    @staticmethod
    def _log_client_map_diagnostics(all_clients: list, clients_on_map: list, map_id: str) -> None:  # WHY: declare private helper _log_client_map_diagnostics
    ...
    def _fetch_clients_on_map(self, site_id: str, map_id: str) -> list:  # WHY: declare private helper _fetch_clients_on_map
    ...
    def _handle_coverage_exception(self, coverage_data: dict) -> None:  # WHY: declare private helper _handle_coverage_exception
        ...
        if "psycopg2" in exception_str or "database" in exception_str.lower():  # WHY: branch on condition
            logging.warning("RF Coverage temporarily unavailable: Mist backend database connectivity issue")  # WHY: surface non-fatal issue
            logging.debug("Coverage API backend error: %s", exception_str)  # WHY: action-log after operation
        else:
            logging.error("Coverage API returned error response (first 500 chars): %s", exception_str[:500])  # WHY: surface fatal issue
            logging.debug("Coverage API full error response: %s", exception_str)  # WHY: action-log after operation
            logging.debug("Error details - Query: %s, URI: %s", coverage_data.get("query"), coverage_data.get("uri"))  # WHY: action-log after operation
        print("  Note: RF Coverage heatmap unavailable (Mist backend issue) - continuing without it")  # WHY: surface user-facing message
    ...
    def _parse_map_coverage_response(self, coverage_response) -> "dict | None":  # WHY: declare private helper _parse_map_coverage_response
        """Extract coverage payload or handle known exception envelopes."""
        if coverage_response.status_code != 200:  # Non-200 -> failure.
            logging.warning("Failed to fetch RF coverage data - HTTP %s", coverage_response.status_code)  # WHY: surface non-fatal issue
            return None  # WHY: return computed result
    ...
    def _fetch_map_coverage(self, site_id: str, map_id: str) -> "dict | None":  # WHY: declare private helper _fetch_map_coverage
        ...
        try:
            ...
            return self._parse_map_coverage_response(self._request_map_coverage(site_id, map_id))  # WHY: return computed result
    ...
    def _load_map_viewer_bundle(self, site_id: str, map_id: str) -> tuple[dict, list, list, list, object, list] | None:  # WHY: declare private helper _load_map_viewer_bundle
        ...
        return map_data, devices_on_map, zones_on_map, clients_on_map, coverage_data, all_sites  # WHY: return computed result

    def _dispatch_map_viewer(  # WHY: declare private helper _dispatch_map_viewer
        self, use_plotly: bool, scope: "MapViewerScope",
        data: "MapViewerData", optional: "MapViewerOptional",
    ) -> None:
        ...
        if use_plotly:  # WHY: branch on condition
            ...
            logging.info("Launching matplotlib fallback viewer for map %s", map_name)  # WHY: action-log before operation
            self._launch_matplotlib_viewer(data.map_data, data.devices)  # WHY: advance computation

    def _run_map_viewer_flow(self, site_id: str, site_name: str) -> None:  # WHY: declare private helper _run_map_viewer_flow
        ...
        if not map_id:  # WHY: guard against missing precondition
            ...
            return  # WHY: return early
        logging.debug("Selected map_id: %s, Total maps available: %s", map_id, len(all_maps))  # WHY: action-log after operation
        ...
        optional = MapViewerOptional(coverage_data=coverage_data, all_maps=all_maps, all_sites=all_sites)  # WHY: compute optional
    ...
    @staticmethod
    def _log_ppm_mismatch(ppm: float, calculated_ppm: float, ppm_ratio: float) -> None:  # WHY: declare private helper _log_ppm_mismatch
    ...
    @staticmethod
    def _survey_path_scatter(path_name: str, path_x: list, path_y: list, point_count: int):  # WHY: declare private helper _survey_path_scatter
    ...
    @staticmethod
    def _survey_path_annotation(path_name: str, path_x: list, path_y: list) -> dict:  # WHY: declare private helper _survey_path_annotation
    ...
    @staticmethod
    def _add_survey_path_trace(fig, path_name: str, path_x: list, path_y: list, point_count: int) -> None:  # WHY: declare private helper _add_survey_path_trace
    ...
    def _add_single_survey_path(self, fig, path: dict, path_idx: int) -> None:  # WHY: declare private helper _add_single_survey_path
        ...
        if not path_coords or len(path_coords) < 2:  # Need at least 2 points to draw a line.
            logging.warning("Validation path '%s' has insufficient coordinates: %d", path_name, len(path_coords))  # WHY: surface non-fatal issue
    ...
    @staticmethod
    def _extract_client_point(client: dict, map_id: str) -> tuple | None:  # WHY: declare private helper _extract_client_point
        ...
        logging.debug(
            ...
            client_mac, x, y, client.get("map_id", "none"), map_id,
    ...
    @staticmethod
    def _collect_client_points(clients: list, map_id: str) -> tuple[list, list, list, list]:  # WHY: declare private helper _collect_client_points
    ...
    @staticmethod
    def _add_client_marker_trace(fig, xs: list, ys: list, hovers: list) -> None:  # WHY: declare private helper _add_client_marker_trace
    ...
    @staticmethod
    def _add_client_label_annotations(fig, xs: list, ys: list, names: list) -> None:  # WHY: declare private helper _add_client_label_annotations
    ...
    def _add_clients_to_figure(self, fig, clients: list, map_id: str) -> None:  # WHY: declare private helper _add_clients_to_figure
        ...
        if not xs:  # No clients had valid coordinates
            logging.warning("Found %d clients but none have x,y coordinates", len(clients))  # WHY: surface non-fatal issue
            return  # WHY: return early
    ...
    @staticmethod
    def _resolve_site_name(all_sites: list[dict], site_id: str) -> str:  # WHY: declare private helper _resolve_site_name
    ...
    @staticmethod
    def _build_node_position_lookup(nodes: list[dict]) -> dict[str, dict[str, float]]:  # WHY: declare private helper _build_node_position_lookup
    ...
    @staticmethod
    def _node_edge_segments(node: dict, node_lookup: dict[str, dict[str, float]]) -> list[dict]:  # WHY: declare private helper _node_edge_segments
    ...
    @staticmethod
    def _edges_to_segments(nodes: list[dict], node_lookup: dict[str, dict[str, float]]) -> list[dict]:  # WHY: declare private helper _edges_to_segments
    ...
    def _extract_wayfinding(self, map_data: dict) -> list[dict]:  # WHY: declare private helper _extract_wayfinding
        ...
        if wayfinding_data:  # WHY: branch on condition
            logging.debug("[Flask API] Raw wayfinding_path data: %s", wayfinding_data)  # WHY: action-log after operation
    ...
    def _collect_map_payload(self, api_session, all_sites, site_id, map_id):  # WHY: declare private helper _collect_map_payload
    ...
    def _build_map_data_response(self, site_id, map_id, map_data, layers):  # WHY: declare private helper _build_map_data_response
...
def _prompt_org_selection(orgs: list) -> str:  # WHY: declare private helper _prompt_org_selection
    ...
    try:
        choice = InputUtils.safe_input("Select organization number: ", context="_prompt_org_selection").strip()  # WHY: compute choice
...
def _filter_org_privileges(privileges: list) -> list:  # WHY: declare private helper _filter_org_privileges
    ...
    return [p["org_id"] for p in privileges if p.get("scope") == "org" and p.get("org_id")]  # WHY: return computed result


def _pick_org_from_privileges(orgs: list[str], test_mode: bool) -> str | None:  # WHY: declare private helper _pick_org_from_privileges
...
def _detect_org_from_session(apisession, test_mode: bool) -> str | None:  # WHY: declare private helper _detect_org_from_session
...
Examples:
    ...
    parser.add_argument("--menu", action="store_true", help="Show operations menu instead of launching viewer directly")  # WHY: assign computed value
    parser.add_argument("--org", type=str, default=None, help="Organization ID to use (optional)")  # WHY: assign computed value
    parser.add_argument("--debug", action="store_true", help="Enable debug logging")  # WHY: assign computed value
    parser.add_argument("--test", action="store_true", help="Run systematic test of safe, non-destructive operations")  # WHY: assign computed value
...
def _require_org_id(apisession, args) -> str:  # WHY: declare private helper _require_org_id
    ...
    if not org_id:  # WHY: guard against missing precondition
        if args.test:  # WHY: branch on condition
            print("ERROR: Organization ID required for test mode. Set org_id in .env or use --org flag")  # WHY: surface user-facing message
            sys.exit(1)  # WHY: advance computation
