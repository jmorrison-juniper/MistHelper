...
class SiteWanUsageExporter:
    ...
    def _persist_site_wan_usages(rawdata: list[Any], site_name: str) -> None:
        ...
        if not rawdata:  # No WAN usage rows for this site -- inform the operator and return.
            ...
            logger.info("! No WAN usage data found for this site")
            return
        ...
        logger.info("! %d WAN usage records exported to %s", len(rawdata), filename)
    ...
    def wan_usages() -> None:
        ...
        logger.info("Site WAN Usage Search:")
        ...
        try:
            ...
            if status_code >= _HTTP_ERROR_MIN:  # WHY: a failing HTTP status makes the empty WAN usage result unsafe.
                logger.error(  # WHY: the operator must see the cloud status instead of a false no-data message.
                    "The cloud returned HTTP %s for WAN usage at site %s",
                    status_code,
                    site_id,
                )
