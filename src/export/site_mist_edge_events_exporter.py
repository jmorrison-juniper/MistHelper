...
class SiteMistEdgeEventsExporter:
    ...
    def _persist_site_mist_edge_events(rawdata: list[Any], site_name: str) -> None:
        ...
        if not rawdata:  # No Mist Edge event rows for this site -- inform the operator and return.
            ...
            logger.info("! No Mist Edge event data found for this site")
            return
        ...
        logger.info("! %d Mist Edge event records exported to %s", len(rawdata), filename)
    ...
    def mist_edge_events() -> None:
        ...
        logger.info("Site Mist Edge Events Search:")
        ...
        try:
            ...
            if status_code >= _HTTP_ERROR_MIN:  # WHY: a failing HTTP status makes the empty event result unsafe.
                logger.error(  # WHY: the operator must see the cloud status instead of a false no-data message.
                    "The cloud returned HTTP %s for Mist Edge events at site %s",
                    status_code,
                    site_id,
                )
