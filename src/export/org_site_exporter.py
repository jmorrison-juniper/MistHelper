...
class OrgSiteExporter:  # Org site exporters.
    ...
    def sites():  # Export the org site list.
        ...
        logger.info("Starting export of organization site list...")  # Log site export start.
    ...
    def sites_list_api():  # Export sites via list API.
        ...
        if os.path.exists(output_file):  # Branch: cached file exists.
            logger.info("! Using cached %s (already exists)", output_file)  # Log cache reuse.
            # WHY: preserve operator notice verbatim. Route through logger for capture/redirection.
            logger.info("! Using cached %s (already exists)", output_file)
            return  # Skip re-fetch.
        logger.info("Fetching all sites using the 'list' sites API endpoint...")  # Log fetch start.
        # WHY: preserve operator notice verbatim. Route through logger for capture/redirection.
        logger.info("Fetching all sites using the 'list' sites API endpoint...")
        ...
        if not sites:  # Branch: no sites returned.
            logger.warning(" No sites returned from API.")  # Log empty result.
            # WHY: preserve operator notice verbatim. Route through logger for capture/redirection.
            logger.info(" No sites returned from API.")
            return  # Skip write.
        ...
        logger.info("! Sites exported to %s", output_file)  # Log the successful export
        # WHY: preserve operator notice verbatim. Route through logger for capture/redirection.
        logger.info("! Sites exported to %s", output_file)
    ...
    def sites_with_location():  # Export sites with location.
        ...
        mh = SourceDependencyResolver  # WHY: resolve source dependencies without importing the root module.
        # WHY: preserve operator notice verbatim. Route through logger for capture/redirection.
        logger.info("Sites with Location and Timezone Info:")
        logger.info("Listing Sites with Full Info:")  # Log listing start.
        org_id = mh.ConfigUtils.get_cached_or_prompted_org_id()  # Resolve org id.
        logger.debug("Using org_id: %s for site location export.", org_id)  # Log org id used.
        sites = mh.APICoreFetchUtils.all_sites_with_limit(org_id)  # Fetch all sites.
        logger.info("Fetched %s sites from the organization.", len(sites))  # Log fetched site count.
        ...
        # WHY: preserve operator notice verbatim. Route through logger for capture/redirection.
        logger.info("! %s sites exported to SitesWithLocations.csv", len(sanitized_sites))
        logger.info(" Full site data written to SitesWithLocations.csv")  # Log write success.
    ...
    def current_guests() -> None:  # Export current guest users.
        ...
        mh = SourceDependencyResolver  # WHY: resolve source dependencies without importing the root module.
        # WHY: preserve operator notice verbatim. Route through logger for capture/redirection.
        logger.info("Current and Historical Guest Users:")
        logger.info("Exporting all current guest users in the org...")  # Log guest export start.
        org_id = mh.ConfigUtils.get_cached_or_prompted_org_id()  # Resolve org id.
        ...
        # WHY: preserve operator notice verbatim. Route through logger for capture/redirection.
        logger.info("! %s current guest users exported to OrgCurrentGuests.csv", len(guests))
        logger.info(" Current guests exported to OrgCurrentGuests.csv")  # Log write success.
    ...
    def historical_guests() -> None:  # Export 7-day guest history.
        ...
        mh = SourceDependencyResolver  # WHY: resolve source dependencies without importing the root module.
        logger.info("Exporting all guest users from the last 7 days...")  # Log historical export start.
        org_id = mh.ConfigUtils.get_cached_or_prompted_org_id()  # Resolve org id.
        end_time = int(time.time())  # Capture end time as now.
        start_time = end_time - 7 * 24 * 3600  # Compute 7-day start time.
        logger.debug("Fetching guest authorizations from %s to %s (epoch seconds).", start_time, end_time)
        ...
        # WHY: preserve operator notice verbatim. Route through logger for capture/redirection.
        logger.info("! %s historical guest users exported to OrgHistoricalGuests.csv", len(guests))
        logger.info(" Historical guests exported to OrgHistoricalGuests.csv")  # Log write success.
