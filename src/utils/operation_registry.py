...
class OperationRegistry:
    ...
    ``skip_reason``.  ``get`` defaults to ``{"category": "safe"}`` for unregistered
    options and emits a warning so classification drift surfaces quickly in tests.
    """
    ...
    SKIP_CATEGORIES = frozenset(
        {"destructive", "wip", "resource_intensive", "websocket", "continuous_loop", "interactive"}
    )
