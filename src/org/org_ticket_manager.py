...
class OrgTicketManager:  # Support ticket operations.
    ...
    def create_ticket() -> None:  # Create a support ticket.
        ...
        mh = SourceDependencyResolver  # WHY: resolve source dependencies without importing the root module.
        logger.info("Menu 189: Starting support ticket creation")  # Log operation entry
        logger.debug("ENTRY: OrgTicketManager.create_ticket()")  # Debug trace
        ...
        if not subject:  # User left subject blank -- abort
            logger.warning("  Ticket creation cancelled -- subject is required.")  # Inform user
            logger.info("Ticket creation cancelled: blank subject")  # Log cancellation
            return  # Early exit
    ...
    def add_comment() -> None:  # Add a comment to a ticket.
        ...
        mh = SourceDependencyResolver  # WHY: resolve source dependencies without importing the root module.
        logger.info("Menu 190: Starting add comment to ticket")  # Log operation entry
        logger.debug("ENTRY: OrgTicketManager.add_comment()")  # Debug trace
        org_id = mh.ConfigUtils.get_cached_or_prompted_org_id()  # Resolve org from cache or user prompt
        ...
        if not ticket_id:  # User cancelled selection -- abort
            logger.warning("  Operation cancelled -- no ticket selected.")  # Inform user
            logger.info("Add comment cancelled: no ticket selected")  # Log the cancellation
        ...
        if not comment_text and not file_path:  # Neither comment nor file provided -- abort
            logger.warning("  Operation cancelled -- provide a comment or file.")  # Inform user
            logger.info("Add comment cancelled: no comment or file provided")  # Log cancellation
            return  # Early exit
    ...
    def update_ticket() -> None:  # Update a support ticket.
        ...
        mh = SourceDependencyResolver  # WHY: resolve source dependencies without importing the root module.
        logger.info("Menu 191: Starting ticket update")  # Log operation entry
        logger.debug("ENTRY: OrgTicketManager.update_ticket()")  # Debug trace
        org_id = mh.ConfigUtils.get_cached_or_prompted_org_id()  # Resolve org from cache or user prompt
        ticket_id = OrgTicketManager._select_ticket(org_id)  # Show ticket list for user selection
        if not ticket_id:  # User cancelled selection -- abort
            logger.warning("  Operation cancelled -- no ticket selected.")  # Inform user
            logger.info("Ticket update cancelled: no ticket selected")  # Log the cancellation
            return  # Early exit
        body = OrgTicketManager._build_update_body()  # Collect changed fields from user
        if not body:  # No fields were changed -- abort
            logger.warning("  No changes specified -- update cancelled.")  # Inform user
            logger.info("Ticket update cancelled: no fields changed")  # Log cancellation
            return  # Early exit
    ...
    def _prompt_ticket_type() -> str:  # Prompt for ticket type.
        ...
        for index, ticket_type in enumerate(OrgTicketManager.TICKET_TYPES, 1):  # Number each type for selection
            logger.warning("    %d. %s", index, ticket_type)  # Display numbered option
    ...
    def _print_ticket_created_summary(ticket_data: dict, subject: str, ticket_type: str) -> None:
        ...
        logger.info("Menu 189: Ticket creation complete, id=%s", ticket_id)  # Log success
    ...
    def _submit_create_ticket(org_id: str, body: dict, subject: str, ticket_type: str) -> None:
        ...
        mh = SourceDependencyResolver  # WHY: resolve source dependencies without importing the root module.
        logger.info("Creating ticket '%s' (type=%s) in org %s", subject, ticket_type, org_id)
        try:
    ...
    def _update_via_api(org_id: str, ticket_id: str, body: dict[str, str]) -> None:
        ...
        mh = SourceDependencyResolver  # WHY: resolve source dependencies without importing the root module.
        logger.info("Updating ticket %s with fields: %s", ticket_id, list(body.keys()))  # Log before API call
        try:
            ...
            logger.debug("Ticket updated: %s", getattr(response, "data", {}))  # Log full response
            logger.warning("\n  Ticket %s updated successfully!", ticket_id)  # Confirm to user
            for field, value in body.items():  # Show each changed field to user
                logger.warning("  %s: %s", field, value)  # Display field name and new value
            logger.info("Menu 191: Ticket update complete for %s", ticket_id)  # Log success
    ...
    def _submit_comment(org_id: str, ticket_id: str, comment_text: str, file_path: str) -> None:
        ...
        if has_file:  # Use multipart upload API when file is attached
            logger.info("Adding comment with attachment to ticket %s", ticket_id)  # Log before API call
            ...
            logger.debug("Comment with file submitted to ticket %s", ticket_id)  # Log after API call
            ...
            logger.warning(  # Warn about missing file path
                "File not found: %s -- adding comment without attachment", file_path
            )
            logger.warning("  Warning: File not found at '%s' -- adding comment only.", file_path)  # Alert user
    ...
    def _submit_text_comment(org_id: str, ticket_id: str, comment_text: str) -> None:  # Submit a text-only comment.
        ...
        mh = SourceDependencyResolver  # WHY: resolve source dependencies without importing the root module.
        logger.info("Adding text comment to ticket %s", ticket_id)  # Log before API call
        ...
        logger.debug("Text comment submitted to ticket %s", ticket_id)  # Log after API call
    ...
    def view_ticket() -> None:  # View a single ticket.
        ...
        mh = SourceDependencyResolver  # WHY: resolve source dependencies without importing the root module.
        logger.info("Menu 192: Starting ticket detail viewer")  # Log operation entry
        logger.debug("ENTRY: OrgTicketManager.view_ticket()")  # Debug trace
        org_id = mh.ConfigUtils.get_cached_or_prompted_org_id()  # Resolve org from cache or user prompt

        ticket_id = OrgTicketManager._select_ticket(org_id)  # Show ticket list for user selection
        if not ticket_id:  # User cancelled selection -- abort
            logger.warning("  Operation cancelled -- no ticket selected.")  # Inform user
            logger.info("View ticket cancelled: no ticket selected")  # Log the cancellation
            return  # Early exit
        ...
        if not ticket_data:  # API returned empty or failed
            logger.error("  Could not retrieve ticket details.")  # Inform user of failure
            return  # Early exit

        OrgTicketManager._display_ticket_detail(ticket_data)  # Format and print to screen
        logger.info("Menu 192: Ticket detail view complete for %s", ticket_id)  # Log success
    ...
    def export_ticket_details() -> None:  # Export ticket details to file.
        ...
        mh = SourceDependencyResolver  # WHY: resolve source dependencies without importing the root module.
        logger.info("Menu 193: Starting full ticket detail export")  # Log operation entry
        logger.debug("ENTRY: OrgTicketManager.export_ticket_details()")  # Debug trace
        org_id = mh.ConfigUtils.get_cached_or_prompted_org_id()  # Resolve org from cache or user prompt
        ...
        if all_details:  # Export if we have any ticket details
            logger.info("Exporting %d ticket details", len(all_details))  # Log before export
            ...
            logger.info("Menu 193: Full ticket detail export complete")  # Log success
    ...
    def _fetch_tickets_for_selection(org_id: str) -> list:
        ...
        mh = SourceDependencyResolver  # WHY: resolve source dependencies without importing the root module.
        logger.info("Fetching ticket list for selection (org %s)", org_id)  # Log before API call
        try:
            ...
            logger.debug("Retrieved %d tickets for selection", len(tickets))  # Log count
    ...
    def _render_ticket_list_table(tickets: list) -> None:
        ...
        for index, ticket in enumerate(tickets, 1):  # Display numbered rows
            ...
            logger.warning("  %-4d %-10s %-18s %s", index, status, ttype, subject)  # Print formatted row
    ...
    def _resolve_ticket_choice(choice: str, tickets: list) -> str:
        ...
        if not 0 <= idx < len(tickets):  # Index out of range
            logger.error("  Invalid selection: %s", choice)  # Inform user of bad input
            return ""  # Signal cancellation
    ...
    def _fetch_ticket_detail(org_id: str, ticket_id: str) -> dict:  # Fetch one ticket detail.
        ...
        mh = SourceDependencyResolver  # WHY: resolve source dependencies without importing the root module.
        logger.info("Fetching detail for ticket %s", ticket_id)  # Log before API call
        try:
            ...
            logger.debug("Received ticket detail: %d fields", len(ticket_data))  # Log field count
    ...
    def _fetch_all_ticket_summaries(org_id: str) -> list:
        ...
        mh = SourceDependencyResolver  # WHY: resolve source dependencies without importing the root module.
        logger.info("Fetching ticket list for org %s", org_id)  # Log before API call
        try:
            ...
            logger.debug("Found %d tickets to export with details", len(tickets))  # Log count
    ...
    def _collect_ticket_details(org_id: str, tickets: list) -> list:
        ...
        for index, ticket in enumerate(tickets, 1):  # Iterate each ticket summary
            ...
            logger.info("Fetching detail %d/%d: ticket %s", index, len(tickets), tid)  # Progress log
            ...
            logger.debug("Fetched detail %d/%d", index, len(tickets))  # Progress debug log
    ...
    def _render_comments_block(comments: list) -> None:
        ...
        if not comments:  # No comments on this ticket
            logger.warning("  No comments on this ticket.")  # Inform user
            return  # Nothing else to render
        logger.warning("  Comments (%d):", len(comments))  # Comment section header with count
        ...
        logger.warning("  %s", "=" * 60)  # Bottom separator bar
