...
class TroubleshootUtils:  # Marvis troubleshoot delegators.
    ...
    def _handle_marvis_invalid_choice(choice: str) -> None:
        ...
        """
        logger.warning(" Invalid option selected.")  # User-visible notice
        logger.warning("MARVIS DEBUG: Invalid troubleshooting option selected: %s", choice)  # Audit trail
        logger.debug("MARVIS DEBUG: Exiting launch_interactive() due to invalid choice")  # Trace exit reason
    ...
    def _handle_marvis_exit() -> None:
        ...
        """
        logger.debug("MARVIS DEBUG: User chose to exit")  # Trace the exit
        logger.warning("Exiting Marvis troubleshooting.")  # Tell the user

    @staticmethod
    def _invoke_marvis_client_connectivity() -> None:
        """Run the client-connectivity troubleshooter."""
        logger.debug("MARVIS DEBUG: Calling TroubleshootUtils.client_connectivity()")  # Trace the call
        TroubleshootUtils.client_connectivity()

    @staticmethod
    def _invoke_marvis_device_performance() -> None:
        """Run the device-performance troubleshooter."""
        logger.debug("MARVIS DEBUG: Calling TroubleshootUtils.device_performance()")  # Trace the call
    ...
    def _invoke_marvis_network_connectivity() -> None:
        ...
        logger.debug("MARVIS DEBUG: Calling TroubleshootUtils.network_connectivity()")  # Trace the call
        TroubleshootUtils.network_connectivity()

    @staticmethod
    def _invoke_marvis_view_insights() -> None:
        """Run the insights viewer."""
        logger.debug("MARVIS DEBUG: Calling TroubleshootUtils.view_insights()")  # Trace the call
    ...
    def launch_interactive() -> None:  # Launch interactive Marvis.
        ...
        mh = SourceDependencyResolver  # WHY: resolve source dependencies without importing the root module.
        logger.info("Entering TroubleshootUtils.launch_interactive")  # Entry envelope for logging compliance
        logger.debug("MARVIS DEBUG: Entering launch_interactive() method")  # Trace the entry.
        TroubleshootUtils._print_marvis_menu()  # Header + divider.
        org_id = mh.ConfigUtils.get_cached_or_prompted_org_id()  # Resolve the org.
        logger.debug("MARVIS DEBUG: Using org_id: %s for Marvis troubleshooting", org_id)  # %s not f-string
        logger.debug("MARVIS DEBUG: Session state - authenticated: %s", mh.apisession is not None)  # %s not f-string
        ...
        logger.debug("MARVIS DEBUG: User selected option: %s", choice)  # %s not f-string
        TroubleshootUtils._dispatch_marvis_choice(choice)  # Route to handler.
        logger.info("Exiting TroubleshootUtils.launch_interactive with choice: %s", choice)  # Exit envelope
