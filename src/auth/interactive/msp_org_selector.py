...
class MspOrgSelector:
    ...
    def select(self) -> None:
        ...
        logger.info("MspOrgSelector.select() starting with %d MSP(s)", len(msp_privileges))  # Trace
        ...
        if not msp_privileges:  # Operator has no MSP grants: skip straight to direct org pick
            logger.debug("No MSP grants present - delegating to direct org selector")  # Trace path
        ...
        if chosen_msp is None:  # User aborted or chose to skip MSP selection
            logger.debug("MSP selection skipped - delegating to direct org selector")  # Trace path
    ...
    def _print_banner() -> None:
        ...
        logger.warning("  SELECT MSP AND ORGANIZATION")  # Banner heading routed via logger
    ...
    def _choose_msp(self, msps: list[dict[str, Any]]) -> dict[str, Any] | None:
        ...
        if len(msps) == 1:  # Auto-pick path preserves the legacy convenience behaviour
            ...
            logger.warning("  Using MSP: %s (only one available)", only["msp_name"])  # Legacy message routed via logger
            logger.debug("Auto-selected the only available MSP: %s", only.get("msp_id"))  # Trace
    ...
    def _prompt_msp(self, msps: list[dict[str, Any]]) -> dict[str, Any] | None:
        ...
        logger.info("Prompting operator to choose MSP from %d options", len(msps))  # Trace
        try:
        ...
        if choice == "":  # Blank input means "skip MSP selection" in the legacy flow
            logger.warning("  Skipping MSP selection - using direct org access")  # Legacy message routed via logger
            return None  # Caller will fall back to direct org selection
        try:
        ...
        if not (0 <= index < len(msps)):  # Out-of-range index falls into the legacy invalid path
            logger.warning("  X Invalid selection - skipping MSP selection")  # Legacy message routed via logger
            return None  # Caller will fall back to direct org selection
        logger.debug("MSP selected at index %d", index)  # Trace the picked index
    ...
    def _render_msp_menu(msps: list[dict[str, Any]]) -> None:
        ...
        logger.warning("  Available MSPs:")  # Legacy header routed via logger
        for index, msp in enumerate(msps, start=1):  # 1-based numbering matches legacy UI
            ...
            logger.warning("    %d. %s (role: %s)", index, msp_name, msp_role)  # Legacy format routed via logger
        logger.warning("")  # Blank spacer matches legacy output exactly
    ...
    def _select_org_under_msp(self, msp: dict[str, Any]) -> None:
        ...
        msp_name = msp.get("msp_name", "Unknown")  # Preserve legacy fallback label
        logger.warning("  + Selected MSP: %s", msp_name)  # Legacy console message routed via logger
        logger.warning("  Fetching organizations under %s...", msp_name)  # Legacy console message routed via logger
        ...
        if apisession is None:  # Defensive guard preserved from the legacy code path
            logger.warning("  X API session not initialized")  # Legacy console message routed via logger
            logger.error("API session not initialized when selecting MSP org")  # Legacy error log
    ...
    def _fetch_msp_orgs(self, apisession: Any, msp_id: str) -> list[dict[str, Any]] | None:
        ...
        logger.info("Calling listMspOrgs for msp_id=%s", msp_id)  # Trace before SDK call
        ...
        if not response or not hasattr(response, "data"):  # Legacy guard for empty/invalid response
            logger.warning("  X Failed to retrieve MSP organizations")  # Legacy console message routed via logger
            logger.debug("listMspOrgs returned an empty or invalid response")  # Trace bad response
        ...
        if not orgs_data:  # Empty list short-circuits to the legacy "no orgs" message
            logger.warning("  No organizations found under this MSP")  # Legacy console message routed via logger
            logger.debug("listMspOrgs returned an empty org list")  # Trace empty list
            return []  # Caller will short-circuit on empty list
        orgs_data = sorted(orgs_data, key=lambda org: org.get("name", "").lower())  # Stable sort
        logger.warning("  Found %d organization(s):", len(orgs_data))  # Legacy console message routed via logger
        logger.warning("")  # Blank spacer matches legacy output exactly
        logger.debug("listMspOrgs returned %d orgs (sorted by name)", len(orgs_data))  # Trace
    ...
    def _resolve_mistapi(self) -> Any:
        ...
        logger.info("Resolving mistapi SDK via fallback import (MSP org fetch)")  # Trace import
    ...
    def _render_page(
        ...
        if total_pages > 1:  # Multi-page mode prints the page counter + nav hint
            logger.warning("  Page %d/%d", current_page + 1, total_pages)  # Legacy page indicator routed via logger
            logger.warning(
                "  Enter number to select, 'n' for next page, 'p' for previous, 'q' to skip"
            )  # Legacy hint routed via logger
        else:
            logger.warning("  Enter number to select, or 'q' to skip")  # Legacy single-page hint routed via logger
    ...
    def _interpret_choice(
        ...
        if choice in {"", "q"}:  # Blank or 'q' both mean "skip" in the legacy flow
            logger.warning("  Skipping org selection")  # Legacy console message routed via logger
        ...
        logger.warning("  X Invalid number - try again")  # Out-of-range index routed via logger
        return ("nav", current_page, None)  # Stay on the same page and loop again
    ...
    def _record_org_selection(self, msp_name: str, org: dict[str, Any]) -> None:
        ...
        logger.warning("")  # Blank spacer matches legacy output exactly
        logger.warning("  + Selected organization: %s", selected_org_name)  # Legacy console message routed via logger
        logger.warning("  + Organization ID: %s", selected_org_id)  # Legacy console message routed via logger
        logger.info(  # Legacy info log preserved verbatim
