...
class CredentialPrompter:
    ...
    def prompt_email(self) -> str | None:
        ...
        logger.info("Prompting user for login email")  # Trace prompt start
        try:
        ...
        if not email:  # Treat blank as a hard validation failure
            logger.warning("X Email is required")  # Legacy console message routed via logger
            logger.debug("Email prompt returned empty string")  # Trace validation failure
            return None  # Caller will short-circuit the login
        logger.debug("Email prompt accepted")  # Trace acceptance without leaking the value
    ...
    def prompt_password(self) -> str | None:
        ...
        logger.info("Prompting user for login password (masked)")  # Trace prompt start
        try:
            ...
            return None  # Signal caller to cancel the login flow
        except OSError as password_error:  # Closed or unavailable terminal streams surface as OS errors.
        ...
        if not password:  # Treat blank password as validation failure
            logger.warning("X Password is required")  # Legacy console message routed via logger
            logger.debug("Password prompt returned empty string")  # Trace validation failure
            return None  # Caller will short-circuit the login
        logger.debug("Password prompt accepted (length=%d)", len(password))  # Trace length only
    ...
    def prompt_two_factor(self) -> str | None:
        ...
        logger.info("Prompting user for 2FA verification code")  # Trace prompt start
        try:
        ...
        if not code:  # Empty 2FA code is treated as a hard failure by the legacy flow
            logger.warning("  X 2FA code is required")  # Legacy console message routed via logger
            logger.debug("2FA prompt returned empty string")  # Trace validation failure
            return None  # Caller will short-circuit the login
        logger.debug("2FA prompt accepted")  # Trace acceptance without leaking the value
