...
class APProfileMigrationManager:  # Preserve the existing behavior during the compliance refactor.
    ...
    def _reassign_one_ap(  # Preserve the existing behavior during the compliance refactor.
        ...
        body = {"deviceprofile_id": target_profile_id}  # Preserve the existing behavior during the compliance refactor.
        last_exc: BaseException | None = None  # Preserve the existing behavior during the compliance refactor.
        ...
        for attempt in range(
            ...
            try:
                ...
                last_exc = exc  # Preserve the existing behavior during the compliance refactor.
                ...
                raise  # Preserve the existing behavior during the compliance refactor.
        # WHY: unreachable; guard against typing lint anyway.
        if last_exc is not None:  # Preserve the existing behavior during the compliance refactor.
            raise last_exc  # Preserve the existing behavior during the compliance refactor.
    ...
    def _revert_one_ap(  # Preserve the existing behavior during the compliance refactor.
        ...
        body = {"deviceprofile_id": source_profile_id}  # Preserve the existing behavior during the compliance refactor.
        last_exc: BaseException | None = None  # Preserve the existing behavior during the compliance refactor.
        for attempt in range(
            len(_RETRY_BACKOFF_SECONDS) + 1
        ):  # Preserve the existing behavior during the compliance refactor.
            ...
            if (
                ...
                last_exc = RuntimeError(
                    f"HTTP {status} on updateSiteDevice for {device_id}"
                )  # Preserve the existing behavior during the compliance refactor.
                if attempt < len(
                    _RETRY_BACKOFF_SECONDS
                ):  # Preserve the existing behavior during the compliance refactor.
