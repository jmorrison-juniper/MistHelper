...
class SpecKitAnalyzer:
    ...
    def _artifact_texts(self, feature_dir: Path) -> dict[str, str]:
        ...
        logging.info("Reading SpecKit artifacts for analysis")  # Record bulk artifact read.
        texts = {name: self._read_optional(feature_dir / name) for name in self.REQUIRED_FILES}
        logging.debug("Read %d SpecKit artifact text entries", len(texts))  # Record artifact count.
        return texts

    def _read_optional(self, path: Path) -> str:
        """Read an artifact when it exists."""
        logging.info("Reading optional SpecKit artifact %s", path)  # Record artifact read.
    ...
    def _missing_file_findings(self, feature_dir: Path, require_analysis: bool) -> list[AnalysisFinding]:
        ...
        logging.info("Checking required SpecKit command artifacts")  # Record required artifact check.
        required = (
            self.REQUIRED_FILES
            if require_analysis
        ...
        logging.debug("Found %d missing required artifacts", len(findings))  # Record missing artifact count.
        return findings
    ...
    def _command_findings(self, texts: dict[str, str], require_analysis: bool) -> list[AnalysisFinding]:
        ...
        logging.info("Checking the full SpecKit command sequence")  # Record sequence validation start.
        ...
        logging.debug("Found %d missing SpecKit command records", len(findings))  # Record sequence gap count.
        return findings

    def _metadata_findings(self, texts: dict[str, str]) -> list[AnalysisFinding]:
        """Return findings for inconsistent measured values."""
        logging.info("Checking measured metadata across artifacts")  # Record metadata validation start.
        ...
        logging.debug("Found %d measured metadata gaps", len(findings))  # Record metadata gap count.
        return findings
    ...
    def _finding(self, index: int, category: str, severity: str, value: str) -> AnalysisFinding:
        ...
        logging.info("Creating a general analysis finding")  # Record finding creation.
        ...
        logging.debug("Created general finding %s", finding.finding_id)  # Record the finding id.
        return finding
