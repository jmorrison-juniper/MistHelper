...
class StageMeasurementReader:
    ...
    def document_from_factory(self, document_key: str, domain: str, skill_name: str) -> DocumentRecord:
        ...
        logging.info("Reading document identity from the factory database")  # Record the database read.
        ...
        logging.debug("Read factory identity for document key %s", document_key)  # Record safe identity.
        return document
    ...
    def details(self, inputs: StageMeasurementInputs) -> dict[str, object]:
        ...
        logging.info("Reading measured audit values for a stage comment")  # Record the measurement action.
        ...
        logging.debug("Built measured audit details for document key %s", inputs.document_key)  # Record key.
        return details
    ...
    def _apply_extraction(self, details: dict[str, object], result: DepthExtractionResult | None) -> None:
        ...
        if result is None:  # Do not invent extraction values when the stage lacks a result object.
            logging.debug("Extraction result not available for audit details")  # Record the missing evidence.
            return
    ...
    def _apply_package(self, details: dict[str, object], package_dir: Path | None, document_key: str) -> None:
        ...
        if package_dir is None or not package_dir.exists():  # Do not report disk values without a package.
            logging.debug("Built package not available for audit details")  # Record the missing evidence.
            return
    ...
    def _apply_guard(self, details: dict[str, object], report: SimilarityGuardReport | None) -> None:
        ...
        if report is None:  # Do not invent a guard result for early stages.
            logging.debug("Similarity guard report not available for audit details")  # Record missing evidence.
            return
    ...
    def _apply_ste(self, details: dict[str, object], report: SteValidationReport | None) -> None:
        ...
        if report is None:  # Do not invent a score before STE validation runs.
            logging.debug("STE validation report not available for audit details")  # Record missing evidence.
            return
    ...
    def _topic_paths(self, package_dir: Path) -> tuple[Path, ...]:
        ...
        logging.debug("Read %d topic files from %s", len(paths), package_dir)  # Record the disk count.
        return paths
    ...
    def _source_text_chars(self, document_key: str) -> int:
        ...
        logging.info("Reading source text size from the factory database")  # Record the metric read.
        ...
        logging.debug("Read %d source text characters for %s", value, document_key)  # Record safe metric.
        return value
