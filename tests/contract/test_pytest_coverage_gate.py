...
class CoverageGateWorkflow:
    ...
    def __init__(self, workflow_path: Path) -> None:
        ...
        logger.info("Storing the workflow path %s", workflow_path)  # Record the workflow path before use.
        self.workflow_path = workflow_path  # Keep the path as state for repeated reads.
        logger.debug("Stored workflow path %s", self.workflow_path)  # Confirm the path that later checks read.

    def load(self) -> dict[str, Any]:
        """Return the parsed workflow mapping."""
        logger.info("Reading workflow file %s", self.workflow_path)  # Record the file read before it starts.
        document = yaml.safe_load(self.workflow_path.read_text(encoding="utf-8"))  # Parse the workflow safely.
        logger.debug("Parsed workflow with %d top-level keys", len(document))  # Record the parsed shape.
    ...
    def job(self, job_name: str) -> dict[str, Any]:
        ...
        logger.info("Reading the workflow job %s", job_name)  # Record the job lookup before it starts.
        jobs = self.load()["jobs"]  # Read all jobs from the parsed workflow.
        job = jobs[job_name]  # Select the expected job by stable identifier.
        logger.debug("The workflow job %s has %d keys", job_name, len(job))  # Record the selected job shape.
...
def fixture_coverage_workflow() -> CoverageGateWorkflow:
    ...
    logger.info("Creating the coverage workflow reader")  # Record fixture construction.
    reader = CoverageGateWorkflow(_WORKFLOW_PATH)  # Create the reader around the workflow path.
    logger.debug("Created the coverage workflow reader")  # Confirm fixture construction.
...
class TestCoverageGateShards:
    ...
    def test_shards_run_in_parallel(self, coverage_workflow: CoverageGateWorkflow) -> None:
        ...
        logger.info("Checking that the shard job uses a matrix")  # Record the workflow assertion.
        ...
        logger.debug("The shard matrix contains %d entries", len(matrix))  # Record the shard count.
    ...
    def test_each_shard_uploads_hidden_coverage_data(self, coverage_workflow: CoverageGateWorkflow) -> None:
        ...
        logger.info("Checking the shard artifact upload")  # Record the artifact assertion.
        shard_job = coverage_workflow.job("pytest_coverage_shards")  # Read the shard job from the workflow.
        upload_step = next(
            step for step in shard_job["steps"] if step.get("name") == "Upload coverage data"
        )  # Find upload.
        logger.debug("The upload step uses %s", upload_step["uses"])  # Record the action version.
    ...
    def test_shards_do_not_apply_the_final_threshold(self, coverage_workflow: CoverageGateWorkflow) -> None:
        ...
        logger.info("Checking that shards disable per-shard coverage failure")  # Record the threshold assertion.
        shard_job = coverage_workflow.job("pytest_coverage_shards")  # Read the shard job from the workflow.
        test_step = next(
        ...
        logger.debug("The shard test step contains %d characters", len(command))  # Record command size, not content.
    ...
    def test_final_gate_keeps_required_check_name(self, coverage_workflow: CoverageGateWorkflow) -> None:
        ...
        logger.info("Checking the final coverage gate name")  # Record the required-check assertion.
        final_job = coverage_workflow.job("pytest")  # Read the final job that branch protection sees.
        logger.debug("The final coverage job name is %s", final_job["name"])  # Record the check name.
    ...
    def test_final_gate_combines_reports_once(self, coverage_workflow: CoverageGateWorkflow) -> None:
        ...
        logger.info("Checking the coverage combine command")  # Record the final command assertion.
        ...
        logger.debug("The combine step contains %d characters", len(command))  # Record command size, not content.
