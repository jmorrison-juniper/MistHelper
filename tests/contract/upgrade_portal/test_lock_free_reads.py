...
class CountingLockStore:
    ...
    def _record(self, name: str) -> None:
        ...
        if self.fail:  # `contracts/site-lock.md:138` asks a read to answer anyway.
...
# ---------------------------------------------------------------------------
# A read while a different operator holds the site. `contracts/site-lock.md:159`.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# A read while the lock store is down. `contracts/site-lock.md:138`.
# ---------------------------------------------------------------------------
...
def test_a_down_lock_store_still_lets_a_read_answer(
    ...
    Why:
        `contracts/site-lock.md:138` asks a read-only page to show the page and
...
def test_the_history_page_reads_while_another_operator_holds_the_site(
    ...
    assert response.status_code == OK_STATUS  # `contracts/site-lock.md:159` names this page by hand.
