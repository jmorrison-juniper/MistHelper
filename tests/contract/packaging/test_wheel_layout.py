...
def fixture_wheel_target() -> dict[str, Any]:
    ...
    logger.info("Reading the wheel target from %s", _PYPROJECT)  # Report the read before the work.
    config = tomllib.loads(_PYPROJECT.read_text(encoding="utf-8"))
    target = config["tool"]["hatch"]["build"]["targets"]["wheel"]
    logger.debug("The wheel target holds %d keys", len(target))  # Record the shape after the read.
    return dict(target)
...
def test_the_wheel_never_packs_the_repository_root(wheel_target: dict[str, Any]) -> None:
    ...
    """
    logger.info("Checking that the wheel names no root package")  # Report the plan.
...
def test_the_wheel_ships_every_import_root(wheel_target: dict[str, Any], name: str) -> None:
    ...
    logger.info("Checking that the wheel ships %s", name)  # Report the plan.
...
def test_the_wheel_ships_no_development_folder(wheel_target: dict[str, Any], name: str) -> None:
    ...
    logger.info("Checking that the wheel skips %s", name)  # Report the plan.
...
def test_the_wheel_ships_every_root_module(wheel_target: dict[str, Any], name: str) -> None:
    ...
    """
    logger.info("Checking that the wheel ships the root module %s", name)  # Report the plan.
...
def test_every_console_script_names_a_shipped_package() -> None:
    ...
    """
    logger.info("Checking every console script against the shipped packages")  # Report the plan.
    config = tomllib.loads(_PYPROJECT.read_text(encoding="utf-8"))
    target = config["tool"]["hatch"]["build"]["targets"]["wheel"]
    ...
    logger.debug("Checked %d console scripts", len(config["project"]["scripts"]))  # Record the count.
