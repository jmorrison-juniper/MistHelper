...
class SequencedStatisticsReader:
    ...
    def __init__(self, schedule: Sequence[Mapping[str, gate.GateReading]]) -> None:
        ...
        self.calls = 0  # Tests read this count to prove the wait length.
...
def test_a_scheduled_reboot_timeout_marks_the_post_reboot_failure() -> None:
    ...
    harness = Harness(
        events, FakeStatisticsReader(stale), deadline_seconds=80, schedule_anchor=START_TIME + 60.0
    )  # Bound.
    outcome = harness.adapter.settle(
        RUN_ID, "switches", [target_entry(SWITCH_MAC)]
    )  # Run until the scheduled window ends.
...
def test_a_scheduled_reboot_before_the_ceiling_is_not_a_return_failure(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    harness = Harness(
        FakeReconnectReader(), FakeStatisticsReader(), deadline_seconds=80, schedule_anchor=START_TIME + 200.0
    )  # Future.
    outcome = harness.adapter.settle(
        RUN_ID, "switches", [target_entry(SWITCH_MAC)]
