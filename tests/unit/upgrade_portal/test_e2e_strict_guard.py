...
def test_the_guard_stays_quiet_when_strict_mode_is_off(
    ...
    logger.info("Checking the guard with strict mode off")  # Report the plan before the work.
...
def test_the_guard_fails_a_run_that_would_skip_every_browser_test(
    ...
    logger.info("Checking the guard with strict mode on")  # Report the plan before the work.
    ...
    logger.debug("The guard reported %s", failure.value)  # Record the message for a failure read.
...
def test_a_missing_parent_package_reports_a_clean_failure(
    ...
    """
    logger.info("Checking the guard against a wholly absent package")  # Report the plan.
    monkeypatch.setenv(guard_module.STRICT_VARIABLE, guard_module.STRICT_ENABLED)  # Turn the guard on.
...
def test_every_browser_module_still_carries_its_own_skip() -> None:
    ...
    logger.info("Checking that every browser module keeps its skip")  # Report the plan.
