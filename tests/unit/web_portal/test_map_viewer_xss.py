...
def _read_template() -> str:
    ...
    logger.info("Reading the map viewer template from %s", TEMPLATE_PATH)
    ...
    logger.debug("Read %d characters from the map viewer template", len(source))
    return source


def _extract_function(source: str, name: str) -> str:
    """Return the body of one JavaScript function from the template text."""
    logger.info("Extracting the body of the function %s", name)
    ...
    for index in range(brace_start, len(source)):
        ...
        # Leave a block.
            ...
            if depth == 0:
                ...
                logger.debug("Extracted %d characters for %s", len(body), name)
                return body
...
def _innerhtml_assignments(body: str) -> list[str]:
    ...
    logger.info("Collecting the innerHTML assignments in the function body")
    ...
    logger.debug("Found %d innerHTML assignments in the function body", len(matches))
    return matches
...
class TestDeviceListDoesNotUseInnerHtml:
    ...
    def test_no_untrusted_field_reaches_innerhtml(self) -> None:
        ...
        logger.info("Checking that no device field reaches innerHTML")
        ...
        logger.debug("No device field reaches innerHTML")

    def test_only_the_empty_string_clears_innerhtml(self) -> None:
        """The one allowed innerHTML assignment clears the list."""
        logger.info("Checking that innerHTML is only assigned an empty string")
        # Read the template and narrow the search to the device list builder.
        body = _extract_function(_read_template(), TARGET_FUNCTION)
        ...
        logger.debug("Every innerHTML assignment writes an empty string")
...
class TestDeviceListUsesTextContent:
    ...
    def test_the_builder_sets_textcontent(self) -> None:
        ...
        logger.info("Checking that the device list builder uses textContent")
        # Read the template and narrow the search to the device list builder.
        body = _extract_function(_read_template(), TARGET_FUNCTION)
        ...
        logger.debug("The device list builder uses textContent")

    def test_the_device_label_uses_textcontent(self) -> None:
        """The device name and the device type both reach textContent."""
        logger.info("Checking that each device field reaches textContent")
        # Read the template and narrow the search to the device list builder.
        body = _extract_function(_read_template(), TARGET_FUNCTION)
        ...
        logger.debug("Each device field reaches textContent")
...
class TestTheGuardStillMatchesTheFile:
    ...
    def test_the_template_exists(self) -> None:
        ...
        logger.info("Checking that the map viewer template exists")
    ...
    def test_the_safe_convention_holds_elsewhere(self) -> None:
        ...
        logger.debug("The site list still uses textContent")
