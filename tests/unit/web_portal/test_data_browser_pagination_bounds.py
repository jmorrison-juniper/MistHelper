...
def browser(tmp_path):
    ...
    logger.info("Building the data browser fixtures in %s", tmp_path)
    ...
    logger.debug("Built four fixtures that each hold %d rows", TOTAL_ROWS)
...
class TestSqlitePageSizeIsBounded:
    ...
    def test_out_of_range_size_returns_at_most_the_cap(self, browser, per_page) -> None:
        ...
        logger.info("Requesting the SQLite preview with per_page=%d", per_page)
        ...
        logger.debug("per_page=%d returned %d rows", per_page, len(result["rows"]))
    ...
    def test_out_of_range_size_with_a_search_returns_at_most_the_cap(self, browser, per_page) -> None:
        ...
        logger.info("Requesting the SQLite search preview with per_page=%d", per_page)
        ...
        logger.debug("The search branch returned %d rows", len(result["rows"]))
...
class TestFilePageSizeIsBounded:
    ...
    def test_out_of_range_size_returns_at_most_the_cap(self, browser, name, per_page) -> None:
        ...
        logger.info("Requesting the preview of %s with per_page=%d", name, per_page)
        ...
        logger.debug("%s returned %d rows", name, len(result["rows"]))
...
class TestPageNumberIsBounded:
    ...
    def test_a_low_page_number_returns_the_first_page(self, browser, page) -> None:
        ...
        logger.info("Requesting the SQLite preview with page=%d", page)
        ...
        logger.debug("page=%d reported page %d", page, result["page"])
...
class TestNormalRequestsStillWork:
    ...
    def test_a_normal_page_size_is_unchanged(self, browser) -> None:
        ...
        logger.info("Requesting the SQLite preview with a normal page size")
        ...
        logger.debug("A normal page size returned %d rows", len(result["rows"]))

    def test_the_second_page_follows_the_first(self, browser) -> None:
        """Paging still walks through the table in order."""
        logger.info("Requesting the second page of the SQLite preview")
        ...
        logger.debug("The second page starts at row %d", result["rows"][0][0])

    def test_the_minimum_page_size_returns_one_row(self, browser) -> None:
        """The lower bound is a single row and not an empty page."""
        logger.info("Requesting the SQLite preview with the minimum page size")
        ...
        logger.debug("The minimum page size returned %d rows", len(result["rows"]))
...
class TestTheBoundsAreNamed:
    ...
    def test_the_bounds_hold_sane_values(self) -> None:
        ...
        logger.info("Checking the page size bounds")
