...
class TestCombinedInventoryHelpers:
    ...
    def test_partition_combined_inventory_rows_identifies_empty_vc_shells(self):
        ...
        site_configs, empty_vc_shells, duplicate_vc_entries = (
            MistHelper.OrgInventoryExporter._partition_combined_inventory_rows(all_devices)
        )
    ...
    def test_build_combined_inventory_weekly_data_groups_rows(self):
        ...
        weekly_data, summary_data = MistHelper.OrgInventoryExporter._build_combined_inventory_weekly_data(
            site_configs, "Customer", "ACC-1"
        )
...
class TestCombinedInventoryOrchestration:
    ...
    def test_combined_inventory_with_site_info_calls_helpers(self, monkeypatch):
        ...
        monkeypatch.setattr(
            MistHelper.os,
            "getenv",
            lambda name: {"END_CUSTOMER_NAME": "Cust", "END_CUSTOMER_ACCOUNT_ID": "Acct"}.get(name),
        )
        ...
        monkeypatch.setattr(
            ...
            lambda current_org_id, fallback_org_name: "Org",
        )
        monkeypatch.setattr(MistHelper.OrgInventoryExporter, "_build_safe_org_name", lambda name: "Org")
        monkeypatch.setattr(
            MistHelper.OrgInventoryExporter,
            "devices_with_site_info",
            lambda: helper_calls.__setitem__("refresh", helper_calls["refresh"] + 1),
        )
        monkeypatch.setattr(
            MistHelper.OrgInventoryExporter,
            "_export_combined_inventory_raw_json",
            lambda output_folder, current_org_id: helper_calls.__setitem__("json", helper_calls["json"] + 1),
        )
        monkeypatch.setattr(
            MistHelper.OrgInventoryExporter,
            "_load_combined_inventory_rows",
            lambda: [
                {
                    "created_time": "1714521600",
                    "serial": "SN1",
        ...
        monkeypatch.setattr(
            MistHelper.OrgInventoryExporter, "_partition_combined_inventory_rows", lambda rows: (rows, [], 0)
        )
        monkeypatch.setattr(
            MistHelper.OrgInventoryExporter,
            "_log_combined_inventory_vc_summary",
            lambda all_devices, site_configs, empty_vc_shells, duplicate_vc_entries: None,
        )
        monkeypatch.setattr(
            MistHelper.OrgInventoryExporter,
            "_build_combined_inventory_weekly_data",
            lambda site_configs, end_customer_name, end_customer_account_id: (
                {"2024_Week_18": [{"x": 1}]},
                {(2024, 18): 1},
            ),
        )
        monkeypatch.setattr(
            MistHelper.OrgInventoryExporter,
            "_write_combined_inventory_weekly_csvs",
            lambda output_folder, fieldnames, weekly_data: helper_calls.__setitem__(
                "weekly", helper_calls["weekly"] + 1
            ),
        )
        monkeypatch.setattr(
            MistHelper.OrgInventoryExporter,
            "_write_combined_inventory_summary",
            lambda output_folder, summary_data: helper_calls.__setitem__("summary", helper_calls["summary"] + 1),
        )
        monkeypatch.setattr(
            MistHelper.OrgInventoryExporter,
            "_write_combined_inventory_master_csv",
            lambda output_folder, safe_org_name, site_configs: (
                helper_calls.__setitem__("master", helper_calls["master"] + 1)
                or ("Org_CombinedInventory_Master.csv", 1)
            ),
        )
