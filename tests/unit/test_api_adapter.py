...
def make_mistapi_for_sites(data):
    ...
    listOrgSites = lambda apisession, org, limit: DummyResp(data)
...
def make_mistapi_for_wlans(data):
    ...
    listSiteWLANS = lambda apisession, site_id, limit: DummyResp(data)
...
class TestMistApiAdapter(unittest.TestCase):
    def test_get_sites_returns_sites(self):
        ...
        adapter = MistApiAdapter(apisession=object(), mistapi_module=mistapi, org_id="org1", max_retries=0, retry_delay=0.0)
    ...
    def test_get_sites_requires_org(self):
        data = [{"id": "s1"}]
        mistapi = make_mistapi_for_sites(data)
        adapter = MistApiAdapter(apisession=object(), mistapi_module=mistapi, org_id=None, max_retries=0, retry_delay=0.0)
