...
class TestMistHelperProxy:
    ...
    def test_getattr_returns_module_attribute(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        logger.info("test: publishing sentinel attribute on MistHelper module")  # WHY: pre-action log.
        ...
        logger.debug("test: proxy returned attribute; identity=%s", id(result))  # WHY: post-action log.
    ...
    def test_module_level_singleton_is_proxy(self) -> None:
        ...
        logger.info("test: asserting module-level _MH is a proxy instance")  # WHY: pre-action log.
        assert _MH is SourceDependencyResolver  # WHY: guard against accidental rebinding.
        logger.debug("test: module-level _MH proxy identity confirmed")  # WHY: post-action log.
...
def _publish_interactive_helpers(
    ...
    """
    logger.info(  # WHY: pre-action log — publishing helper stubs.
    ...
    logger.debug(  # WHY: post-action log — helpers published.
...
class TestInitializeInteractive:
    ...
    def test_happy_path_returns_true(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        logger.info("test: happy-path initialize() expecting True")  # WHY: pre-action log.
        ...
        logger.debug("test: initialize() returned %s", result)  # WHY: post-action log.
    ...
    def test_failed_login_returns_false(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        logger.info("test: failed-login initialize() expecting False")  # WHY: pre-action log.
    ...
    def test_falsy_execute_result_coerces_to_false(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        logger.info("test: falsy-return initialize() expecting False via bool() coercion")  # pre-action.
        mocks = _publish_interactive_helpers(monkeypatch, execute_returns=None)  # WHY: None is falsy.
        result = MistSessionInteractiveInitializer.initialize()  # WHY: SUT invocation.
        logger.debug("test: initialize() returned %s (coerced from None)", result)  # post-action.
    ...
    def test_orchestrator_receives_injected_deps(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        logger.info("test: verifying orchestrator constructor kwargs wiring")  # pre-action.
        mocks = _publish_interactive_helpers(monkeypatch, execute_returns=True)  # WHY: wire helpers.
        ...
        logger.debug("test: orchestrator kwargs=%s", sorted(call_kwargs.keys()))  # post-action.
    ...
    def test_detect_msp_adapter_calls_extracted_detector(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        logger.info("test: exercising _detect_msp_for_login DI adapter")  # pre-action.
        mocks = _publish_interactive_helpers(monkeypatch, execute_returns=True)  # WHY: wire helpers.
        ...
        logger.debug("test: adapter returned %s", result)  # post-action.
