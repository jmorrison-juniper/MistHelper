...
def test_no_new_test_imports_of_selected_legacy_facades() -> None:  # Enforce migration away from legacy facades.
    ...
    for python_file in (repository_root / "tests").rglob("*.py"):  # Scan only tests to control US3 migration drift.
        ...
        if relative_path == "tests/unit/test_no_new_legacy_facade_imports.py":  # Exclude this guard test file.
            continue
        if relative_path in allowed_existing_references:  # Skip known temporary exceptions.
            continue
        content_text = python_file.read_text(encoding="utf-8", errors="ignore")  # Read safely; tolerate bad chars.
    ...
    assert not violations, f"New legacy facade test references detected: {violations}"  # Fail if new legacy refs added.
