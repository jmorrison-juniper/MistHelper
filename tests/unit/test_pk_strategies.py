...
class TestEndpointPrimaryKeyStrategies:
    ...
    def test_all_types_are_valid(self):
        for endpoint, config in ENDPOINT_PRIMARY_KEY_STRATEGIES.items():
            assert config["type"] in VALID_PK_TYPES, (
                f"{endpoint} has invalid type '{config['type']}'"
            )
        ...
        for endpoint, config in ENDPOINT_PRIMARY_KEY_STRATEGIES.items():
            for key in config["primary_key"]:
                assert isinstance(key, str), (
                    f"{endpoint} primary_key contains non-string: {key}"
                )

    def test_indexes_is_a_list(self):
        for endpoint, config in ENDPOINT_PRIMARY_KEY_STRATEGIES.items():
            assert isinstance(config["indexes"], list), (
                f"{endpoint} indexes is not a list"
            )
    ...
    def test_description_is_non_empty_string(self):
        for endpoint, config in ENDPOINT_PRIMARY_KEY_STRATEGIES.items():
            ...
            assert isinstance(desc, str) and len(desc) > 0, (
                f"{endpoint} has empty or non-string description"
            )
    ...
    def test_natural_pk_uses_id_field(self):
        ...
        for endpoint, config in ENDPOINT_PRIMARY_KEY_STRATEGIES.items():
            if config["type"] == "natural_pk":
                pk = config["primary_key"]
                assert len(pk) == 1, (
                    f"{endpoint} natural_pk should have exactly 1 primary key, got {pk}"
                )
    ...
    def test_composite_pk_has_multiple_keys(self):
        ...
        for endpoint, config in ENDPOINT_PRIMARY_KEY_STRATEGIES.items():
            if config["type"] == "composite_pk":
                pk = config["primary_key"]
                assert len(pk) >= 2, (
                    f"{endpoint} composite_pk should have >=2 keys, got {pk}"
                )
