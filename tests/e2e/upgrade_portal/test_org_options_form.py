...
def test_refused_review_moves_the_message_into_view(page: Any) -> None:
    """A refused Review must focus the flash and hide internal field names."""
    _open_org_options(page)  # Open the multi-site form with the stand-in sites.
    ...
    sync_api.expect(flash).to_contain_text("Target version")  # The message must name a page label.
    ...
    assert "Target version" in active_text  # Focus must move to the message so the refusal is in view.
