...
def stand_in_capture(capture_id: str, role: str, version: str, started_at: str) -> dict[str, Any]:
    ...
    return {
        ...
        "devices": records,
        "clients": {
            "wired": [{"mac": "aabbcc000088", "hostname": "e2e-wired", "port_id": "ge-0/0/2"}],
            "wireless": clients,
            "guest": [{"mac": "aabbcc000099", "hostname": "e2e-guest", "ssid": "guest"}],
        },
        "extras": {
            "switch_ports": [{"mac": str(records[0]["mac"]), "port_id": "ge-0/0/1", "up": True}],
            "poe": [{"mac": str(records[0]["mac"]), "port_id": "ge-0/0/1", "power": 4.2}],
            "radios": [{"mac": str(records[0]["mac"]), "band": "5", "channel": 36}],
            "tunnels": [{"name": "e2e-tunnel", "status": "up"}],
            "bgp_peers": [{"neighbor": "192.0.2.1", "state": "established"}],
            "alarms": [{"id": "e2e-alarm", "type": "switch_down", "severity": "warn"}],
        },
        "counts": {
            "devices_total": len(records),
            "clients_wired": 1,
            "clients_wireless": len(clients),
            "clients_guest": 1,
        },
        "partial_reasons": [],
    }
