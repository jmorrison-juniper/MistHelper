...
What this module skips and what it fails:
    ...
    absent, and when a page needs a session that no browser test can hold. It
    reports a failure when a page answers 200 and the identifier contract does
    not hold. A missing part of the environment never reports a pass, and a
    broken page never reports a skip.
...
def _lazy_fixture(request: pytest.FixtureRequest, name: str) -> Any:
    ...
    Why:
        The server fixture needs a portal on port 8056 and the page fixture
        needs a browser binary. Neither part exists on every workstation. A
        plain request would report an error, which reads in a report as a broken
        test. A skip that carries the real cause reads as the missing part that
        it is.
...
def _require_built_route(status: int, path: str) -> None:
    ...
    Why:
        ...
        route is an environment gap and a page that needs a session is one as
        well, because no browser test may sign in against a real cloud. Any
    ...
    if status == UNAUTHORIZED_STATUS:  # `identity.require_session` refused the request.
        pytest.skip(f"{path} answered 401. No browser test signs in, so no session reaches this page.")
