...
class MistapiGuardReport:
    ...
    def failure_messages(self) -> tuple[str, ...]:
        ...
        messages.extend(self.failure_messages_for_unresolved_baseline(UNRESOLVED_CALL_SITE_BASELINE))  # Add coverage.
...
def test_guard_fails_for_deliberately_missing_sdk_function() -> None:
    missing_path = Path("src") / "example.py"  # Build a synthetic path that citation lint will not resolve.
    ...
    assert report.failure_messages == (
        f"FAIL {missing_path.as_posix()}:2: missing mistapi.api.v1.orgs.fake.missingFunction",
    )  # Prove failure.
...
def test_guard_fails_when_unresolved_call_sites_exceed_baseline() -> None:
    ...
    report = MistapiSdkCompatibilityGuard(REPO_ROOT).evaluate_sources(
        sources, {"api.v1.self.self.getSelf"}
    )  # Evaluate a dynamic module path against a real function surface.
