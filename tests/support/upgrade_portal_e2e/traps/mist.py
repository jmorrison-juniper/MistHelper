"""Fail every Mist connector call from an isolated E2E application."""

from __future__ import annotations  # Keep annotations independent from import order.

import logging  # Record the blocked connector call without a credential value.
...
FAILURE_MESSAGE = "E2E isolation failed: a Mist cloud connector was called."  # Keep the contract text exact.


class MistConnectorTrap:  # Block the Mist cloud boundary during E2E work.
    """Record and refuse every Mist connector call."""
    ...
    def __call__(self, *arguments: Any, **options: Any) -> Any:  # Block one connector construction.
        ...
        del arguments, options  # Never retain a token or a cloud request value.
        logger.info("Block a Mist connector call in the E2E application")  # Record the blocked action.
        self.calls.append("connect")  # Keep a safe stable call name.
        logger.debug("The Mist trap recorded %s call(s)", len(self.calls))  # Report a safe count.
    ...
    def assert_not_called(self) -> None:  # Verify that no connector call crossed the trap.
        """Fail when any Mist connector call was recorded."""
        logger.info("Check the Mist connector trap")  # Record the isolation assertion.
        ...
        logger.debug("The Mist connector trap recorded zero calls")  # Confirm the safe result.
