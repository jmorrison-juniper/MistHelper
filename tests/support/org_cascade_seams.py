...
class ScriptedCascadeStarter:
    ...
    Why:
        ...
        that waits, and it moves the next phase to the waiting state. The last
        call writes the finished state, so the poll stops. A cancellation
        request writes the stopped state, as the real walk does.
    ...
    def __call__(self, record: Mapping[str, Any], deps: Any) -> bool:
        ...
        Args:
            ...
            deps: The collaborators of the walk. The starter reads the store only.
        ...
        with self._guard:  # One step at a time for each process.
            ...
            OrgPhaseStore(deps.store, operation_id, ScriptedCascadeStarter._now_text).update(
                self._advance
            )  # The store writes one step.
    ...
    def _advance(candidate: MutableMapping[str, Any]) -> None:
        ...
        cancellation = candidate.get("cancellation")  # The cancel route sets requested to True.
        if isinstance(cancellation, Mapping) and cancellation.get("requested") is True:  # The operator cancelled.
            OrgPhaseEntries.reset_waiting(candidate)  # No phase waits after the stop.
            OrgPhaseWatch.set_state(candidate, WatchState.STOPPED, STOPPED_NOTE, None)  # The real stop note.
        ...
        ScriptedCascadeStarter._next(candidate)  # Start the next phase, or end the watch.

    @staticmethod
    def _next(candidate: MutableMapping[str, Any]) -> None:
        """Move the first pending phase to the waiting state, or write the finished state."""
        pending = [
        ...
        if not pending:  # Every phase ended.
            reason = OrgPhaseEntries.first_failure(candidate)  # The single-site reason rule.
            OrgPhaseWatch.set_state(candidate, WatchState.FINISHED, FINISHED_NOTE, reason)  # The real end note.
            return  # The watch ended.
