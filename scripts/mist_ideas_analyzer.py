...
:mod:`mist_ideas_analyzer_pkg` package. This script exists only to preserve
the historical ``python scripts/mist_ideas_analyzer.py`` invocation path used
by operators, CI, and documentation. All classification, provisioning, and
reporting logic has been relocated to the package so the compliance analyzer
can score this tiny wrapper in isolation.
"""
...
# WHY: prepend the scripts/ directory so ``import mist_ideas_analyzer_pkg`` works when the
# script is invoked directly (``python scripts/mist_ideas_analyzer.py``) without an
# active editable install; ``__file__.parent`` is the sibling of the package.
sys.path.insert(0, str(Path(__file__).resolve().parent))

from mist_ideas_analyzer_pkg import main  # noqa: E402  # WHY: import after sys.path setup so the package resolves
