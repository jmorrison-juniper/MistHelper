...
`mistapi.api.v1.msps.sso.listMspSsos()`

## Usage Context

Lists all Single Sign-On (SSO) configurations for an MSP. SSO enables MSP administrators to authenticate through their organization's identity provider (IdP) using SAML, reducing credential management overhead.

## Gotchas

- Multiple SSO configurations can coexist for different IdPs or admin groups.
- SSO configurations are MSP-scoped and separate from org-level SSO settings.

## Related Endpoints

- [GET_msps_msp_id_ssos_sso_id.md](GET_msps_msp_id_ssos_sso_id.md) — Get specific SSO config details
- [POST_msps_msp_id_ssos.md](POST_msps_msp_id_ssos.md) — Create a new SSO configuration
- [GET_msps_msp_id_ssoroles.md](GET_msps_msp_id_ssoroles.md) — SSO role mappings
