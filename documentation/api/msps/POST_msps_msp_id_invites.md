...
`mistapi.api.v1.msps.admins.inviteMspAdmin()`

## Usage Context

Sends an invitation email to a new MSP administrator with a specified role and privilege set. The invited admin must accept the invitation to gain access to MSP management functions.

## Gotchas

- Invitations expire after a set period — check the pending invitations list if the admin reports not receiving the invite.
- The invited email address must not already be an active admin on this MSP.

## Related Endpoints

- [PUT_msps_msp_id_invites_invite_id.md](PUT_msps_msp_id_invites_invite_id.md) — Update a pending invitation
- [DELETE_msps_msp_id_invites_invite_id.md](DELETE_msps_msp_id_invites_invite_id.md) — Cancel a pending invitation
- [GET_msps_msp_id_admins.md](GET_msps_msp_id_admins.md) — List admins including pending invites
