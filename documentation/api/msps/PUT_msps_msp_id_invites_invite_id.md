...
`mistapi.api.v1.msps.admins.updateMspAdminInvite()`

## Usage Context

Updates a pending MSP admin invitation, allowing changes to the invited admin's role or privilege scope before they accept. Use this to correct mistakes in the original invitation.

## Gotchas

- Can only update invitations that have not yet been accepted.
- No known gotchas with the endpoint itself.

## Related Endpoints

- [POST_msps_msp_id_invites.md](POST_msps_msp_id_invites.md) — Create the original invitation
- [DELETE_msps_msp_id_invites_invite_id.md](DELETE_msps_msp_id_invites_invite_id.md) — Cancel the invitation instead
- [GET_msps_msp_id_admins.md](GET_msps_msp_id_admins.md) — List all admins and pending invites
