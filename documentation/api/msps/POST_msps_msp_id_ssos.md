...
`mistapi.api.v1.msps.sso.createMspSso()`

## Usage Context

Creates a new SSO configuration for MSP administrator authentication. Requires SAML IdP metadata including entity ID, SSO URL, and X.509 certificate from the identity provider.

## Gotchas

- The SSO configuration must match the IdP's SAML settings exactly — entity ID and certificate mismatches cause authentication failures.
- Test SSO login after creation to verify the configuration works before enforcing SSO-only access.

## Related Endpoints

- [GET_msps_msp_id_ssos.md](GET_msps_msp_id_ssos.md) — List SSO configs to verify creation
- [GET_msps_msp_id_ssos_sso_id_metadata.md](GET_msps_msp_id_ssos_sso_id_metadata.md) — Download SAML metadata to provide to the IdP
- [POST_msps_msp_id_ssoroles.md](POST_msps_msp_id_ssoroles.md) — Create SSO role mappings after SSO setup
