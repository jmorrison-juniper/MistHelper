...
`mistapi.api.v1.msps.sso.updateMspSso()`

## Usage Context

Updates an existing MSP SSO configuration, such as rotating the IdP certificate, changing the SSO URL, or updating attribute mappings. Use this when the identity provider configuration changes.

## Gotchas

- Updating the certificate or SSO URL can break active SSO sessions — coordinate changes with the IdP administrator.
- Test login after any SSO configuration update.

## Related Endpoints

- [GET_msps_msp_id_ssos_sso_id.md](GET_msps_msp_id_ssos_sso_id.md) — Get current config before updating
- [DELETE_msps_msp_id_ssos_sso_id.md](DELETE_msps_msp_id_ssos_sso_id.md) — Delete the SSO configuration
- [GET_msps_msp_id_ssos_sso_id_failures.md](GET_msps_msp_id_ssos_sso_id_failures.md) — Check for SSO login failures after update
