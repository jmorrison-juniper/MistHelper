...
```json
{
  ...
  "properties": {
    "duration": {
      "maximum": 180.0,
      "type": "integer",
      "description": "recording length in seconds, max is 180. Default value is also 180.",
    ...
    "name": {
      "type": "string",
      "description": "Name of the recording, the name of the sdk client would be a good default choice"
    },
    "sdkclient_id": {
      "type": "string",
...
`mistapi.api.v1.sites.rfdiags.startSiteRecording()`

## Usage Context

Starts a new RF diagnostics (RFdiag) recording session for a site. Captures RF environment data.

## Gotchas

- Only one RFdiag session per AP can run at a time.

## Related Endpoints

- [POST_sites_site_id_rfdiags_rfdiag_id_stop.md](POST_sites_site_id_rfdiags_rfdiag_id_stop.md) — Stop recording
- [GET_sites_site_id_rfdiags.md](GET_sites_site_id_rfdiags.md) — List sessions
