...
>
> The current overrides can be retrieved with the API Call [Get Site Device](GET_sites_site_id_devices_device_id.md). The local overrides will show up separately from the `port_config` in the `local_port_config` so it can be easily identified (and cleared)
