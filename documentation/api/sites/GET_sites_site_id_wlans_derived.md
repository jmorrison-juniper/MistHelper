...
|------|------|----------|---------|------|-------------|
| resolve | boolean | No | False |  | Whether to resolve SITE_VARS |
| wlan_id | string | No |  |  | Filter by WLAN ID |
...
`mistapi.api.v1.sites.wlans.listSiteWlansDerived()`

## Usage Context

Retrieves the effective (derived/resolved) WLAN configurations for a site, merging org WLAN templates with site overrides.

## Gotchas

- Shows WLANs as they are actually broadcast, including template-applied SSIDs.

## Related Endpoints

- [GET_sites_site_id_wlans.md](GET_sites_site_id_wlans.md) — List raw site WLANs
- [../orgs/GET_orgs_org_id_wlans.md](../orgs/GET_orgs_org_id_wlans.md) — Org WLANs

## MistHelper Notes

Not currently used by MistHelper directly. Menu **49** uses `listSiteWlans` (non-derived).
