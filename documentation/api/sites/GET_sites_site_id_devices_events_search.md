...
Searches device events at a site (reboots, config pushes, firmware changes, PoE events, etc.) with filtering by device, event type, and time.

## Gotchas

- Uses cursor-based pagination. Active sites may generate many events.
- Specify `start`/`end` for bounded queries.

## Related Endpoints

- [GET_sites_site_id_devices_events_count.md](GET_sites_site_id_devices_events_count.md) — Count events
