...
Lists all devices (APs, switches, gateways) assigned to a site. Core endpoint for device inventory.

## Gotchas

- **CRITICAL**: Defaults to APs only. Pass `type=all` to include switches and gateways.
- Returns configuration data, not real-time stats. Use `listSiteDevicesStats` for live status.

## Related Endpoints

- [GET_sites_site_id_devices_device_id.md](GET_sites_site_id_devices_device_id.md) — Get specific device
