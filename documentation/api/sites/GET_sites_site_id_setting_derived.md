...
`mistapi.api.v1.sites.setting.getSiteSettingDerived()`

## Usage Context

Retrieves the fully derived site settings, merging org defaults, site-group overrides, and site-level settings into the final effective configuration.

## Gotchas

- This is the definitive view of what settings are actually applied at the site. Use this instead of raw site settings to understand effective behavior.

## Related Endpoints

- [GET_sites_site_id_setting.md](GET_sites_site_id_setting.md) — Raw site settings (overrides only)
- [PUT_sites_site_id_setting.md](PUT_sites_site_id_setting.md) — Update site settings

## MistHelper Notes

Not currently used by MistHelper directly. `getSiteSetting` (non-derived) is used in Menus **4, 18, 103, 118, 119, 120**.
