...
`mistapi.api.v1.sites.device_profiles.listSiteDeviceProfilesDerived()`

## Usage Context

Retrieves the effective (derived/resolved) device profiles for a site, merging org-level profiles with site overrides.

## Gotchas

- Multiple profiles may apply per device type. Check the device’s assigned profile for actual effective config.

## Related Endpoints

- [../orgs/GET_orgs_org_id_deviceprofiles.md](../orgs/GET_orgs_org_id_deviceprofiles.md) — Org device profiles
- [GET_sites_site_id_devices.md](GET_sites_site_id_devices.md) — List site devices

## MistHelper Notes

Not currently used by MistHelper directly. Menu **43** uses `listOrgDeviceProfiles` at org level.
