...
`mistapi.api.v1.sites.psks.createSitePsk()`

## Usage Context

Creates a new Pre-Shared Key (PSK) for a site WLAN. Used for device onboarding with unique per-device keys.

## Gotchas

- PSK passphrase must meet minimum length requirements. SSID must already exist for WLAN-bound PSKs.

## Related Endpoints

- [POST_sites_site_id_psks_import.md](POST_sites_site_id_psks_import.md) — Bulk import
- [GET_sites_site_id_psks.md](GET_sites_site_id_psks.md) — List PSKs

## MistHelper Notes

Used by MistHelper via `createSitePsk` in Menu 108 (PSK creation).
