...
`mistapi.api.v1.sites.network_templates.listSiteNetworkTemplatesDerived()`

## Usage Context

Retrieves the effective (derived/resolved) network template (switch configuration template) for a site.

## Gotchas

- Network templates control switch port profiles, VLANs, and RADIUS settings. The derived view shows the merged result.

## Related Endpoints

- [../orgs/GET_orgs_org_id_networktemplates.md](../orgs/GET_orgs_org_id_networktemplates.md) — Org network templates
- [GET_sites_site_id_setting.md](GET_sites_site_id_setting.md) — Site settings

## MistHelper Notes

Not currently used by MistHelper directly. Menu **45** uses `listOrgNetworkTemplates` at org level.
