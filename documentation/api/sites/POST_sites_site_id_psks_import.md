...
`mistapi.api.v1.sites.psks.importSitePsks()`

## Usage Context

Bulk-imports Pre-Shared Keys (PSKs) for a site from a CSV or JSON payload. Useful for onboarding many devices.

## Gotchas

- Duplicate PSK names are rejected. Validate uniqueness before import.

## Related Endpoints

- [POST_sites_site_id_psks.md](POST_sites_site_id_psks.md) — Create single PSK
- [GET_sites_site_id_psks.md](GET_sites_site_id_psks.md) — List PSKs

## MistHelper Notes

MistHelper uses `importSitePsks` for PSK import operations.
