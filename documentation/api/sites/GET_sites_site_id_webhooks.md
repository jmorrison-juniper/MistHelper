...
`mistapi.api.v1.sites.webhooks.listSiteWebhooks()`

## Usage Context

Lists webhook configurations at a site. Webhooks push real-time event notifications to external systems.

## Gotchas

- Webhook events are site-scoped. For org-level webhooks, use the orgs API.

## Related Endpoints

- [GET_sites_site_id_webhooks_webhook_id.md](GET_sites_site_id_webhooks_webhook_id.md) — Get specific webhook
- [POST_sites_site_id_webhooks.md](POST_sites_site_id_webhooks.md) — Create webhook

## MistHelper Notes

Used by Menu **57** via `listSiteWebhooks` for webhook data export.
