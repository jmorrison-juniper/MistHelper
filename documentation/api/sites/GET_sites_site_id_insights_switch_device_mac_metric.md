...
`GET /api/v1/sites/{site_id}/insights/switch/{device_mac}/{metric}`
...
`mistapi.api.v1.sites.insights.getSiteInsightMetricsForSwitch()`

## Usage Context

Retrieves switch-specific insight metrics for a device (port utilization, PoE consumption, VLAN traffic, STP events).

## Gotchas

- Only applicable to EX series switches. APs and gateways use different insight endpoints.
