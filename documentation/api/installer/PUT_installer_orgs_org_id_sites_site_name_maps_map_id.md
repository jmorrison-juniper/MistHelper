...
`mistapi.api.v1.installer.installer.updateInstallerMap()`

## Usage Context

Use this endpoint to update an existing floor map. Common use cases:

- Updating AP placement positions on a map after physical installation
- Modifying map properties such as scale or orientation

## Gotchas

- Only updates the map metadata; use the image upload endpoint to change the floor plan image
- The `{map_id}` must match an existing map for the specified site

## Related Endpoints

- [GET_installer_orgs_org_id_sites_site_name_maps.md](GET_installer_orgs_org_id_sites_site_name_maps.md) -- List maps to find the map ID
