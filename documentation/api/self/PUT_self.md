...
`mistapi.api.v1.self.account.updateSelf()`

## Usage Context

Use this endpoint to update the current admin's profile information. Common use cases:

- Changing the admin's display name or phone number
- Updating account preferences

## Gotchas

- Cannot change the email address through this endpoint. Use `POST /api/v1/self/update` to initiate an email change, which requires verification
- Only the fields included in the request body are updated; omitted fields remain unchanged

## Related Endpoints

- [GET_self.md](GET_self.md) -- Retrieve current profile before updating
- [DELETE_self.md](DELETE_self.md) -- Delete the admin account entirely
- [POST_self_update.md](POST_self_update.md) -- Change email address (requires verification)
