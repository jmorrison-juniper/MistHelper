...
`mistapi.api.v1.orgs.service_policies.createOrgServicePolicy()`

## Usage Context

Creates a new service policy (application policy) in the organization.

## Gotchas

- Service policies reference services and networks to define traffic handling.

## Related Endpoints

- [GET_orgs_org_id_servicepolicies.md](GET_orgs_org_id_servicepolicies.md) — List policies
- [POST_orgs_org_id_services.md](POST_orgs_org_id_services.md) — Create service

## MistHelper Notes

Service policy listing uses Menu 4 (`listOrgServicePolicies`). Creation is not used directly.
