...
|------|------|----------|---------|------|-------------|
| forward | string | Yes |  |  | Mist portal url to which backend needs to redirect after successful OAuth authorization. **Required** to get the `authorization_url` |
...
`mistapi.api.v1.orgs.linked_applications.getOrgOauthAppLinkedStatus()`

## Usage Context

Retrieves linked accounts for a specific app integration in org settings.

## Gotchas

- App name must be a valid integration type (e.g., `jamf`, `crowdstrike`).
