...
`mistapi.api.v1.orgs.wlans.updateOrgWlan()`

## Usage Context

Updates an existing org-level WLAN configuration.

## Gotchas

- WLAN changes propagate to all sites using the associated template.
- Changing auth type or SSID name may disconnect active clients.
...
WLAN listing uses Menu 48 (`listOrgWlans`). Update is not used directly.
