...
`mistapi.api.v1.orgs.psks.updateOrgPsk()`

## Usage Context

Updates an existing PSK (passphrase, expiration, SSID binding, etc.).

## Gotchas

- Changing the passphrase triggers a rotation window for the old passphrase.
...
PSK listing uses Menu 46 (`listOrgPsks`). Update is not used directly.
