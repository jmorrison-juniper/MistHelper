...
`mistapi.api.v1.orgs.api_tokens.updateOrgApiToken()`

## Usage Context

Updates an existing org API token's name or privileges.

## Gotchas

- The token value itself cannot be changed; only metadata is updatable.
...
API token listing uses Menu 80 (`listOrgApiTokens`). Update is not used directly.
