...
`mistapi.api.v1.orgs.wlans.createOrgWlan()`

## Usage Context

Creates a new org-level WLAN (SSID) configuration.

## Gotchas

- Org-level WLANs are applied through templates to sites.
- SSID names must be unique within the template scope.
...
WLAN creation uses Menu 48, 102, 122 (`createOrgWlan`). WLAN listing uses Menu 48 (`listOrgWlans`).
