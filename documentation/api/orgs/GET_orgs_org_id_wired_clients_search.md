...
|------|------|----------|---------|------|-------------|
| auth_state | string | No |  |  | Authentication state |
| auth_method | string | No |  |  | Authentication method |
| source | string | No |  |  | source from where the client was learned (lldp, mac) |
| site_id | string | No |  |  | Site ID |
| device_mac | string | No |  |  | Device mac (Gateway/Switch) where the client has connected to |
| mac | string | No |  |  | Partial / full MAC address |
| port_id | string | No |  |  | Port id where the client has connected to |
| vlan | integer | No |  |  | VLAN |
| ip | string | No |  |  |  |
| manufacture | string | No |  |  | Client manufacturer |
| text | string | No |  |  | Partial / full MAC address, hostname or username |
...
`mistapi.api.v1.orgs.clients_-_wired.searchOrgWiredClients()`

## Usage Context

Searches for wired clients across the organization.

## Gotchas

- Can filter by mac, hostname, device_mac, port_id, vlan, etc.

## Related Endpoints

- [GET_orgs_org_id_wired_clients_count.md](GET_orgs_org_id_wired_clients_count.md) — Count wired clients
- [GET_orgs_org_id_clients_search.md](GET_orgs_org_id_clients_search.md) — Search all clients
