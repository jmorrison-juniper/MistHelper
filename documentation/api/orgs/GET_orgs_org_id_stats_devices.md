...
| site_id | string | No |  |  |  |
| mac | string | No |  |  |  |
| evpntopo_id | string | No |  |  | EVPN Topology ID |
| evpn_unused | string | No |  |  | If `evpn_unused`==`true`, find EVPN eligible switches which don’t belong to any EVPN Topology yet |
...
`mistapi.api.v1.orgs.stats_-_devices.listOrgDevicesStats()`

## Usage Context

Retrieves device statistics across the entire organization.

## Gotchas

- Returns stats for all device types (APs, switches, gateways).
- Can be very large for organizations with many devices.
