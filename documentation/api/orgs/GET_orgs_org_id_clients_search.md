...
| band | string | No |  |  | Radio band. enum: `24`, `5`, `6` |
| device | string | No |  |  | Device type, e.g. Mac, Nvidia, iPhone |
| os | string | No |  |  | Only available for clients running the Marvis Client app, os, e.g. Sierra, Yosemite, Windows 10 |
| model | string | No |  |  | Only available for clients running the Marvis Client app, model, e.g. "MBP 15 late 2013", 6, 6s, "8+ GSM" |
| ap | string | No |  |  | AP mac where the client has connected to |
| psk_id | string | No |  |  | PSK ID |
| psk_name | string | No |  |  | Only available for clients using PPSK authentication, the Name of the PSK |
| username | string | No |  |  | Only available for clients using 802.1X authentication, partial / full username |
| vlan | string | No |  |  | VLAN |
| ssid | string | No |  |  | SSID |
...
`mistapi.api.v1.orgs.clients_-_wireless.searchOrgWirelessClients()`

## Usage Context

Searches wireless clients across the organization with extensive filtering options.

## Gotchas

- Returns active and recently seen clients.
- Use `limit` to control page size for large result sets.

## Related Endpoints

- [GET_orgs_org_id_clients_count.md](GET_orgs_org_id_clients_count.md) — Client counts
- [GET_orgs_org_id_clients_events_search.md](GET_orgs_org_id_clients_events_search.md) — Client events
