...
`mistapi.api.v1.utilities.pcaps.getOrgCapturingStatus()`

## Usage Context

Checks the status of an active org-level packet capture, including whether it is running, the device being captured, and elapsed time.

## Gotchas

- Returns 404 if no capture is currently active.
- Only one capture can be active per scope at a time.

## Related Endpoints

- [POST_orgs_org_id_pcaps_capture.md](POST_orgs_org_id_pcaps_capture.md) — Start a capture
- [DELETE_orgs_org_id_pcaps_capture.md](DELETE_orgs_org_id_pcaps_capture.md) — Stop the active capture
- [GET_orgs_org_id_pcaps.md](GET_orgs_org_id_pcaps.md) — List completed captures

## MistHelper Notes

Used by Menu **10** (`PacketCaptureManager.start_org_packet_capture`) to check capture status.
