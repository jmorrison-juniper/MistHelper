...
- New package `src/upgrade_portal/` on port 8056 (`CAPTURE_PORT`). Menu 238 and the `--capture-portal` flag both start it (1823-upgrade-capture-portal)
...
## Recent Changes
- 1823-upgrade-capture-portal: New package `src/upgrade_portal/` (outside `web_portal/`, which ruff and mypy exclude) on port 8056; new upgrade seam `src/firmware/upgrade_service.py`; menu 238; 30-second JSON poll instead of server-sent events; Redis site lock.
