...
def check_and_install_dependencies():
    ...
    if missing:
        ...
        try:
            # Try UV first
            result = subprocess.run(["uv", "pip", "install"] + missing, capture_output=True, timeout=120)
            if result.returncode != 0:
                # Fallback to pip
                subprocess.run([sys.executable, "-m", "pip", "install"] + missing, check=True, timeout=120)
...
class StarlinkTUI:
    ...
    def create_header(self) -> Panel:
        ...
        # Timestamp
        timestamp = Text(f" Last Update: {self.last_update.strftime('%Y-%m-%d %H:%M:%S')} ", style="dim white")

        header_line = Text.assemble(header_text, " ", status, " ", timestamp)

        return Panel(Align.center(header_line), box=box.DOUBLE, style="bold cyan", padding=(0, 1))
    ...
    def create_connection_panel(self) -> Panel:
        ...
        return Panel(table, title="[bold cyan]Connection", border_style="cyan", box=box.ROUNDED)
    ...
    def create_info_panel(self) -> Panel:
        ...
        return Panel(table, title="[bold yellow]Terminal Information", border_style="yellow", box=box.ROUNDED)
    ...
    def create_alignment_panel(self) -> Panel:
        ...
        return Panel(table, title="[bold magenta]Dish Alignment", border_style="magenta", box=box.ROUNDED)
    ...
    def create_help_panel(self) -> Panel:
        ...
        return Panel(Align.center(help_text), box=box.ROUNDED, style="dim white")
    ...
    def create_layout(self) -> Layout:
        ...
        layout["left"].split_column(Layout(name="connection", size=8), Layout(name="status", size=12))

        layout["right"].split_column(Layout(name="info", size=10), Layout(name="alignment", size=10))
    ...
    def run(self):
        ...
        try:
            with Live(self.create_layout(), console=self.console, screen=True, refresh_per_second=2) as live:
...
Keyboard Controls:
  ...
  Ctrl+C - Exit
    ...
    parser.add_argument(
        "--ip", type=str, default="192.168.100.1", help="Starlink terminal IP address (default: 192.168.100.1)"
    )
