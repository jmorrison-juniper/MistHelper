...
| JUEX-L2-000180 | medium | Verify two-way connectivity on all interswitch trunked interfaces | Use one alternative: `set protocols oam ethernet link-fault-management interface <interface name>` for LFM, or `set interfaces <interface name> ether-options 802.3ad ae<bundle number>` for LAG. Check LFM with `show configuration protocols oam ethernet link-fault-management | display set`. Check LAG with `show configuration interfaces <interface name> ether-options | display set`. Source: U_Juniper_EX_Switches_L2S_STIG_V2R5_Manual-xccdf.xml. |
