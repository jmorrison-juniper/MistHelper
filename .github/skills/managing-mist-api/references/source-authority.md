...
Use the repository sources before remembered knowledge.
No Mist credential is necessary to read these sources.

| Priority | Source | Use |
| - | - | - |
| 1 | The installed `mistapi` package, version `0.64.0` | Verify SDK module names, function names, call signatures, parameter defaults, and response object access. |
| 2 | [OpenAPI 3.1 JSON](../../../../documentation/mist-api-openapi31json.json) and [OpenAPI 3.1 YAML](../../../../documentation/mist-api-openapi31yaml.yaml) | Read exact HTTP operations, path parameters, query parameters, media types, and schemas. |
| 3 | [Endpoint index](../../../../documentation/api/INDEX.md) and its endpoint pages | Find a source quickly and read generated operation notes. Verify each material note against the SDK or OpenAPI source. |
| 4 | [Endpoint catalog](../../../../src/export/endpoint_catalog.py) and [Operation registry](../../../../src/utils/operation_registry.py) | Determine what MistHelper implements and how it classifies a menu operation. |
| 5 | [Saved overview](../../../../documentation/Overview%20_%20API%20_%20Mist.html), [authentication](../../../../documentation/Auth%20_%20API%20_%20Mist.html), [organization](../../../../documentation/Org%20_%20API%20_%20Mist.html), [site](../../../../documentation/Site%20_%20API%20_%20Mist.html), and [MSP](../../../../documentation/MSP%20_%20API%20_%20Mist.html) guides | Read workflow descriptions, restrictions, streaming channels, and explanatory examples. |
| 6 | [OpenAPI 3.0 JSON](../../../../documentation/mist-api-openapi3json.json) and [YAML](../../../../documentation/mist-api-openapi3yaml.yaml) | Examine older behavior only when a current source does not explain a legacy reference. |
| 7 | [Architecture](../../../../documentation/architecture.md), [development setup](../../../../documentation/development-setup.md), and other MistHelper guides | Apply repository integration rules, not new cloud API guarantees. |

This priority resolves a question only within that source's authority.
The installed SDK wins for callable names and call signatures.
The OpenAPI source wins for HTTP paths, methods, parameters, and schemas.
The operation registry wins for MistHelper menu support and safety category.
...
`SDK_*.md` files record an SDK snapshot.
They do not establish a complete HTTP contract.
The [SDK-only index](../../../../documentation/api/INDEX.md#library-only-mistapi-sdk-not-in-openapi-spec) identifies these entries.
An SDK-only file upload helper can use an existing endpoint.
It is not necessarily a separate HTTP endpoint.

### Verified source editions

These measurements describe the snapshot checked on 2026-09-16.

| Property | Primary edition | Older edition |
| - | - | - |
| `openapi` | `3.1.0` | `3.0.0` |
| `info.version` | `2607.1.1` | `2508.1.1` |
| Version inside `info.description` | `2607.1.1` | `2508.1.1` |
| Date inside `info.description` | August 17, 2026 | August 8, 2025 |
| Paths | 756 | 714 |
| HTTP operations | 1,059 | 1,006 |
| GET / POST / PUT / DELETE | 529 / 303 / 109 / 118 | 490 / 295 / 108 / 113 |
| Referenced category tags | 212 | 203 |
| Component schemas | 2,892 | 1,682 |

The endpoint index reports 1,059 specification operations and 23 library-only stubs.
The `documentation/api/` tree contains 1,124 Markdown files, including the index and enrichment guide.
These checks do not certify every generated sentence or cross-reference.

### YAML scalar interpretation

A safe YAML loader prevents object construction.
It does not necessarily apply YAML 1.2 scalar rules.
The default PyYAML loader uses YAML 1.1 rules that can reinterpret examples.

| Value class | Primary pair | Older pair | Default PyYAML effect |
| - | -: | -: | - |
| Timestamp text | 0 | 131 | The loader constructs a datetime object instead of preserving the JSON string. |
| Date text | 0 | 0 | The loader constructs a date object instead of preserving the JSON string. |
| Time-of-day text | 0 | 14 | The loader converts the unquoted value `12:00` into the integer `720`. |
...
For exhaustive coverage, enumerate the specification structurally.
A text search alone can miss referenced schemas.
...
| - | - | - |
| Primary version | `info.version` and the description both say `2607.1.1`. | Record the value and the source hash. |
...
| AOSCX SDK module | The installed SDK exposes `mistapi.api.v1.orgs.aoscx.getOrgAoscxRegisterCmd`. The old `mistapi.api.v1.orgs.aos` module does not import. | Use the installed SDK module for calls. Treat an older source name as stale until verified. |
| Site SDK functions | The installed SDK exposes `mistapi.api.v1.sites.sites.getSiteInfo`. It does not expose `getSite` or `listSites`. | Verify `hasattr()` before writing a call. Do not let a broad exception hide `AttributeError`. |
