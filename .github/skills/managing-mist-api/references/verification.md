...
| SRC-03A | A call uses `getSite` or `listSites` from `mistapi.api.v1.sites.sites`. | Reject the call after `hasattr()` proves that the function is absent. |
...
| SRC-04A | The OpenAPI source and the SDK disagree on a callable path. | Use the installed SDK for the call signature and record the conflict. |
| SRC-05 | The primary version appears as `2607.1.1` in `info.version` and the description. | Preserve the exact value and source hash. |
