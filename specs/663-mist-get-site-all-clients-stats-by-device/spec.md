# Feature Specification: Mist API Read Operation -- getSiteAllClientsStatsByDevice

**Feature Branch**: `663-mist-get-site-all-clients-stats-by-device`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getSiteAllClientsStatsByDevice` and add it as a new MistHelper menu item."
...
(org / site / device as applicable), and receives the JSON payload exposed by `getSiteAllClientsStatsByDevice` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.stats.devices.clients.getSiteAllClientsStatsByDevice()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.stats.devices.clients.getSiteAllClientsStatsByDevice()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getSiteAllClientsStatsByDevice` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
