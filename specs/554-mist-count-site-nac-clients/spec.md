# Feature Specification: Mist API Read Operation -- countSiteNacClients

**Feature Branch**: `554-mist-count-site-nac-clients`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `countSiteNacClients` and add it as a new MistHelper menu item."
...
- **mistapi SDK module**: `mistapi.api.v1.sites.nac_clients.count`
...
- `distinct` (optional)
- `last_nacrule_id` (optional)
- `nacrule_matched` (optional)
- `auth_type` (optional)
- `last_vlan_id` (optional)
- `last_nas_vendor` (optional)
- `idp_id` (optional)
- `last_ssid` (optional)
- `last_username` (optional)
- `timestamp` (optional)
- `last_ap` (optional)
- `mac` (optional)
- `last_status` (optional)
- `type` (optional)
- `mdm_compliance_status` (optional)
...
(org / site / device as applicable), and receives the JSON payload exposed by `countSiteNacClients` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.nac_clients.count.countSiteNacClients()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.nac_clients.count.countSiteNacClients()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `countSiteNacClients` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
