# Feature Specification: Mist API Read Operation -- searchSiteIotEndpoints

**Feature Branch**: `963-mist-search-site-iot-endpoints`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `searchSiteIotEndpoints` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `searchSiteIotEndpoints`
- **Method**: `GET`
- **Path**: `/api/v1/sites/{site_id}/iotendpoints/search`
- **Tag**: `Sites Stats - IoT Endpoints`
- **mistapi SDK module**: `mistapi.api.v1.sites.iotendpoints`
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchSiteIotEndpoints` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.iotendpoints.searchSiteIotEndpoints()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.iotendpoints.searchSiteIotEndpoints()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `searchSiteIotEndpoints` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
