# Implementation Plan: GetOrgLicenseAsyncClaimStatus Menu Item

**Branch**: `500-mist-get-org-license-async-claim-status` | **Date**: 2026-06-28 | **Spec**: [spec.md](./spec.md)
**Input**: Feature specification from `specs/500-mist-get-org-license-async-claim-status/spec.md`

## Summary

Add a new read-only MistHelper menu item that invokes the Mist API endpoint
`GET /api/v1/orgs/{org_id}/claim/status` (operationId `GetOrgLicenseAsyncClaimStatus`)
to retrieve the processing status of the most recent asynchronous device claim job for an
organization. The menu item prompts the user for an `org_id` via `safe_input()`, optionally
asks whether per-device detail is required (the `detail` query parameter), invokes the
`mistapi` SDK, flattens the multi-array response into one summary row plus zero or more
per-device detail rows, and persists the result through
`DataExporter.write_with_format_selection()` so CSV, SQLite, and ArangoDB+Redis backends all
receive consistent output. A new entry is registered in `ENDPOINT_PRIMARY_KEY_STRATEGIES`
for clean SQLite upserts on repeated runs. The new operation is proposed as menu number
**95** -- the next available slot in the safe-org-exports / org-license cluster, sitting
adjacent to the existing license summary and by-site operations.
...
destructive skip list (14, 18, 63-65, 90-100) is unaffected -- new item 95 sits inside the
...
**Performance Goals**: Single GET request completes in <=5 seconds for typical claim jobs
(the endpoint is non-paginated and the response is a single JSON object). Adaptive delay
...
`LicenseExportUtils` class, one new entry in `ENDPOINT_PRIMARY_KEY_STRATEGIES`, two new
CSV/SQLite tables (`org_claim_status_summary` and `org_claim_status_details`), one menu
...
- **STATUS: PASS** -- The new menu method `export_org_license_async_claim_status()` stays
  under 25 lines, takes <=3 parameters (`self`, `org_id`, `include_detail`), and contains
  <=5 logical blocks (prompt -> API call -> flatten summary -> flatten details ->
  DataExporter call). Hierarchy is unchanged: one new method on an existing class. No new
  packages, modules, or top-level constants are introduced. Two output flatteners are
...
- **STATUS: PASS** -- The new behavior is added as a method on the existing
  `LicenseExportUtils` class (the same class that owns the related `getOrgLicensesSummary`
  and `getOrgLicensesBySite` exports). No standalone wrapper function is introduced. The
  menu dispatch in the main loop references the class method directly. Variable names use
  full words (`device_detail`, `claim_status_row`) -- no single-letter iterators.

### Principle III: Safety-First (NON-NEGOTIABLE)

- **STATUS: PASS** -- All user input is collected through `safe_input()` with explicit
  `context=` strings (`"org_license_claim_status:org_id"`,
  `"org_license_claim_status:detail"`) so SSH / container EOF exits cleanly with code 0
  ...
  Mist UUID shape before the API call; on validation failure the method logs a warning
...
- **STATUS: PASS** -- After implementation the standard pipeline applies without
  ...
  -> commit with `version YY.MM.DD.HH.MM - add menu 95 GetOrgLicenseAsyncClaimStatus`
...
- **STATUS: PASS** -- All log calls use ASCII text and `%s` style formatting. `INFO` is
  emitted before the API call ("Fetching async claim status for org %s"); `DEBUG` after
  the call with summary counts ("Claim status: status=%s total=%d succeed=%d failed=%d");
...
- **STATUS: PASS** -- Every executable line in the new method, the new PK strategy
  ...
  lines in the touched block (the existing license-export menu cluster) get comments
...
```text
specs/500-mist-get-org-license-async-claim-status/
├── plan.md              # This file
├── research.md          # Phase 0 - SDK signature, PK strategy, naming, menu placement
├── data-model.md        # Phase 1 - response entities + DDL + PK registration
├── quickstart.md        # Phase 1 - local run + .env + quality gates
├── contracts/
│   └── get_org_license_async_claim_status.md   # Phase 1 - HTTP + SDK contract
...
```text
MistHelper.py            # New method on LicenseExportUtils class + PK strategy + menu 95
...
method on the existing `LicenseExportUtils` class in `MistHelper.py` (the same class that
owns the other org-licenses exports). The menu number proposal is **95**, chosen because
operations 51-95 are the Safe Org Exports / Org-License / SLE cluster and 95 is the next
available slot below the resource-intensive block at 96-101. The full menu list will be
re-verified at task generation time; if 95 collides with an in-flight feature branch, the
...
- **Principle II (Class-Based)**: PASS -- All work lives on `LicenseExportUtils`. No
  wrappers introduced. Flattening helpers, if needed, are added as private methods on the
  same class.
