# Feature Specification: Mist API Read Operation -- getMspSle

**Feature Branch**: `587-mist-get-msp-sle`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getMspSle` and add it as a new MistHelper menu item."
...
- **Tag**: `MSPs SLEs`
- **mistapi SDK module**: `mistapi.api.v1.msps.insights`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getMspSle` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.msps.insights.getMspSle()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.msps.insights.getMspSle()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getMspSle` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
