# Feature Specification: Mist API Read Operation -- searchSiteMarvisConfigActions

**Feature Branch**: `964-mist-search-site-marvis-config-actions`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `searchSiteMarvisConfigActions` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `searchSiteMarvisConfigActions`
- **Method**: `GET`
- **Path**: `/api/v1/sites/{site_id}/marvis_configs/search`
- **Tag**: `Sites Marvis Configs`
- **mistapi SDK module**: `mistapi.api.v1.sites.marvis_configs`
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchSiteMarvisConfigActions` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.marvis_configs.searchSiteMarvisConfigActions()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.marvis_configs.searchSiteMarvisConfigActions()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `searchSiteMarvisConfigActions` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
