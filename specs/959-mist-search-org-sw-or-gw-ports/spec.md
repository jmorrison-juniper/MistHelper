# Feature Specification: Mist API Read Operation -- searchOrgSwOrGwPorts

**Feature Branch**: `959-mist-search-org-sw-or-gw-ports`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `searchOrgSwOrGwPorts` and add it as a new MistHelper menu item."
...
- `auth_state` (optional)
- `full_duplex` (optional)
- `lte_imsi` (optional)
- `lte_iccid` (optional)
- `lte_imei` (optional)
- `mac` (optional)
- `neighbor_mac` (optional)
- `neighbor_port_desc` (optional)
- `neighbor_system_name` (optional)
- `poe_disabled` (optional)
- `poe_mode` (optional)
- `poe_on` (optional)
- `poe_priority` (optional)
- `port_id` (optional)
- `port_mac` (optional)
- `speed` (optional)
- `stp_state` (optional)
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchOrgSwOrGwPorts` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.stats.searchOrgSwOrGwPorts()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.stats.searchOrgSwOrGwPorts()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `searchOrgSwOrGwPorts` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
