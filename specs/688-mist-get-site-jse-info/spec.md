# Feature Specification: Mist API Read Operation -- getSiteJseInfo

**Feature Branch**: `688-mist-get-site-jse-info`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getSiteJseInfo` and add it as a new MistHelper menu item."
...
- **Tag**: `Sites JSE`
- **mistapi SDK module**: `mistapi.api.v1.sites.setting.jse.info`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getSiteJseInfo` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.setting.jse.info.getSiteJseInfo()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.setting.jse.info.getSiteJseInfo()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getSiteJseInfo` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
