# Implementation Plan: countSiteNacClientEvents Menu Item

**Branch**: `553-mist-count-site-nac-client-events` | **Date**: 2026-06-29 | **Spec**: [spec.md](./spec.md)
**Input**: Feature specification from `specs/553-mist-count-site-nac-client-events/spec.md`

## Summary

Add a new read-only MistHelper menu item that invokes the Mist API endpoint
`GET /api/v1/sites/{site_id}/nac_clients/events/count` (operationId
`countSiteNacClientEvents`) to retrieve a server-side aggregated count of NAC
(802.1X / RADIUS) client events at a site, grouped by a caller-chosen distinct
attribute (for example `type`, `nas_vendor`, `auth_type`, `ssid`, `vlan`). The
menu item prompts the user for `site_id` via `safe_input()`, then optionally
collects the `distinct`, `type`, `start`, `end`, `duration`, and `limit` query
parameters with sensible defaults (distinct=`type`, duration=`1d`, limit=`100`),
invokes the `mistapi` SDK, flattens the envelope response into one summary row
plus N per-bucket result rows, and persists everything through
...
new operation is proposed as menu number **96** -- the next available slot inside
the Interactive Safe Site cluster (60-96), sitting near the existing
NAC-events-search and site-events viewers, with re-verification at task time if
an in-flight feature branch claims the same integer first.
...
a known `MIST_SITE_ID` from `.env`. Local quality gates: `python -m py_compile
...
new item at 96 sits at the boundary of the test sweep range and is included.
...
**Performance Goals**: Single GET request completes in <=5 seconds for a 1-day window
(server-side aggregation, response is bounded by the `limit` query parameter, default
100). Adaptive delay metrics in `delay_metrics.json` and `tuning_data.json` continue to
govern back-off; the endpoint is light enough that no special tuning is required.
...
5-Item Rule (<=25 lines, <=5 params, <=5 nesting blocks per function).
**Scale/Scope**: One new public menu method (~22 lines) on the existing
`SiteEventsExportUtils` class (or the nearest existing class that owns site-scoped NAC /
events exports -- verified at task time), one new entry in
`ENDPOINT_PRIMARY_KEY_STRATEGIES`, two new CSV/SQLite tables
(`site_nac_client_events_count_summary` and `site_nac_client_events_count_results`), one
...
- **STATUS: PASS** -- The new menu method `export_site_nac_client_events_count()` stays
  under 25 lines, takes <=4 parameters (`self`, `site_id`, `distinct`, `query_window`),
  and contains <=5 logical blocks (prompt -> validate -> API call -> flatten summary +
  ...
  step is a single list comprehension over `response["results"]`; if it grows past 5
  lines during implementation it is extracted to a private helper on the same class.

### Principle II: Class-Based Architecture (No Wrappers)

- **STATUS: PASS** -- The new behavior is added as a method on the existing class that
  owns adjacent site-scoped NAC / event exports (for example the same class that hosts
  `searchSiteNacClientEvents` and `countSiteNacClients`). No standalone wrapper function
  is introduced. The menu dispatch in the main loop references the class method
  directly. Variable names use full words (`distinct_attribute`, `bucket_row`,
  `query_window_seconds`); no single-letter iterators.

### Principle III: Safety-First (NON-NEGOTIABLE)

- **STATUS: PASS** -- All user input is collected through `safe_input()` with explicit
  `context=` strings (`"site_nac_events_count:site_id"`,
  `"site_nac_events_count:distinct"`, `"site_nac_events_count:duration"`) so SSH /
  container EOF exits cleanly with code 0 and no traceback. The endpoint is strictly
  read-only (HTTP GET), so no typed destructive-confirmation gate is required. `site_id`
  is validated against the Mist UUID shape before the API call; on validation failure
...
- **STATUS: PASS** -- After implementation the standard pipeline applies without
  ...
  -> commit with `version YY.MM.DD.HH.MM - add menu 96 countSiteNacClientEvents` ->
...
- **STATUS: PASS** -- All log calls use ASCII text and `%s` style formatting. `INFO` is
  emitted before the API call ("Counting NAC client events for site %s distinct=%s
  duration=%s"); `DEBUG` after the call with summary counts ("NAC events count: total=%d
  distinct_buckets=%d limit=%d"); `WARNING` on 404 / empty `results`; `ERROR` on
...
- **STATUS: PASS** -- Every executable line in the new method, the new PK strategy
  ...
  lines in the touched block (the existing NAC / site-events export cluster) get
...
```text
specs/553-mist-count-site-nac-client-events/
├── plan.md              # This file
├── research.md          # Phase 0 - SDK signature, PK strategy, naming, menu placement
├── data-model.md        # Phase 1 - response entities + DDL + PK registration
├── quickstart.md        # Phase 1 - local run + .env + quality gates
├── contracts/
│   └── count_site_nac_client_events.md   # Phase 1 - HTTP + SDK contract
...
```text
MistHelper.py            # New method on the site-NAC-events export class + PK strategy
...
method on the existing site-scoped NAC / events export class in `MistHelper.py` (the
same class that hosts `searchSiteNacClientEvents` and `countSiteNacClients`; exact class
name confirmed at task time by `grep` against the monolith). The menu number proposal
is **96**, chosen because operations 60-96 are the Interactive Safe Site cluster and 96
is the next available integer at the upper boundary before the Resource Intensive block
at 97-101. The full menu list will be re-verified at task generation time; if 96
collides with an in-flight feature branch, the next free integer in the same cluster is
used.
...
`contracts/count_site_nac_client_events.md`), the seven principles are re-evaluated
...
- **Principle I (Five-Item Rule)**: PASS -- The method outline in `quickstart.md`
  ...
  `ENDPOINT_PRIMARY_KEY_STRATEGIES` dictionary update is a single insert (existing
  structure), so no level-5 hierarchy explosion.
- **Principle II (Class-Based)**: PASS -- All work lives on the existing site-NAC /
  events export class. No wrappers introduced. The flatten helper, if extracted,
