# Feature Specification: Mist API Read Operation -- searchOrgTunnelsStats

**Feature Branch**: `960-mist-search-org-tunnels-stats`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `searchOrgTunnelsStats` and add it as a new MistHelper menu item."
...
- `mxcluster_id` (optional)
- `site_id` (optional)
- `wxtunnel_id` (optional)
- `ap` (optional)
- `mac` (optional)
- `node` (optional)
- `peer_ip` (optional)
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchOrgTunnelsStats` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.stats.searchOrgTunnelsStats()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.stats.searchOrgTunnelsStats()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `searchOrgTunnelsStats` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
