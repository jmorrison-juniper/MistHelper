# Feature Specification: Mist API Read Operation -- getSiteSleThreshold

**Feature Branch**: `710-mist-get-site-sle-threshold`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getSiteSleThreshold` and add it as a new MistHelper menu item."
...
- **Tag**: `Sites SLEs`
- **mistapi SDK module**: `mistapi.api.v1.sites.sle.metric.threshold`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getSiteSleThreshold` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.sle.metric.threshold.getSiteSleThreshold()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.sle.metric.threshold.getSiteSleThreshold()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getSiteSleThreshold` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
