# Feature Specification: Mist API Read Operation -- getOrgOtherDeviceStats

**Feature Branch**: `630-mist-get-org-other-device-stats`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getOrgOtherDeviceStats` and add it as a new MistHelper menu item."
...
- **mistapi SDK module**: `mistapi.api.v1.orgs.stats.otherdevices`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getOrgOtherDeviceStats` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.stats.otherdevices.getOrgOtherDeviceStats()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.stats.otherdevices.getOrgOtherDeviceStats()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getOrgOtherDeviceStats` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
