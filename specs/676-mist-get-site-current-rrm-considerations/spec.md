# Feature Specification: Mist API Read Operation -- getSiteCurrentRrmConsiderations

**Feature Branch**: `676-mist-get-site-current-rrm-considerations`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getSiteCurrentRrmConsiderations` and add it as a new MistHelper menu item."
...
- **Tag**: `Sites RRM`
- **mistapi SDK module**: `mistapi.api.v1.sites.rrm.current.devices.band`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getSiteCurrentRrmConsiderations` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.rrm.current.devices.band.getSiteCurrentRrmConsiderations()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.rrm.current.devices.band.getSiteCurrentRrmConsiderations()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getSiteCurrentRrmConsiderations` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
