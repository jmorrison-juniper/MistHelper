# Feature Specification: Mist API Read Operation -- getSiteMachineLearningCurrentStat

**Feature Branch**: `690-mist-get-site-machine-learning-current-stat`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getSiteMachineLearningCurrentStat` and add it as a new MistHelper menu item."
...
- **Tag**: `Sites Location`
- **mistapi SDK module**: `mistapi.api.v1.sites.location.ml.current`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getSiteMachineLearningCurrentStat` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.location.ml.current.getSiteMachineLearningCurrentStat()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.location.ml.current.getSiteMachineLearningCurrentStat()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getSiteMachineLearningCurrentStat` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
