# Feature Specification: Mist API Read Operation -- listSiteMxEdgeUpgrades

**Feature Branch**: `945-mist-list-site-mx-edge-upgrades`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `listSiteMxEdgeUpgrades` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `listSiteMxEdgeUpgrades`
- **Method**: `GET`
- **Path**: `/api/v1/sites/{site_id}/mxedges/upgrade`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listSiteMxEdgeUpgrades` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.mxedges.listSiteMxEdgeUpgrades()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.mxedges.listSiteMxEdgeUpgrades()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `listSiteMxEdgeUpgrades` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
