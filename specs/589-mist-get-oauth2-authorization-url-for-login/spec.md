# Feature Specification: Mist API Read Operation -- getOauth2AuthorizationUrlForLogin

**Feature Branch**: `589-mist-get-oauth2-authorization-url-for-login`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getOauth2AuthorizationUrlForLogin` and add it as a new MistHelper menu item."
...
- **mistapi SDK module**: `mistapi.api.v1.login.oauth`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getOauth2AuthorizationUrlForLogin` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.login.oauth.getOauth2AuthorizationUrlForLogin()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.login.oauth.getOauth2AuthorizationUrlForLogin()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getOauth2AuthorizationUrlForLogin` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
