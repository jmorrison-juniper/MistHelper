# Feature Specification: Mist API Read Operation -- getApiToken

**Feature Branch**: `578-mist-get-api-token`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getApiToken` and add it as a new MistHelper menu item."
...
- **Tag**: `Self API Token`
- **mistapi SDK module**: `mistapi.api.v1.self.apitokens`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getApiToken` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.self.apitokens.getApiToken()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.self.apitokens.getApiToken()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getApiToken` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
