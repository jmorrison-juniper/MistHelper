# Feature Specification: Mist API Read Operation -- getAdminRegistrationInfo

**Feature Branch**: `577-mist-get-admin-registration-info`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getAdminRegistrationInfo` and add it as a new MistHelper menu item."
...
- **Tag**: `Admins`
- **mistapi SDK module**: `mistapi.api.v1.register.recaptcha`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getAdminRegistrationInfo` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.register.recaptcha.getAdminRegistrationInfo()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.register.recaptcha.getAdminRegistrationInfo()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getAdminRegistrationInfo` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
