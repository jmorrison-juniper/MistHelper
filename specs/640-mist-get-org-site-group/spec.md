# Feature Specification: Mist API Read Operation -- getOrgSiteGroup

**Feature Branch**: `640-mist-get-org-site-group`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getOrgSiteGroup` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs Sitegroups`
- **mistapi SDK module**: `mistapi.api.v1.orgs.sitegroups`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getOrgSiteGroup` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.sitegroups.getOrgSiteGroup()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.sitegroups.getOrgSiteGroup()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getOrgSiteGroup` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
