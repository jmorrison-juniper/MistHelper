# Feature Specification: Mist API Read Operation -- listOrgSsoLatestFailures

**Feature Branch**: `789-mist-list-org-sso-latest-failures`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listOrgSsoLatestFailures` and add it as a new MistHelper menu item."
...
(org / site / device as applicable), and receives the JSON payload exposed by `listOrgSsoLatestFailures` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.ssos.failures.listOrgSsoLatestFailures()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.ssos.failures.listOrgSsoLatestFailures()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listOrgSsoLatestFailures` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
