# Feature Specification: Mist API Read Operation -- listMspLicenses

**Feature Branch**: `752-mist-list-msp-licenses`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listMspLicenses` and add it as a new MistHelper menu item."
...
- **Tag**: `MSPs Licenses`
- **mistapi SDK module**: `mistapi.api.v1.msps.licenses`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listMspLicenses` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.msps.licenses.listMspLicenses()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.msps.licenses.listMspLicenses()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listMspLicenses` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
