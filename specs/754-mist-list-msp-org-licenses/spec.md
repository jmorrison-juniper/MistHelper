# Feature Specification: Mist API Read Operation -- listMspOrgLicenses

**Feature Branch**: `754-mist-list-msp-org-licenses`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listMspOrgLicenses` and add it as a new MistHelper menu item."
...
- **Tag**: `MSPs Licenses`
- **mistapi SDK module**: `mistapi.api.v1.msps.stats.licenses`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listMspOrgLicenses` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.msps.stats.licenses.listMspOrgLicenses()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.msps.stats.licenses.listMspOrgLicenses()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listMspOrgLicenses` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
