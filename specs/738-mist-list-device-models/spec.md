# Feature Specification: Mist API Read Operation -- listDeviceModels

**Feature Branch**: `738-mist-list-device-models`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listDeviceModels` and add it as a new MistHelper menu item."
...
- **mistapi SDK module**: `mistapi.api.v1.const.device_models`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listDeviceModels` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.const.device_models.listDeviceModels()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.const.device_models.listDeviceModels()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listDeviceModels` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
