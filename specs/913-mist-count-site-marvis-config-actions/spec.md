# Feature Specification: Mist API Read Operation -- countSiteMarvisConfigActions

**Feature Branch**: `913-mist-count-site-marvis-config-actions`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `countSiteMarvisConfigActions` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `countSiteMarvisConfigActions`
- **Method**: `GET`
- **Path**: `/api/v1/sites/{site_id}/marvis_configs/count`
...
(org / site / device as applicable), and receives the JSON payload exposed by `countSiteMarvisConfigActions` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.marvis_configs.countSiteMarvisConfigActions()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.marvis_configs.countSiteMarvisConfigActions()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `countSiteMarvisConfigActions` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
