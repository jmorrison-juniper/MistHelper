# Feature Specification: Mist API Read Operation -- countOrgMarvisClientEvents

**Feature Branch**: `909-mist-count-org-marvis-client-events`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `countOrgMarvisClientEvents` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `countOrgMarvisClientEvents`
- **Method**: `GET`
- **Path**: `/api/v1/orgs/{org_id}/marvisclients/events/count`
...
(org / site / device as applicable), and receives the JSON payload exposed by `countOrgMarvisClientEvents` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.marvisclients.countOrgMarvisClientEvents()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.marvisclients.countOrgMarvisClientEvents()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `countOrgMarvisClientEvents` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
