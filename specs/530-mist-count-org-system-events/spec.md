# Feature Specification: Mist API Read Operation -- countOrgSystemEvents

**Feature Branch**: `530-mist-count-org-system-events`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `countOrgSystemEvents` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs Events`
- **mistapi SDK module**: `mistapi.api.v1.orgs.events.system.count`
...
(org / site / device as applicable), and receives the JSON payload exposed by `countOrgSystemEvents` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.events.system.count.countOrgSystemEvents()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.events.system.count.countOrgSystemEvents()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `countOrgSystemEvents` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
