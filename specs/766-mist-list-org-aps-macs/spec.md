# Feature Specification: Mist API Read Operation -- listOrgApsMacs

**Feature Branch**: `766-mist-list-org-aps-macs`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listOrgApsMacs` and add it as a new MistHelper menu item."
...
(org / site / device as applicable), and receives the JSON payload exposed by `listOrgApsMacs` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.devices.radio_macs.listOrgApsMacs()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.devices.radio_macs.listOrgApsMacs()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listOrgApsMacs` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
