# Feature Specification: Mist API Read Operation -- listInstallerSiteGroups

**Feature Branch**: `746-mist-list-installer-site-groups`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listInstallerSiteGroups` and add it as a new MistHelper menu item."
...
- **Tag**: `Installer`
- **mistapi SDK module**: `mistapi.api.v1.installer.orgs.sitegroups`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listInstallerSiteGroups` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.installer.orgs.sitegroups.listInstallerSiteGroups()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.installer.orgs.sitegroups.listInstallerSiteGroups()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listInstallerSiteGroups` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
