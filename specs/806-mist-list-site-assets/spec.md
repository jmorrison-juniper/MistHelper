# Feature Specification: Mist API Read Operation -- listSiteAssets

**Feature Branch**: `806-mist-list-site-assets`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listSiteAssets` and add it as a new MistHelper menu item."
...
- **Tag**: `Sites Assets`
- **mistapi SDK module**: `mistapi.api.v1.sites.assets`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listSiteAssets` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.assets.listSiteAssets()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.assets.listSiteAssets()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listSiteAssets` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
