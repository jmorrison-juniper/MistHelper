# Feature Specification: Mist API Read Operation -- listSiteIdpProfilesDerived

**Feature Branch**: `816-mist-list-site-idp-profiles-derived`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listSiteIdpProfilesDerived` and add it as a new MistHelper menu item."
...
(org / site / device as applicable), and receives the JSON payload exposed by `listSiteIdpProfilesDerived` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.idpprofiles.derived.listSiteIdpProfilesDerived()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.idpprofiles.derived.listSiteIdpProfilesDerived()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listSiteIdpProfilesDerived` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
