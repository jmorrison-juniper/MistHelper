# Feature Specification: Mist API Read Operation -- searchOrgNacClientEvents

**Feature Branch**: `955-mist-search-org-nac-client-events`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `searchOrgNacClientEvents` and add it as a new MistHelper menu item."
...
- `dryrun_nacrule_id` (optional)
- `dryrun_nacrule_matched` (optional)
- `auth_type` (optional)
- `vlan` (optional)
- `nas_vendor` (optional)
- `bssid` (optional)
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchOrgNacClientEvents` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.nac_clients.searchOrgNacClientEvents()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.nac_clients.searchOrgNacClientEvents()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `searchOrgNacClientEvents` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
