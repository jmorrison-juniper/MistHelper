# Feature Specification: Mist API Read Operation -- adoptOrgJsiDevice

**Feature Branch**: `503-mist-adopt-org-jsi-device`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `adoptOrgJsiDevice` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs JSI`
- **mistapi SDK module**: `mistapi.api.v1.orgs.jsi.devices.outbound_ssh_cmd`
...
(org / site / device as applicable), and receives the JSON payload exposed by `adoptOrgJsiDevice` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.jsi.devices.outbound_ssh_cmd.adoptOrgJsiDevice()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.jsi.devices.outbound_ssh_cmd.adoptOrgJsiDevice()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `adoptOrgJsiDevice` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
