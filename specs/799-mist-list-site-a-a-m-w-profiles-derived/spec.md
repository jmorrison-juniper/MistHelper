# Feature Specification: Mist API Read Operation -- listSiteAAMWProfilesDerived

**Feature Branch**: `799-mist-list-site-a-a-m-w-profiles-derived`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listSiteAAMWProfilesDerived` and add it as a new MistHelper menu item."
...
Get the list of derived Advanced Anti Malware Profile (SkyAtp) for a site
...
(org / site / device as applicable), and receives the JSON payload exposed by `listSiteAAMWProfilesDerived` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.aamwprofiles.derived.listSiteAAMWProfilesDerived()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.aamwprofiles.derived.listSiteAAMWProfilesDerived()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listSiteAAMWProfilesDerived` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
