# Feature Specification: Mist API Read Operation -- downloadOrgSamlMetadata

**Feature Branch**: `573-mist-download-org-saml-metadata`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `downloadOrgSamlMetadata` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs SSO`
- **mistapi SDK module**: `mistapi.api.v1.orgs.ssos.metadata_xml`
...
(org / site / device as applicable), and receives the JSON payload exposed by `downloadOrgSamlMetadata` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.ssos.metadata_xml.downloadOrgSamlMetadata()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.ssos.metadata_xml.downloadOrgSamlMetadata()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `downloadOrgSamlMetadata` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
