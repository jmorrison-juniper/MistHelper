# Feature Specification: Mist API Read Operation -- listMspSsoRoles

**Feature Branch**: `758-mist-list-msp-sso-roles`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listMspSsoRoles` and add it as a new MistHelper menu item."
...
- **Tag**: `MSPs SSO Roles`
- **mistapi SDK module**: `mistapi.api.v1.msps.ssoroles`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listMspSsoRoles` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.msps.ssoroles.listMspSsoRoles()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.msps.ssoroles.listMspSsoRoles()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listMspSsoRoles` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
