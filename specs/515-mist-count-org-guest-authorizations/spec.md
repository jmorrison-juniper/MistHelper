# Feature Specification: Mist API Read Operation -- countOrgGuestAuthorizations

**Feature Branch**: `515-mist-count-org-guest-authorizations`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `countOrgGuestAuthorizations` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs Guests`
- **mistapi SDK module**: `mistapi.api.v1.orgs.guests.count`
...
(org / site / device as applicable), and receives the JSON payload exposed by `countOrgGuestAuthorizations` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.guests.count.countOrgGuestAuthorizations()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.guests.count.countOrgGuestAuthorizations()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `countOrgGuestAuthorizations` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
