# Feature Specification: Mist API Read Operation -- countOrgSiteMxEdgeEvents

**Feature Branch**: `527-mist-count-org-site-mx-edge-events`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `countOrgSiteMxEdgeEvents` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs MxEdges`
- **mistapi SDK module**: `mistapi.api.v1.orgs.mxedges.events.count`
...
(org / site / device as applicable), and receives the JSON payload exposed by `countOrgSiteMxEdgeEvents` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.mxedges.events.count.countOrgSiteMxEdgeEvents()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.mxedges.events.count.countOrgSiteMxEdgeEvents()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `countOrgSiteMxEdgeEvents` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
