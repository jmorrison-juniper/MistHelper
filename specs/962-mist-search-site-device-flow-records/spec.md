# Feature Specification: Mist API Read Operation -- searchSiteDeviceFlowRecords

**Feature Branch**: `962-mist-search-site-device-flow-records`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `searchSiteDeviceFlowRecords` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `searchSiteDeviceFlowRecords`
- **Method**: `GET`
- **Path**: `/api/v1/sites/{site_id}/devices/{device_id}/flow_records/search`
- **Tag**: `Utilities LAN`
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchSiteDeviceFlowRecords` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.devices.searchSiteDeviceFlowRecords()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.devices.searchSiteDeviceFlowRecords()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `searchSiteDeviceFlowRecords` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
