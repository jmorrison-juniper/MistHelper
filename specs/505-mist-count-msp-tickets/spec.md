# Feature Specification: Mist API Read Operation -- countMspTickets

**Feature Branch**: `505-mist-count-msp-tickets`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `countMspTickets` and add it as a new MistHelper menu item."
...
- **Tag**: `MSPs Tickets`
- **mistapi SDK module**: `mistapi.api.v1.msps.tickets.count`
...
(org / site / device as applicable), and receives the JSON payload exposed by `countMspTickets` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.msps.tickets.count.countMspTickets()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.msps.tickets.count.countMspTickets()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `countMspTickets` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
