# Feature Specification: Mist API Read Operation -- countSiteOspfStats

**Feature Branch**: `555-mist-count-site-ospf-stats`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `countSiteOspfStats` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `countSiteOspfStats`
- **Method**: `GET`
- **Path**: `/api/v1/sites/{site_id}/stats/ospf_peers/count`
- **Tag**: `Sites Stats - Ospf`
- **mistapi SDK module**: `mistapi.api.v1.sites.stats.ospf_peers.count`
...
(org / site / device as applicable), and receives the JSON payload exposed by `countSiteOspfStats` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.stats.ospf_peers.count.countSiteOspfStats()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.stats.ospf_peers.count.countSiteOspfStats()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `countSiteOspfStats` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
