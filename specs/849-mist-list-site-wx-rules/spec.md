# Feature Specification: Mist API Read Operation -- listSiteWxRules

**Feature Branch**: `849-mist-list-site-wx-rules`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listSiteWxRules` and add it as a new MistHelper menu item."
...
(org / site / device as applicable), and receives the JSON payload exposed by `listSiteWxRules` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.wxrules.listSiteWxRules()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.wxrules.listSiteWxRules()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listSiteWxRules` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
