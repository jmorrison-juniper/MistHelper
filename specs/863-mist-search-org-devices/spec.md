# Feature Specification: Mist API Read Operation -- searchOrgDevices

**Feature Branch**: `863-mist-search-org-devices`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `searchOrgDevices` and add it as a new MistHelper menu item."
...
- `band_24_bandwidth` (optional)
- `band_24_channel` (optional)
- `band_24_power` (optional)
- `band_5_bandwidth` (optional)
- `band_5_channel` (optional)
- `band_5_power` (optional)
- `band_6_bandwidth` (optional)
- `band_6_channel` (optional)
- `band_6_power` (optional)
- `cpu` (optional)
- `clustered` (optional)
- `eth0_port_speed` (optional)
- `evpntopo_id` (optional)
- `ext_ip` (optional)
- `hostname` (optional)
- `ip` (optional)
- `last_config_status` (optional)
- `last_hostname` (optional)
- `lldp_mgmt_addr` (optional)
- `lldp_port_id` (optional)
- `lldp_power_allocated` (optional)
- `lldp_power_draw` (optional)
- `lldp_system_desc` (optional)
- `lldp_system_name` (optional)
- `mac` (optional)
- `model` (optional)
- `mxedge_id` (optional)
- `mxedge_ids` (optional)
- `mxtunnel_status` (optional)
- `node` (optional)
- `node0_mac` (optional)
- `node1_mac` (optional)
- `power_constrained` (optional)
- `site_id` (optional)
- `t128agent_version` (optional)
- `version` (optional)
- `type` (optional)
- `limit` (optional)
- `start` (optional)
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchOrgDevices` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.devices.search.searchOrgDevices()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.devices.search.searchOrgDevices()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `searchOrgDevices` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
