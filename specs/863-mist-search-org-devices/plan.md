...
**Scale/Scope**: One exporter method, one menu entry, one registry entry, one primary-key strategy, tests, and documentation.
...
The endpoint uses a composite `["id", "mac"]` key. The pair identifies a device
search row when the API returns repeated device records.
