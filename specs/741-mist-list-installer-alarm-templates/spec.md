# Feature Specification: Mist API Read Operation -- listInstallerAlarmTemplates

**Feature Branch**: `741-mist-list-installer-alarm-templates`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listInstallerAlarmTemplates` and add it as a new MistHelper menu item."
...
- **Tag**: `Installer`
- **mistapi SDK module**: `mistapi.api.v1.installer.orgs.alarmtemplates`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listInstallerAlarmTemplates` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.installer.orgs.alarmtemplates.listInstallerAlarmTemplates()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.installer.orgs.alarmtemplates.listInstallerAlarmTemplates()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listInstallerAlarmTemplates` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
