# Feature Specification: Mist API Read Operation -- getSiteAutoMapAssignmentStatus

**Feature Branch**: `921-mist-get-site-auto-map-assignment-status`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `getSiteAutoMapAssignmentStatus` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `getSiteAutoMapAssignmentStatus`
- **Method**: `GET`
- **Path**: `/api/v1/sites/{site_id}/auto_map_assignment`
- **Tag**: `Sites Auto Map Assignment`
- **mistapi SDK module**: `mistapi.api.v1.sites.auto_map_assignment`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getSiteAutoMapAssignmentStatus` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.auto_map_assignment.getSiteAutoMapAssignmentStatus()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.auto_map_assignment.getSiteAutoMapAssignmentStatus()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `getSiteAutoMapAssignmentStatus` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
