# Feature Specification: Mist API Read Operation -- getOrgWLAN

**Feature Branch**: `652-mist-get-org-w-l-a-n`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getOrgWLAN` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs Wlans`
- **mistapi SDK module**: `mistapi.api.v1.orgs.wlans`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getOrgWLAN` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.wlans.getOrgWLAN()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.wlans.getOrgWLAN()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getOrgWLAN` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
