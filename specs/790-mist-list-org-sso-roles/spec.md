# Feature Specification: Mist API Read Operation -- listOrgSsoRoles

**Feature Branch**: `790-mist-list-org-sso-roles`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listOrgSsoRoles` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs SSO Roles`
- **mistapi SDK module**: `mistapi.api.v1.orgs.ssoroles`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listOrgSsoRoles` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.ssoroles.listOrgSsoRoles()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.ssoroles.listOrgSsoRoles()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listOrgSsoRoles` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
