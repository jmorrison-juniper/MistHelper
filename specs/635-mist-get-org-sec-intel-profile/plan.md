# Implementation Plan: GetOrgSecIntelProfile Menu Item

**Branch**: `635-mist-get-org-sec-intel-profile` | **Date**: 2026-06-30 | **Spec**: [spec.md](./spec.md)
**Input**: Feature specification from `specs/635-mist-get-org-sec-intel-profile/spec.md`

## Summary

Add a new read-only MistHelper menu item that invokes the Mist API endpoint
`GET /api/v1/orgs/{org_id}/secintelprofiles/{secintelprofile_id}` (operationId
`getOrgSecIntelProfile`) to retrieve a single Security Intelligence profile that
configures threat-intelligence feeds on SRX gateways. The menu item prompts the
user for `org_id` and `secintelprofile_id` via `safe_input()`, invokes the
`mistapi.api.v1.orgs.secintel_profiles.getOrgSecIntelProfile()` SDK call, flattens
the response (a single profile object with a nested `profiles` array of
category/action rules) into one summary row plus zero or more per-rule detail
rows, and persists the result through
...
new operation is proposed as menu number **89**, the next available slot in the
Safe Org Exports cluster adjacent to existing per-object read endpoints.
...
non-interactive mode using a known org and a known secintelprofile_id from
`.env` (new optional var `MIST_SECINTEL_PROFILE_ID`). Local quality gates:
...
(14, 18, 63-65, 90-100) is unaffected -- new item 89 sits inside the default
test sweep range.
...
**Performance Goals**: Single non-paginated GET request completes in <=5
seconds for typical profiles. Adaptive delay metrics in `delay_metrics.json`
...
`SecurityExportUtils` class (created if not present, otherwise reuse the
closest existing org-security-related class), one new entry in
`ENDPOINT_PRIMARY_KEY_STRATEGIES`, two new CSV/SQLite tables
(`org_secintel_profile_summary` and `org_secintel_profile_rules`), one menu
...
- **STATUS: PASS** -- The new menu method
  `export_org_secintel_profile()` stays under 25 lines, takes <=3 parameters
  (`self`, `org_id`, `secintelprofile_id`), and contains <=5 logical blocks
  (prompt -> API call -> flatten summary -> flatten rules -> DataExporter
  call). Hierarchy is unchanged: one new method on an existing class. The
  nested `profiles` array flattener is inlined as a single list comprehension;
...
- **STATUS: PASS** -- The new behavior is added as a method on the existing
  org-security export class (`SecurityExportUtils` or the closest sibling that
  already owns `listOrgSecIntelProfiles` when that arrives). No standalone
  ...
  (`profile_summary_row`, `profile_rule_row`) -- no single-letter iterators.

### Principle III: Safety-First (NON-NEGOTIABLE)

- **STATUS: PASS** -- All user input is collected through `safe_input()` with
  explicit `context=` strings
  (`"org_secintel_profile:org_id"`, `"org_secintel_profile:profile_id"`) so
  SSH / container EOF exits cleanly with code 0 and no traceback. The endpoint
  is strictly read-only (HTTP GET), so no typed destructive-confirmation gate
  is required. Both UUIDs are validated against the Mist UUID shape before the
...
- **STATUS: PASS** -- After implementation the standard pipeline applies
  ...
  `version YY.MM.DD.HH.MM - add menu 89 getOrgSecIntelProfile` -> `git push
...
- **STATUS: PASS** -- All log calls use ASCII text and `%s` style formatting.
  `INFO` is emitted before the API call ("Fetching SecIntel profile %s for
  org %s"); `DEBUG` after the call with counts ("SecIntel profile: name=%s
  rules=%d"); `WARNING` on 404 / empty payload; `ERROR` on unexpected
...
```text
specs/635-mist-get-org-sec-intel-profile/
├── plan.md              # This file
├── research.md          # Phase 0 - SDK signature, PK strategy, naming, menu placement
├── data-model.md        # Phase 1 - response entities + DDL + PK registration
├── quickstart.md        # Phase 1 - local run + .env + quality gates
├── contracts/
│   └── get_org_sec_intel_profile.md   # Phase 1 - HTTP + SDK contract
...
```text
MistHelper.py            # New method on SecurityExportUtils class + PK
                         # strategy entry + menu 89 registration. No new
                         # modules; same single-file monolith.
README.md                # Operation count bump + new row in the menu table
                         # for op 89 (getOrgSecIntelProfile).
...
new public method on the `SecurityExportUtils` class in `MistHelper.py` (the
class that already owns other org-security read exports; if it does not yet
exist, it is created adjacent to `LicenseExportUtils` since SecIntel profiles
are org-scoped configuration objects that fit the same shape). The menu
number proposal is **89**, chosen because operations 1-89 are the Safe Org
Exports cluster and 89 is the next available slot below the resource-intensive
block at 97-101. The full menu list is re-verified at task generation time; if
89 collides with an in-flight feature branch, the next free integer in the
...
- **Principle II (Class-Based)**: PASS -- All work lives on
  `SecurityExportUtils`. No wrappers introduced. Flattening helpers, if
