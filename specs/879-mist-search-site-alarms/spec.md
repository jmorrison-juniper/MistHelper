# Feature Specification: Mist API Read Operation -- searchSiteAlarms

**Feature Branch**: `879-mist-search-site-alarms`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `searchSiteAlarms` and add it as a new MistHelper menu item."
...
- **Tag**: `Sites Alarms`
- **mistapi SDK module**: `mistapi.api.v1.sites.alarms.search`
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchSiteAlarms` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.alarms.search.searchSiteAlarms()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.alarms.search.searchSiteAlarms()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `searchSiteAlarms` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
