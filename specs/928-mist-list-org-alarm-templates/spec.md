# Feature Specification: Mist API Read Operation -- listOrgAlarmTemplates

**Feature Branch**: `928-mist-list-org-alarm-templates`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `listOrgAlarmTemplates` and add it as a new MistHelper menu item."
...
(org / site / device as applicable), and receives the JSON payload exposed by `listOrgAlarmTemplates` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.alarmtemplates.listOrgAlarmTemplates()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.alarmtemplates.listOrgAlarmTemplates()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `listOrgAlarmTemplates` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
