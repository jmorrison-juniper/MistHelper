# Feature Specification: Mist API Read Operation -- getOrgE911Report

**Feature Branch**: `916-mist-get-org-e911-report`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `getOrgE911Report` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs Reports`
- **mistapi SDK module**: `mistapi.api.v1.orgs.exports`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getOrgE911Report` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.exports.getOrgE911Report()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.exports.getOrgE911Report()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `getOrgE911Report` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
