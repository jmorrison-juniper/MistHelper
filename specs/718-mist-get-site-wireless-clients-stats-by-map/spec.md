# Feature Specification: Mist API Read Operation -- getSiteWirelessClientsStatsByMap

**Feature Branch**: `718-mist-get-site-wireless-clients-stats-by-map`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getSiteWirelessClientsStatsByMap` and add it as a new MistHelper menu item."
...
- **mistapi SDK module**: `mistapi.api.v1.sites.stats.maps.clients`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getSiteWirelessClientsStatsByMap` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.stats.maps.clients.getSiteWirelessClientsStatsByMap()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.stats.maps.clients.getSiteWirelessClientsStatsByMap()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getSiteWirelessClientsStatsByMap` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
