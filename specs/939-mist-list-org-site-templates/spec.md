# Feature Specification: Mist API Read Operation -- listOrgSiteTemplates

**Feature Branch**: `939-mist-list-org-site-templates`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `listOrgSiteTemplates` and add it as a new MistHelper menu item."
...
- **mistapi SDK module**: `mistapi.api.v1.orgs.sitetemplates`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listOrgSiteTemplates` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.sitetemplates.listOrgSiteTemplates()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.sitetemplates.listOrgSiteTemplates()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `listOrgSiteTemplates` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
