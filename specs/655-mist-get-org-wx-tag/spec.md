# Feature Specification: Mist API Read Operation -- getOrgWxTag

**Feature Branch**: `655-mist-get-org-wx-tag`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getOrgWxTag` and add it as a new MistHelper menu item."
...
(org / site / device as applicable), and receives the JSON payload exposed by `getOrgWxTag` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.wxtags.getOrgWxTag()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.wxtags.getOrgWxTag()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getOrgWxTag` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
