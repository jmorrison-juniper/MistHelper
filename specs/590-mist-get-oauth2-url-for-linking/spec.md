# Feature Specification: Mist API Read Operation -- getOauth2UrlForLinking

**Feature Branch**: `590-mist-get-oauth2-url-for-linking`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getOauth2UrlForLinking` and add it as a new MistHelper menu item."
...
- **Tag**: `Self OAuth2`
- **mistapi SDK module**: `mistapi.api.v1.self.oauth`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getOauth2UrlForLinking` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.self.oauth.getOauth2UrlForLinking()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.self.oauth.getOauth2UrlForLinking()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getOauth2UrlForLinking` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
