# Feature Specification: Mist API Read Operation -- searchSiteOspfStats

**Feature Branch**: `893-mist-search-site-ospf-stats`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `searchSiteOspfStats` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `searchSiteOspfStats`
- **Method**: `GET`
- **Path**: `/api/v1/sites/{site_id}/stats/ospf_peers/search`
- **Tag**: `Sites Stats - Ospf`
- **mistapi SDK module**: `mistapi.api.v1.sites.stats.ospf_peers.search`
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchSiteOspfStats` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.stats.ospf_peers.search.searchSiteOspfStats()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.stats.ospf_peers.search.searchSiteOspfStats()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `searchSiteOspfStats` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
