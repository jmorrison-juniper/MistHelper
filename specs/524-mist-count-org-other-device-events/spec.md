# Feature Specification: Mist API Read Operation -- countOrgOtherDeviceEvents

**Feature Branch**: `524-mist-count-org-other-device-events`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `countOrgOtherDeviceEvents` and add it as a new MistHelper menu item."
...
- **mistapi SDK module**: `mistapi.api.v1.orgs.otherdevices.events.count`
...
(org / site / device as applicable), and receives the JSON payload exposed by `countOrgOtherDeviceEvents` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.otherdevices.events.count.countOrgOtherDeviceEvents()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.otherdevices.events.count.countOrgOtherDeviceEvents()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `countOrgOtherDeviceEvents` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
