# Feature Specification: Mist API Read Operation -- searchMspOrgs

**Feature Branch**: `860-mist-search-msp-orgs`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `searchMspOrgs` and add it as a new MistHelper menu item."
...
- **Tag**: `MSPs Orgs`
- **mistapi SDK module**: `mistapi.api.v1.msps.orgs.search`
...
- `name` (optional)
- `org_id` (optional)
- `sub_insufficient` (optional)
- `trial_enabled` (optional)
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchMspOrgs` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.msps.orgs.search.searchMspOrgs()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.msps.orgs.search.searchMspOrgs()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `searchMspOrgs` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
