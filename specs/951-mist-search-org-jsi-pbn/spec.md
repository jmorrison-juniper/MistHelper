# Feature Specification: Mist API Read Operation -- searchOrgJsiPbn

**Feature Branch**: `951-mist-search-org-jsi-pbn`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `searchOrgJsiPbn` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `searchOrgJsiPbn`
- **Method**: `GET`
- **Path**: `/api/v1/orgs/{org_id}/jsi/pbn/search`
- **Tag**: `Orgs JSI`
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchOrgJsiPbn` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.jsi.searchOrgJsiPbn()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.jsi.searchOrgJsiPbn()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `searchOrgJsiPbn` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
