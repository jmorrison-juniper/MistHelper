# Feature Specification: Mist API Read Operation -- countOrgSwOrGwPorts

**Feature Branch**: `529-mist-count-org-sw-or-gw-ports`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `countOrgSwOrGwPorts` and add it as a new MistHelper menu item."
...
- **mistapi SDK module**: `mistapi.api.v1.orgs.stats.ports.count`
...
- `tx_bps` (optional)
- `rx_bps` (optional)
- `tx_mcast_pkts` (optional)
- `tx_bcast_pkts` (optional)
- `rx_mcast_pkts` (optional)
- `rx_bcast_pkts` (optional)
- `speed` (optional)
- `stp_state` (optional)
- `stp_role` (optional)
- `auth_state` (optional)
...
(org / site / device as applicable), and receives the JSON payload exposed by `countOrgSwOrGwPorts` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.stats.ports.count.countOrgSwOrGwPorts()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.stats.ports.count.countOrgSwOrGwPorts()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `countOrgSwOrGwPorts` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
