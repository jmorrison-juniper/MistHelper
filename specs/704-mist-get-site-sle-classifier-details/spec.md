# Feature Specification: Mist API Read Operation -- getSiteSleClassifierDetails

**Feature Branch**: `704-mist-get-site-sle-classifier-details`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getSiteSleClassifierDetails` and add it as a new MistHelper menu item."
...
This API Endpoint is deprecated and replaced by [Get Site SLE Classifier Summary Trend](../../documentation/api/sites/GET_sites_site_id_sle_scope_scope_id_metric_metric_classifier_classifier_summary-trend.md)
...
(org / site / device as applicable), and receives the JSON payload exposed by `getSiteSleClassifierDetails` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.sle.metric.classifier.summary.getSiteSleClassifierDetails()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.sle.metric.classifier.summary.getSiteSleClassifierDetails()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getSiteSleClassifierDetails` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
