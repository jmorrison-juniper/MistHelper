# Feature Specification: Mist API Read Operation -- getOrgMxEdgeCluster

**Feature Branch**: `616-mist-get-org-mx-edge-cluster`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getOrgMxEdgeCluster` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs MxClusters`
- **mistapi SDK module**: `mistapi.api.v1.orgs.mxclusters`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getOrgMxEdgeCluster` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.mxclusters.getOrgMxEdgeCluster()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.mxclusters.getOrgMxEdgeCluster()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getOrgMxEdgeCluster` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
