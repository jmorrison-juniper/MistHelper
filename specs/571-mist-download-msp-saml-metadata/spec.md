# Feature Specification: Mist API Read Operation -- downloadMspSamlMetadata

**Feature Branch**: `571-mist-download-msp-saml-metadata`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `downloadMspSamlMetadata` and add it as a new MistHelper menu item."
...
- **Tag**: `MSPs SSO`
- **mistapi SDK module**: `mistapi.api.v1.msps.ssos.metadata_xml`
...
(org / site / device as applicable), and receives the JSON payload exposed by `downloadMspSamlMetadata` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.msps.ssos.metadata_xml.downloadMspSamlMetadata()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.msps.ssos.metadata_xml.downloadMspSamlMetadata()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `downloadMspSamlMetadata` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
