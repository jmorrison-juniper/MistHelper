# Feature Specification: Mist API Read Operation -- getOrgMarvisClientInsights

**Feature Branch**: `919-mist-get-org-marvis-client-insights`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `getOrgMarvisClientInsights` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `getOrgMarvisClientInsights`
- **Method**: `GET`
- **Path**: `/api/v1/orgs/{org_id}/insights/marvisclient/{marvisclient_id}/marvisclient-metrics`
- **Tag**: `Orgs Clients - Marvis`
- **mistapi SDK module**: `mistapi.api.v1.orgs.insights`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getOrgMarvisClientInsights` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.insights.getOrgMarvisClientInsights()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.insights.getOrgMarvisClientInsights()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `getOrgMarvisClientInsights` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
