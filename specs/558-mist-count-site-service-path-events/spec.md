# Feature Specification: Mist API Read Operation -- countSiteServicePathEvents

**Feature Branch**: `558-mist-count-site-service-path-events`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `countSiteServicePathEvents` and add it as a new MistHelper menu item."
...
- **Tag**: `Sites Services`
- **mistapi SDK module**: `mistapi.api.v1.sites.services.events.count`
...
(org / site / device as applicable), and receives the JSON payload exposed by `countSiteServicePathEvents` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.services.events.count.countSiteServicePathEvents()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.services.events.count.countSiteServicePathEvents()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `countSiteServicePathEvents` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
