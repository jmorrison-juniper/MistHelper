# Feature Specification: Mist API Read Operation -- getSiteAssetFilter

**Feature Branch**: `668-mist-get-site-asset-filter`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getSiteAssetFilter` and add it as a new MistHelper menu item."
...
- **mistapi SDK module**: `mistapi.api.v1.sites.assetfilters`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getSiteAssetFilter` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.assetfilters.getSiteAssetFilter()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.assetfilters.getSiteAssetFilter()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getSiteAssetFilter` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
