# Feature Specification: Mist API Read Operation -- searchOrgJsiAssetsAndContracts

**Feature Branch**: `865-mist-search-org-jsi-assets-and-contracts`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `searchOrgJsiAssetsAndContracts` and add it as a new MistHelper menu item."
...
- `sku` (optional)
- `status` (optional)
- `warranty_type` (optional)
- `eol_after` (optional)
- `eol_before` (optional)
- `eos_after` (optional)
- `eos_before` (optional)
- `version_eos_after` (optional)
- `version_eos_before` (optional)
- `has_support` (optional)
- `sirt_id` (optional)
- `pbn_id` (optional)
- `text` (optional)
- `limit` (optional)
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchOrgJsiAssetsAndContracts` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.jsi.inventory.search.searchOrgJsiAssetsAndContracts()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.jsi.inventory.search.searchOrgJsiAssetsAndContracts()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `searchOrgJsiAssetsAndContracts` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
