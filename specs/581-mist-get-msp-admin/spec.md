# Feature Specification: Mist API Read Operation -- getMspAdmin

**Feature Branch**: `581-mist-get-msp-admin`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getMspAdmin` and add it as a new MistHelper menu item."
...
(org / site / device as applicable), and receives the JSON payload exposed by `getMspAdmin` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.msps.admins.getMspAdmin()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.msps.admins.getMspAdmin()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getMspAdmin` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
