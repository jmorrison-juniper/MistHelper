# Feature Specification: Mist API Read Operation -- listSiteMapStacks

**Feature Branch**: `818-mist-list-site-map-stacks`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listSiteMapStacks` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `listSiteMapStacks`
- **Method**: `GET`
- **Path**: `/api/v1/sites/{site_id}/mapstacks`
- **Tag**: `Sites Map Stacks`
- **mistapi SDK module**: `mistapi.api.v1.sites.mapstacks`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listSiteMapStacks` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.mapstacks.listSiteMapStacks()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.mapstacks.listSiteMapStacks()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listSiteMapStacks` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
