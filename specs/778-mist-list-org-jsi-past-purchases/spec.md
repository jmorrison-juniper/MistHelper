# Feature Specification: Mist API Read Operation -- listOrgJsiPastPurchases

**Feature Branch**: `778-mist-list-org-jsi-past-purchases`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listOrgJsiPastPurchases` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs JSI`
- **mistapi SDK module**: `mistapi.api.v1.orgs.jsi.inventory`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listOrgJsiPastPurchases` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.jsi.inventory.listOrgJsiPastPurchases()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.jsi.inventory.listOrgJsiPastPurchases()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listOrgJsiPastPurchases` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
