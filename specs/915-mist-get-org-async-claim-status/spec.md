# Feature Specification: Mist API Read Operation -- getOrgAsyncClaimStatus

**Feature Branch**: `915-mist-get-org-async-claim-status`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `getOrgAsyncClaimStatus` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `getOrgAsyncClaimStatus`
- **Method**: `GET`
- **Path**: `/api/v1/orgs/{org_id}/claims/{claim_id}`
- **Tag**: `Orgs Licenses`
- **mistapi SDK module**: `mistapi.api.v1.orgs.claims`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getOrgAsyncClaimStatus` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.claims.getOrgAsyncClaimStatus()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.claims.getOrgAsyncClaimStatus()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `getOrgAsyncClaimStatus` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
