# Feature Specification: Mist API Read Operation -- getSelf

**Feature Branch**: `661-mist-get-self`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getSelf` and add it as a new MistHelper menu item."
...
- **Path**: `/api/v1/self`
- **Tag**: `Self Account`
- **mistapi SDK module**: `mistapi.api.v1.self.self`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getSelf` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.self.getSelf()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.self.getSelf()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getSelf` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
