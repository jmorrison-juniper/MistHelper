# Feature Specification: Mist API Read Operation -- getOrg128TRegistrationCommands

**Feature Branch**: `591-mist-get-org128-t-registration-commands`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getOrg128TRegistrationCommands` and add it as a new MistHelper menu item."
...
(org / site / device as applicable), and receives the JSON payload exposed by `getOrg128TRegistrationCommands` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.128routers.register_cmd.getOrg128TRegistrationCommands()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.128routers.register_cmd.getOrg128TRegistrationCommands()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getOrg128TRegistrationCommands` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
