# Feature Specification: Mist API Read Operation -- getOrgAAMWProfile

**Feature Branch**: `592-mist-get-org-a-a-m-w-profile`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getOrgAAMWProfile` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `getOrgAAMWProfile`
- **Method**: `GET`
- **Path**: `/api/v1/orgs/{org_id}/aamwprofiles/{aamwprofile_id}`
- **Tag**: `Orgs Advanced Anti Malware Profiles`
- **mistapi SDK module**: `mistapi.api.v1.orgs.aamwprofiles`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getOrgAAMWProfile` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.aamwprofiles.getOrgAAMWProfile()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.aamwprofiles.getOrgAAMWProfile()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getOrgAAMWProfile` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
