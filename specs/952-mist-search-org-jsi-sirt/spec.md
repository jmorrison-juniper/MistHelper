# Feature Specification: Mist API Read Operation -- searchOrgJsiSirt

**Feature Branch**: `952-mist-search-org-jsi-sirt`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `searchOrgJsiSirt` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `searchOrgJsiSirt`
- **Method**: `GET`
- **Path**: `/api/v1/orgs/{org_id}/jsi/sirt/search`
- **Tag**: `Orgs JSI`
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchOrgJsiSirt` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.jsi.searchOrgJsiSirt()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.jsi.searchOrgJsiSirt()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `searchOrgJsiSirt` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
