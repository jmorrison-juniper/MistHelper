# Feature Specification: Mist API Read Operation -- generateSecretFor2faVerification

**Feature Branch**: `576-mist-generate-secret-for2fa-verification`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `generateSecretFor2faVerification` and add it as a new MistHelper menu item."
...
- **Tag**: `Self MFA`
- **mistapi SDK module**: `mistapi.api.v1.self.two_factor.token`
...
(org / site / device as applicable), and receives the JSON payload exposed by `generateSecretFor2faVerification` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.self.two_factor.token.generateSecretFor2faVerification()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.self.two_factor.token.generateSecretFor2faVerification()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `generateSecretFor2faVerification` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
