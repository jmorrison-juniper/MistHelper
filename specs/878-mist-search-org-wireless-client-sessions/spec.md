# Feature Specification: Mist API Read Operation -- searchOrgWirelessClientSessions

**Feature Branch**: `878-mist-search-org-wireless-client-sessions`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `searchOrgWirelessClientSessions` and add it as a new MistHelper menu item."
...
- **mistapi SDK module**: `mistapi.api.v1.orgs.clients.sessions.search`
...
- `ap` (optional)
- `band` (optional)
- `client_family` (optional)
- `client_manufacture` (optional)
- `client_model` (optional)
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchOrgWirelessClientSessions` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.clients.sessions.search.searchOrgWirelessClientSessions()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.clients.sessions.search.searchOrgWirelessClientSessions()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `searchOrgWirelessClientSessions` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
