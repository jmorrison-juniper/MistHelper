# Feature Specification: Mist API Read Operation -- countOrgMxEdges

**Feature Branch**: `520-mist-count-org-mx-edges`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `countOrgMxEdges` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs MxEdges`
- **mistapi SDK module**: `mistapi.api.v1.orgs.mxedges.count`
...
(org / site / device as applicable), and receives the JSON payload exposed by `countOrgMxEdges` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.mxedges.count.countOrgMxEdges()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.mxedges.count.countOrgMxEdges()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `countOrgMxEdges` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
