# Feature Specification: Mist API Read Operation -- listSiteVpnsDerived

**Feature Branch**: `847-mist-list-site-vpns-derived`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listSiteVpnsDerived` and add it as a new MistHelper menu item."
...
- **Tag**: `Sites VPNs`
- **mistapi SDK module**: `mistapi.api.v1.sites.vpns.derived`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listSiteVpnsDerived` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.vpns.derived.listSiteVpnsDerived()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.vpns.derived.listSiteVpnsDerived()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listSiteVpnsDerived` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
