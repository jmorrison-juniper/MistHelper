# Feature Specification: Mist API Read Operation -- listOrgApiTokens

**Feature Branch**: `929-mist-list-org-api-tokens`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `listOrgApiTokens` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs API Tokens`
- **mistapi SDK module**: `mistapi.api.v1.orgs.apitokens`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listOrgApiTokens` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.apitokens.listOrgApiTokens()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.apitokens.listOrgApiTokens()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `listOrgApiTokens` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
