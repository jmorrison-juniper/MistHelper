# Feature Specification: Mist API Read Operation -- getSiteInsightMetricsForAP

**Feature Branch**: `923-mist-get-site-insight-metrics-for-a-p`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `getSiteInsightMetricsForAP` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `getSiteInsightMetricsForAP`
- **Method**: `GET`
- **Path**: `/api/v1/sites/{site_id}/insights/ap/{device_id}/stats`
- **Tag**: `Sites Insights`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getSiteInsightMetricsForAP` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.insights.getSiteInsightMetricsForAP()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.insights.getSiteInsightMetricsForAP()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `getSiteInsightMetricsForAP` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
