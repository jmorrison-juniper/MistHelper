...
**Feature Branch**: `914-mist-get-org-aoscx-register-cmd`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `getOrgAoscxRegisterCmd` and add it as a new MistHelper menu item."
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.aoscx.getOrgAoscxRegisterCmd()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.aoscx.getOrgAoscxRegisterCmd()` via the `mistapi` SDK when available.
