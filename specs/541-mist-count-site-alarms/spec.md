# Feature Specification: Mist API Read Operation -- countSiteAlarms

**Feature Branch**: `541-mist-count-site-alarms`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `countSiteAlarms` and add it as a new MistHelper menu item."
...
- **Tag**: `Sites Alarms`
- **mistapi SDK module**: `mistapi.api.v1.sites.alarms.count`
...
(org / site / device as applicable), and receives the JSON payload exposed by `countSiteAlarms` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.alarms.count.countSiteAlarms()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.alarms.count.countSiteAlarms()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `countSiteAlarms` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
