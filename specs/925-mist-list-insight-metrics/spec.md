# Feature Specification: Mist API Read Operation -- listInsightMetrics

**Feature Branch**: `925-mist-list-insight-metrics`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `listInsightMetrics` and add it as a new MistHelper menu item."
...
- **mistapi SDK module**: `mistapi.api.v1.const.insight_metrics`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listInsightMetrics` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.const.insight_metrics.listInsightMetrics()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.const.insight_metrics.listInsightMetrics()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `listInsightMetrics` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
