# Feature Specification: Mist API Read Operation -- listMarvisClientEventsDefinitions

**Feature Branch**: `926-mist-list-marvis-client-events-definitions`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `listMarvisClientEventsDefinitions` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `listMarvisClientEventsDefinitions`
- **Method**: `GET`
- **Path**: `/api/v1/const/marvisclient_events`
- **Tag**: `Constants Definitions`
- **mistapi SDK module**: `mistapi.api.v1.const.marvisclient_events`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listMarvisClientEventsDefinitions` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.const.marvisclient_events.listMarvisClientEventsDefinitions()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.const.marvisclient_events.listMarvisClientEventsDefinitions()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `listMarvisClientEventsDefinitions` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
