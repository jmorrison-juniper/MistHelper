# Feature Specification: Mist API Read Operation -- getOrgSsrUpgrade

**Feature Branch**: `646-mist-get-org-ssr-upgrade`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getOrgSsrUpgrade` and add it as a new MistHelper menu item."
...
(org / site / device as applicable), and receives the JSON payload exposed by `getOrgSsrUpgrade` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.ssr.upgrade.cancel.getOrgSsrUpgrade()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.ssr.upgrade.cancel.getOrgSsrUpgrade()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getOrgSsrUpgrade` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
