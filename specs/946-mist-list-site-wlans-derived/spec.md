# Feature Specification: Mist API Read Operation -- listSiteWlansDerived

**Feature Branch**: `946-mist-list-site-wlans-derived`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `listSiteWlansDerived` and add it as a new MistHelper menu item."
...
- **Tag**: `Sites Wlans`
- **mistapi SDK module**: `mistapi.api.v1.sites.wlans`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listSiteWlansDerived` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.wlans.listSiteWlansDerived()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.wlans.listSiteWlansDerived()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `listSiteWlansDerived` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
