# Feature Specification: Mist API Read Operation -- getSiteApAutoOrientation

**Feature Branch**: `664-mist-get-site-ap-auto-orientation`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getSiteApAutoOrientation` and add it as a new MistHelper menu item."
...
(org / site / device as applicable), and receives the JSON payload exposed by `getSiteApAutoOrientation` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.maps.auto_orient.getSiteApAutoOrientation()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.maps.auto_orient.getSiteApAutoOrientation()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getSiteApAutoOrientation` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
