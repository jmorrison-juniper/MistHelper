# Feature Specification: Mist API Read Operation -- countOrgWebhooksDeliveries

**Feature Branch**: `536-mist-count-org-webhooks-deliveries`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `countOrgWebhooksDeliveries` and add it as a new MistHelper menu item."
...
(org / site / device as applicable), and receives the JSON payload exposed by `countOrgWebhooksDeliveries` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.webhooks.events.count.countOrgWebhooksDeliveries()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.webhooks.events.count.countOrgWebhooksDeliveries()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `countOrgWebhooksDeliveries` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
