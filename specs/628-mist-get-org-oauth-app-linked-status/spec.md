# Feature Specification: Mist API Read Operation -- getOrgOauthAppLinkedStatus

**Feature Branch**: `628-mist-get-org-oauth-app-linked-status`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getOrgOauthAppLinkedStatus` and add it as a new MistHelper menu item."
...
- **mistapi SDK module**: `mistapi.api.v1.orgs.setting.link_accounts`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getOrgOauthAppLinkedStatus` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.setting.link_accounts.getOrgOauthAppLinkedStatus()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.setting.link_accounts.getOrgOauthAppLinkedStatus()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getOrgOauthAppLinkedStatus` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
