# Feature Specification: Mist API Read Operation -- searchSiteDevices

**Feature Branch**: `886-mist-search-site-devices`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `searchSiteDevices` and add it as a new MistHelper menu item."
...
- **Tag**: `Sites Devices`
- **mistapi SDK module**: `mistapi.api.v1.sites.devices.search`
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchSiteDevices` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.devices.search.searchSiteDevices()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.devices.search.searchSiteDevices()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `searchSiteDevices` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
