# Feature Specification: Mist API Read Operation -- searchOrgAssets

**Feature Branch**: `948-mist-search-org-assets`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `searchOrgAssets` and add it as a new MistHelper menu item."
...
- `name` (optional)
- `map_id` (optional)
- `ibeacon_uuid` (optional)
- `ibeacon_major` (optional)
- `ibeacon_minor` (optional)
- `eddystone_uid_namespace` (optional)
- `eddystone_uid_instance` (optional)
- `eddystone_url` (optional)
- `ap_mac` (optional)
- `beam` (optional)
- `rssi` (optional)
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchOrgAssets` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.stats.searchOrgAssets()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.stats.searchOrgAssets()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `searchOrgAssets` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
