# Feature Specification: Mist API Read Operation -- listMspAuditLogs

**Feature Branch**: `751-mist-list-msp-audit-logs`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listMspAuditLogs` and add it as a new MistHelper menu item."
...
(org / site / device as applicable), and receives the JSON payload exposed by `listMspAuditLogs` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.msps.logs.listMspAuditLogs()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.msps.logs.listMspAuditLogs()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listMspAuditLogs` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
