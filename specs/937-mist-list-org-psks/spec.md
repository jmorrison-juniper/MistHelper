# Feature Specification: Mist API Read Operation -- listOrgPsks

**Feature Branch**: `937-mist-list-org-psks`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `listOrgPsks` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs Psks`
- **mistapi SDK module**: `mistapi.api.v1.orgs.psks`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listOrgPsks` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.psks.listOrgPsks()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.psks.listOrgPsks()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `listOrgPsks` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
