# Feature Specification: Mist API Read Operation -- listApLedDefinition

**Feature Branch**: `730-mist-list-ap-led-definition`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listApLedDefinition` and add it as a new MistHelper menu item."
...
- **mistapi SDK module**: `mistapi.api.v1.const.ap_led_status`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listApLedDefinition` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.const.ap_led_status.listApLedDefinition()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.const.ap_led_status.listApLedDefinition()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listApLedDefinition` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
