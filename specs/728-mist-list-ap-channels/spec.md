# Feature Specification: Mist API Read Operation -- listApChannels

**Feature Branch**: `728-mist-list-ap-channels`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listApChannels` and add it as a new MistHelper menu item."
...
- **mistapi SDK module**: `mistapi.api.v1.const.ap_channels`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listApChannels` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.const.ap_channels.listApChannels()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.const.ap_channels.listApChannels()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listApChannels` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
