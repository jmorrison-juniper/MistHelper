# Feature Specification: Mist API Read Operation -- getOrgMistScep

**Feature Branch**: `614-mist-get-org-mist-scep`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getOrgMistScep` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs SCEP`
- **mistapi SDK module**: `mistapi.api.v1.orgs.setting.mist_scep`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getOrgMistScep` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.setting.mist_scep.getOrgMistScep()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.setting.mist_scep.getOrgMistScep()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getOrgMistScep` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
