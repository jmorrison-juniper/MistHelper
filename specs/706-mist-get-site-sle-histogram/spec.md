# Feature Specification: Mist API Read Operation -- getSiteSleHistogram

**Feature Branch**: `706-mist-get-site-sle-histogram`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getSiteSleHistogram` and add it as a new MistHelper menu item."
...
- **Tag**: `Sites SLEs`
- **mistapi SDK module**: `mistapi.api.v1.sites.sle.metric.histogram`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getSiteSleHistogram` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.sle.metric.histogram.getSiteSleHistogram()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.sle.metric.histogram.getSiteSleHistogram()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getSiteSleHistogram` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
