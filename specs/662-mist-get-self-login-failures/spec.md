# Feature Specification: Mist API Read Operation -- getSelfLoginFailures

**Feature Branch**: `662-mist-get-self-login-failures`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getSelfLoginFailures` and add it as a new MistHelper menu item."
...
- **Tag**: `Self Account`
- **mistapi SDK module**: `mistapi.api.v1.self.login_failures`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getSelfLoginFailures` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.self.login_failures.getSelfLoginFailures()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.self.login_failures.getSelfLoginFailures()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getSelfLoginFailures` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
