# Feature Specification: Mist API Read Operation -- getSiteSsrUpgrade

**Feature Branch**: `711-mist-get-site-ssr-upgrade`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getSiteSsrUpgrade` and add it as a new MistHelper menu item."
...
(org / site / device as applicable), and receives the JSON payload exposed by `getSiteSsrUpgrade` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.ssr.upgrade.getSiteSsrUpgrade()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.ssr.upgrade.getSiteSsrUpgrade()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getSiteSsrUpgrade` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
