# Feature Specification: Mist API Read Operation -- listSiteSleImpactedWirelessClients

**Feature Branch**: `839-mist-list-site-sle-impacted-wireless-clients`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listSiteSleImpactedWirelessClients` and add it as a new MistHelper menu item."
...
- **Tag**: `Sites SLEs`
- **mistapi SDK module**: `mistapi.api.v1.sites.sle.metric.impacted_users`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listSiteSleImpactedWirelessClients` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.sle.metric.impacted_users.listSiteSleImpactedWirelessClients()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.sle.metric.impacted_users.listSiteSleImpactedWirelessClients()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listSiteSleImpactedWirelessClients` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
