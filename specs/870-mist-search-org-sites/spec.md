# Feature Specification: Mist API Read Operation -- searchOrgSites

**Feature Branch**: `870-mist-search-org-sites`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `searchOrgSites` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs Sites`
- **mistapi SDK module**: `mistapi.api.v1.orgs.sites.search`
...
- `analytic_enabled` (optional)
- `app_waking` (optional)
- `asset_enabled` (optional)
- `auto_upgrade_enabled` (optional)
- `auto_upgrade_version` (optional)
- `country_code` (optional)
- `honeypot_enabled` (optional)
- `id` (optional)
- `locate_unconnected` (optional)
- `mesh_enabled` (optional)
- `name` (optional)
- `rogue_enabled` (optional)
- `remote_syslog_enabled` (optional)
- `rtsa_enabled` (optional)
- `vna_enabled` (optional)
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchOrgSites` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.sites.search.searchOrgSites()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.sites.search.searchOrgSites()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `searchOrgSites` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
