# Feature Specification: Mist API Read Operation -- searchOrgMarvisClientEvents

**Feature Branch**: `953-mist-search-org-marvis-client-events`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `searchOrgMarvisClientEvents` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `searchOrgMarvisClientEvents`
- **Method**: `GET`
- **Path**: `/api/v1/orgs/{org_id}/marvisclients/events/search`
- **Tag**: `Orgs Clients - Marvis`
- **mistapi SDK module**: `mistapi.api.v1.orgs.marvisclients`
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchOrgMarvisClientEvents` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.marvisclients.searchOrgMarvisClientEvents()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.marvisclients.searchOrgMarvisClientEvents()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `searchOrgMarvisClientEvents` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
