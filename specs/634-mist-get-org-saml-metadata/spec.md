# Feature Specification: Mist API Read Operation -- getOrgSamlMetadata

**Feature Branch**: `634-mist-get-org-saml-metadata`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getOrgSamlMetadata` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs SSO`
- **mistapi SDK module**: `mistapi.api.v1.orgs.ssos.metadata`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getOrgSamlMetadata` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.ssos.metadata.getOrgSamlMetadata()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.ssos.metadata.getOrgSamlMetadata()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getOrgSamlMetadata` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
