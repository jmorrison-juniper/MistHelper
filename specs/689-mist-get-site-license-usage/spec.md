# Feature Specification: Mist API Read Operation -- getSiteLicenseUsage

**Feature Branch**: `689-mist-get-site-license-usage`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getSiteLicenseUsage` and add it as a new MistHelper menu item."
...
- **Tag**: `Sites Licenses`
- **mistapi SDK module**: `mistapi.api.v1.sites.licenses.usages`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getSiteLicenseUsage` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.licenses.usages.getSiteLicenseUsage()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.licenses.usages.getSiteLicenseUsage()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getSiteLicenseUsage` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
