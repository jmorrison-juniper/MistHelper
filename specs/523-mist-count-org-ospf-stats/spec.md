# Feature Specification: Mist API Read Operation -- countOrgOspfStats

**Feature Branch**: `523-mist-count-org-ospf-stats`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `countOrgOspfStats` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `countOrgOspfStats`
- **Method**: `GET`
- **Path**: `/api/v1/orgs/{org_id}/stats/ospf_peers/count`
- **Tag**: `Orgs Stats - Ospf`
- **mistapi SDK module**: `mistapi.api.v1.orgs.stats.ospf_peers.count`
...
(org / site / device as applicable), and receives the JSON payload exposed by `countOrgOspfStats` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.stats.ospf_peers.count.countOrgOspfStats()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.stats.ospf_peers.count.countOrgOspfStats()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `countOrgOspfStats` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
