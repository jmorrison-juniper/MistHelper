# Feature Specification: Mist API Read Operation -- listSiteSleImpactedApplications

**Feature Branch**: `832-mist-list-site-sle-impacted-applications`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listSiteSleImpactedApplications` and add it as a new MistHelper menu item."
...
- **Tag**: `Sites SLEs`
- **mistapi SDK module**: `mistapi.api.v1.sites.sle.metric.impacted_applications`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listSiteSleImpactedApplications` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.sle.metric.impacted_applications.listSiteSleImpactedApplications()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.sle.metric.impacted_applications.listSiteSleImpactedApplications()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listSiteSleImpactedApplications` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
