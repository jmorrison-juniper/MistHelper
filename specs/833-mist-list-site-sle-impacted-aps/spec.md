# Feature Specification: Mist API Read Operation -- listSiteSleImpactedAps

**Feature Branch**: `833-mist-list-site-sle-impacted-aps`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listSiteSleImpactedAps` and add it as a new MistHelper menu item."
...
- **Tag**: `Sites SLEs`
- **mistapi SDK module**: `mistapi.api.v1.sites.sle.metric.impacted_aps`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listSiteSleImpactedAps` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.sle.metric.impacted_aps.listSiteSleImpactedAps()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.sle.metric.impacted_aps.listSiteSleImpactedAps()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listSiteSleImpactedAps` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
