# Feature Specification: Mist API Read Operation -- listSiteCurrentRrmNeighbors

**Feature Branch**: `810-mist-list-site-current-rrm-neighbors`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listSiteCurrentRrmNeighbors` and add it as a new MistHelper menu item."
...
- **Tag**: `Sites RRM`
- **mistapi SDK module**: `mistapi.api.v1.sites.rrm.neighbors.band`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listSiteCurrentRrmNeighbors` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.rrm.neighbors.band.listSiteCurrentRrmNeighbors()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.rrm.neighbors.band.listSiteCurrentRrmNeighbors()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listSiteCurrentRrmNeighbors` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
