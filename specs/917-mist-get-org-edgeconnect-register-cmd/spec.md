# Feature Specification: Mist API Read Operation -- getOrgEdgeconnectRegisterCmd

**Feature Branch**: `917-mist-get-org-edgeconnect-register-cmd`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `getOrgEdgeconnectRegisterCmd` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `getOrgEdgeconnectRegisterCmd`
- **Method**: `GET`
- **Path**: `/api/v1/orgs/{org_id}/edgeconnect/register_cmd`
- **Tag**: `Orgs Devices - EdgeConnect`
- **mistapi SDK module**: `mistapi.api.v1.orgs.edgeconnect`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getOrgEdgeconnectRegisterCmd` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.edgeconnect.getOrgEdgeconnectRegisterCmd()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.edgeconnect.getOrgEdgeconnectRegisterCmd()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `getOrgEdgeconnectRegisterCmd` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
