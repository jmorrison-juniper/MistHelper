# Feature Specification: Mist API Read Operation -- countOrgDeviceLastConfigs

**Feature Branch**: `513-mist-count-org-device-last-configs`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `countOrgDeviceLastConfigs` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs Devices`
- **mistapi SDK module**: `mistapi.api.v1.orgs.devices.last_config.count`
...
(org / site / device as applicable), and receives the JSON payload exposed by `countOrgDeviceLastConfigs` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.devices.last_config.count.countOrgDeviceLastConfigs()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.devices.last_config.count.countOrgDeviceLastConfigs()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `countOrgDeviceLastConfigs` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
