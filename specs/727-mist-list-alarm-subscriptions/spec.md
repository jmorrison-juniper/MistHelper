# Feature Specification: Mist API Read Operation -- listAlarmSubscriptions

**Feature Branch**: `727-mist-list-alarm-subscriptions`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listAlarmSubscriptions` and add it as a new MistHelper menu item."
...
- **Tag**: `Self Alarms`
- **mistapi SDK module**: `mistapi.api.v1.self.subscriptions`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listAlarmSubscriptions` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.self.subscriptions.listAlarmSubscriptions()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.self.subscriptions.listAlarmSubscriptions()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listAlarmSubscriptions` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
