# Feature Specification: Mist API Read Operation -- countOrgMarvisClientsStats

**Feature Branch**: `910-mist-count-org-marvis-clients-stats`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `countOrgMarvisClientsStats` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `countOrgMarvisClientsStats`
- **Method**: `GET`
- **Path**: `/api/v1/orgs/{org_id}/stats/marvisclients/count`
...
(org / site / device as applicable), and receives the JSON payload exposed by `countOrgMarvisClientsStats` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.stats.countOrgMarvisClientsStats()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.stats.countOrgMarvisClientsStats()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `countOrgMarvisClientsStats` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
