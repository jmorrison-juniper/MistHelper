# Feature Specification: Mist API Read Operation -- getOrgRfTemplate

**Feature Branch**: `633-mist-get-org-rf-template`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getOrgRfTemplate` and add it as a new MistHelper menu item."
...
- **mistapi SDK module**: `mistapi.api.v1.orgs.rftemplates`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getOrgRfTemplate` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.rftemplates.getOrgRfTemplate()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.rftemplates.getOrgRfTemplate()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getOrgRfTemplate` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
