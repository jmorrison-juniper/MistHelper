# Feature Specification: Mist API Read Operation -- listOrgWxRules

**Feature Branch**: `793-mist-list-org-wx-rules`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listOrgWxRules` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs WxRules`
- **mistapi SDK module**: `mistapi.api.v1.orgs.wxrules`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listOrgWxRules` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.wxrules.listOrgWxRules()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.wxrules.listOrgWxRules()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listOrgWxRules` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
