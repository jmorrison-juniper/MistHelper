# Feature Specification: Mist API Read Operation -- getSiteSettingDerived

**Feature Branch**: `702-mist-get-site-setting-derived`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getSiteSettingDerived` and add it as a new MistHelper menu item."
...
- **Tag**: `Sites Setting`
- **mistapi SDK module**: `mistapi.api.v1.sites.setting.derived`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getSiteSettingDerived` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.setting.derived.getSiteSettingDerived()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.setting.derived.getSiteSettingDerived()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getSiteSettingDerived` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
