...
- [ ] T009 [US1] Implement collector to fetch org templates, site assignments, and SSID data using `mistapi` in `src/ssid_consolidation/collector.py` (respect rate limits) [src/ssid_consolidation/collector.py]
- [ ] T010 [US1] Implement caching layer (SQLite) honoring `SSID_CONSOLIDATION_CACHE_MINUTES` in `src/ssid_consolidation/cache.py` and integrate with `collector.py` [src/ssid_consolidation/cache.py]
- [ ] T011 [US1] Implement deviation analysis (per-cluster and cross-cluster drift) in `src/ssid_consolidation/analysis.py` following FR-010a/FR-010b [src/ssid_consolidation/analysis.py]
- [ ] T012 [US1] Implement CSV + SQLite export using `DataExporter.write_with_format_selection()` in `src/ssid_consolidation/exporter.py` (output to `data/ssid-consolidation/`) [src/ssid_consolidation/exporter.py]
...
- [ ] T014 [US1] Wire CLI/menu entry in `MistHelper.py` to invoke `SSIDTemplateConsolidationManager.phase1_collect()` via `menu 159` [MistHelper.py]
