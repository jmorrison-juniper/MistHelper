# Feature Specification: Mist API Read Operation -- getOrgMxEdgeUpgradeInfo

**Feature Branch**: `618-mist-get-org-mx-edge-upgrade-info`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getOrgMxEdgeUpgradeInfo` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `getOrgMxEdgeUpgradeInfo`
- **Method**: `GET`
- **Path**: `/api/v1/orgs/{org_id}/mxedges/versions`
- **Tag**: `Orgs MxEdges`
- **mistapi SDK module**: `mistapi.api.v1.orgs.mxedges.versions`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getOrgMxEdgeUpgradeInfo` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.mxedges.versions.getOrgMxEdgeUpgradeInfo()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.mxedges.versions.getOrgMxEdgeUpgradeInfo()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getOrgMxEdgeUpgradeInfo` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
