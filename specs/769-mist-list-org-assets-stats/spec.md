# Feature Specification: Mist API Read Operation -- listOrgAssetsStats

**Feature Branch**: `769-mist-list-org-assets-stats`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listOrgAssetsStats` and add it as a new MistHelper menu item."
...
- **mistapi SDK module**: `mistapi.api.v1.orgs.stats.assets`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listOrgAssetsStats` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.stats.assets.listOrgAssetsStats()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.stats.assets.listOrgAssetsStats()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listOrgAssetsStats` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
