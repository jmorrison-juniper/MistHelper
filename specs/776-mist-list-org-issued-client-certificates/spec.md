# Feature Specification: Mist API Read Operation -- listOrgIssuedClientCertificates

**Feature Branch**: `776-mist-list-org-issued-client-certificates`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listOrgIssuedClientCertificates` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs SCEP`
- **mistapi SDK module**: `mistapi.api.v1.orgs.setting.mist_scep.client_certs`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listOrgIssuedClientCertificates` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.setting.mist_scep.client_certs.listOrgIssuedClientCertificates()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.setting.mist_scep.client_certs.listOrgIssuedClientCertificates()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listOrgIssuedClientCertificates` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
