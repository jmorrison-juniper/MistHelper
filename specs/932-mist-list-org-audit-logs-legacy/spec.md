# Feature Specification: Mist API Read Operation -- listOrgAuditLogsLegacy

**Feature Branch**: `932-mist-list-org-audit-logs-legacy`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `listOrgAuditLogsLegacy` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs Logs`
- **mistapi SDK module**: _No installed mistapi SDK module found._
...
(org / site / device as applicable), and receives the JSON payload exposed by `listOrgAuditLogsLegacy` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes the OpenAPI operation `listOrgAuditLogsLegacy` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes the OpenAPI operation `listOrgAuditLogsLegacy` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `listOrgAuditLogsLegacy` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
