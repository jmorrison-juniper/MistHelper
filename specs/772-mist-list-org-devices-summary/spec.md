# Feature Specification: Mist API Read Operation -- listOrgDevicesSummary

**Feature Branch**: `772-mist-list-org-devices-summary`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listOrgDevicesSummary` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs Devices`
- **mistapi SDK module**: `mistapi.api.v1.orgs.devices.summary`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listOrgDevicesSummary` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.devices.summary.listOrgDevicesSummary()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.devices.summary.listOrgDevicesSummary()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listOrgDevicesSummary` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
