# Feature Specification: Mist API Read Operation -- searchOrgMarvisClientsStats

**Feature Branch**: `954-mist-search-org-marvis-clients-stats`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `searchOrgMarvisClientsStats` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `searchOrgMarvisClientsStats`
- **Method**: `GET`
- **Path**: `/api/v1/orgs/{org_id}/stats/marvisclients/search`
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchOrgMarvisClientsStats` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.stats.searchOrgMarvisClientsStats()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.stats.searchOrgMarvisClientsStats()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `searchOrgMarvisClientsStats` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
