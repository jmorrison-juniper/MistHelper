# Feature Specification: Mist API Read Operation -- getSiteInsightMetricsForMxEdge

**Feature Branch**: `686-mist-get-site-insight-metrics-for-mx-edge`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getSiteInsightMetricsForMxEdge` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `getSiteInsightMetricsForMxEdge`
- **Method**: `GET`
- **Path**: `/api/v1/sites/{site_id}/insights/mxedge/{device_mac}/{metric}`
- **Tag**: `Sites Insights`
- **mistapi SDK module**: `mistapi.api.v1.sites.insights.mxedge`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getSiteInsightMetricsForMxEdge` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.insights.mxedge.getSiteInsightMetricsForMxEdge()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.insights.mxedge.getSiteInsightMetricsForMxEdge()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getSiteInsightMetricsForMxEdge` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
