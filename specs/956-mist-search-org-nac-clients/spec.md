# Feature Specification: Mist API Read Operation -- searchOrgNacClients

**Feature Branch**: `956-mist-search-org-nac-clients`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `searchOrgNacClients` and add it as a new MistHelper menu item."
...
- **mistapi SDK module**: `mistapi.api.v1.orgs.nac_clients`
...
- `ap` (optional)
- `auth_type` (optional)
- `cert_expiry_duration` (optional)
- `edr_managed` (optional)
- `edr_providers` (optional)
- `edr_status` (optional)
- `family` (optional)
- `hostname` (optional)
- `idp_id` (optional)
- `mac` (optional)
- `mdm_compliance` (optional)
- `mdm_provider` (optional)
- `mdm_managed` (optional)
- `mfg` (optional)
- `model` (optional)
- `nacrule_name` (optional)
- `nacrule_id` (optional)
- `nacrule_matched` (optional)
- `nas_vendor` (optional)
- `nas_ip` (optional)
- `ingress_vlan` (optional)
- `os` (optional)
- `ssid` (optional)
- `status` (optional)
- `text` (optional)
- `type` (optional)
- `usermac_label` (optional)
- `username` (optional)
- `vlan` (optional)
- `site_id` (optional)
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchOrgNacClients` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.nac_clients.searchOrgNacClients()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.nac_clients.searchOrgNacClients()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `searchOrgNacClients` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
...
**FR-008**: Update the README.md menu table with the new operation number. Add the release note as a new fragment under `changelog.d/`, named `pr-<number>.md`, `issue-<number>-<slug>.md`, or `<YYYY-MM-DD>-<slug>.md`. Never edit CHANGELOG.md on a feature branch.
