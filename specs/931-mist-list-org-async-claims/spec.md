# Feature Specification: Mist API Read Operation -- listOrgAsyncClaims

**Feature Branch**: `931-mist-list-org-async-claims`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `listOrgAsyncClaims` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `listOrgAsyncClaims`
- **Method**: `GET`
- **Path**: `/api/v1/orgs/{org_id}/claims`
- **Tag**: `Orgs Licenses`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listOrgAsyncClaims` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.claims.listOrgAsyncClaims()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.claims.listOrgAsyncClaims()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `listOrgAsyncClaims` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
