# Feature Specification: Mist API Read Operation -- listSitePsks

**Feature Branch**: `823-mist-list-site-psks`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listSitePsks` and add it as a new MistHelper menu item."
...
- **Tag**: `Sites Psks`
- **mistapi SDK module**: `mistapi.api.v1.sites.psks`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listSitePsks` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.psks.listSitePsks()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.psks.listSitePsks()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listSitePsks` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
