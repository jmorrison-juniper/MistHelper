# Feature Specification: Mist API Read Operation -- getOrgApplicationList

**Feature Branch**: `597-mist-get-org-application-list`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getOrgApplicationList` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs WxTags`
- **mistapi SDK module**: `mistapi.api.v1.orgs.wxtags.apps`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getOrgApplicationList` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.wxtags.apps.getOrgApplicationList()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.wxtags.apps.getOrgApplicationList()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getOrgApplicationList` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
