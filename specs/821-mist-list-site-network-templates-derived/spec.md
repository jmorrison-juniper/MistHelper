# Feature Specification: Mist API Read Operation -- listSiteNetworkTemplatesDerived

**Feature Branch**: `821-mist-list-site-network-templates-derived`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listSiteNetworkTemplatesDerived` and add it as a new MistHelper menu item."
...
- **mistapi SDK module**: `mistapi.api.v1.sites.networktemplates.derived`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listSiteNetworkTemplatesDerived` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.networktemplates.derived.listSiteNetworkTemplatesDerived()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.networktemplates.derived.listSiteNetworkTemplatesDerived()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listSiteNetworkTemplatesDerived` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
