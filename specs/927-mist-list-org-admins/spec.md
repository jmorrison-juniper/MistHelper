# Feature Specification: Mist API Read Operation -- listOrgAdmins

**Feature Branch**: `927-mist-list-org-admins`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `listOrgAdmins` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs Admins`
- **mistapi SDK module**: `mistapi.api.v1.orgs.admins`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listOrgAdmins` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.admins.listOrgAdmins()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.admins.listOrgAdmins()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `listOrgAdmins` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
