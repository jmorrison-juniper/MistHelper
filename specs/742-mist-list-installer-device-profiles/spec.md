# Feature Specification: Mist API Read Operation -- listInstallerDeviceProfiles

**Feature Branch**: `742-mist-list-installer-device-profiles`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listInstallerDeviceProfiles` and add it as a new MistHelper menu item."
...
- **Tag**: `Installer`
- **mistapi SDK module**: `mistapi.api.v1.installer.orgs.deviceprofiles`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listInstallerDeviceProfiles` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.installer.orgs.deviceprofiles.listInstallerDeviceProfiles()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.installer.orgs.deviceprofiles.listInstallerDeviceProfiles()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listInstallerDeviceProfiles` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
