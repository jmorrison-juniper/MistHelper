# Feature Specification: Mist API Read Operation -- searchOrgBgpStats

**Feature Branch**: `949-mist-search-org-bgp-stats`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `searchOrgBgpStats` and add it as a new MistHelper menu item."
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchOrgBgpStats` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.stats.searchOrgBgpStats()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.stats.searchOrgBgpStats()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `searchOrgBgpStats` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
