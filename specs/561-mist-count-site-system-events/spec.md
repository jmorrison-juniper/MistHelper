# Feature Specification: Mist API Read Operation -- countSiteSystemEvents

**Feature Branch**: `561-mist-count-site-system-events`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `countSiteSystemEvents` and add it as a new MistHelper menu item."
...
- **Tag**: `Sites Events`
- **mistapi SDK module**: `mistapi.api.v1.sites.events.system.count`
...
(org / site / device as applicable), and receives the JSON payload exposed by `countSiteSystemEvents` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.events.system.count.countSiteSystemEvents()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.events.system.count.countSiteSystemEvents()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `countSiteSystemEvents` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
