# Feature Specification: Mist API Read Operation -- listMspAdmins

**Feature Branch**: `750-mist-list-msp-admins`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listMspAdmins` and add it as a new MistHelper menu item."
...
- **Tag**: `MSPs Admins`
- **mistapi SDK module**: `mistapi.api.v1.msps.admins`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listMspAdmins` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.msps.admins.listMspAdmins()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.msps.admins.listMspAdmins()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listMspAdmins` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
