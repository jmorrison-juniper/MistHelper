# Feature Specification: Mist API Read Operation -- countMspAuditLogs

**Feature Branch**: `504-mist-count-msp-audit-logs`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `countMspAuditLogs` and add it as a new MistHelper menu item."
...
(org / site / device as applicable), and receives the JSON payload exposed by `countMspAuditLogs` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.msps.logs.count.countMspAuditLogs()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.msps.logs.count.countMspAuditLogs()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `countMspAuditLogs` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
