# Feature Specification: Mist API Read Operation -- getSiteRssiZone

**Feature Branch**: `697-mist-get-site-rssi-zone`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getSiteRssiZone` and add it as a new MistHelper menu item."
...
- **mistapi SDK module**: `mistapi.api.v1.sites.rssizones`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getSiteRssiZone` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.rssizones.getSiteRssiZone()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.rssizones.getSiteRssiZone()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getSiteRssiZone` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
