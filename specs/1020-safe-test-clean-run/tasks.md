...
- [ ] T001 Record the pre-change tooling baseline by running (and saving console output as an implementation note, not a new file) `python -m py_compile MistHelper.py`, `python -m ruff check MistHelper.py src tests`, `python -m black --check MistHelper.py src tests`, `python -m mypy src --config-file pyproject.toml`, and `python -m pytest --cov=src --cov=tests --cov-report=term-missing` from the repository root — this is the "before" state every later gate re-run is compared against.
...
- T044/T045 explicitly marked not-yet-run, pending an operator copying `deploy/.env.example` to `deploy/.env` (or equivalent env vars) with a real `MIST_HOST`, `MIST_APITOKEN`/`MIST_API_TOKEN`, and `org_id`, outside of source control.
