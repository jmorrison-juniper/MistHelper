# Feature Specification: Mist API Read Operation -- getOrgJseIntegration

**Feature Branch**: `610-mist-get-org-jse-integration`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getOrgJseIntegration` and add it as a new MistHelper menu item."
...
- **mistapi SDK module**: `mistapi.api.v1.orgs.setting.jse.setup`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getOrgJseIntegration` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.setting.jse.setup.getOrgJseIntegration()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.setting.jse.setup.getOrgJseIntegration()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getOrgJseIntegration` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
