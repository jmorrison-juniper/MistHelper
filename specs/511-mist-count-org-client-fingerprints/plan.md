# Implementation Plan: countOrgClientFingerprints Menu Item

**Branch**: `511-mist-count-org-client-fingerprints` | **Date**: 2026-06-28 | **Spec**: [spec.md](./spec.md)
**Input**: Feature specification from `specs/511-mist-count-org-client-fingerprints/spec.md`

## Summary

Add a new read-only MistHelper menu item that invokes the Mist API endpoint
`GET /api/v1/sites/{site_id}/insights/fingerprints/count` (operationId
`countOrgClientFingerprints`) to return aggregate counts of client device
fingerprints observed at a site, optionally grouped by a `distinct` field
(for example `family`, `model`, `os`, or `manufacturer`) within a bounded
time window. The menu item prompts the user for `site_id` plus the optional
`distinct`, `start`, `end`, `duration`, and `limit` query parameters using
`safe_input()`, calls the `mistapi` SDK exactly once, flattens the response
into one summary row plus zero or more per-bucket rows, and persists results
...
repeated runs. The new operation is proposed as menu number **79** -- the
next available slot in the Insights cluster (73-79), adjacent to existing
site insight exports. If 79 collides with an in-flight feature branch at
task-generation time, the next free integer in the same cluster is taken.
...
sole permitted interface to Mist Cloud); `requests` (transport,
transitive); `python-dotenv` (for `.env` loading of `MIST_HOST` and
`MIST_API_TOKEN`).
**Storage**: Multi-backend via
`DataExporter.write_with_format_selection()`. SQLite file
`data/mist_data.db` is the local fallback; CSV files land in `data/`;
...
(14, 18, 63-65, 90-100) is unaffected -- new item 79 sits inside the
default test sweep.
...
typical sites. The endpoint returns one aggregate JSON object (no large
result arrays), so the adaptive delay system in `delay_metrics.json` and
`tuning_data.json` continues to govern back-off without any per-endpoint
tuning.
...
site-insights export class in `MistHelper.py`, one new entry in
`ENDPOINT_PRIMARY_KEY_STRATEGIES`, two new CSV/SQLite tables
(`site_client_fingerprints_count_summary` and
`site_client_fingerprints_count_buckets`), one menu registration entry,
one README operation-count bump, one CHANGELOG line. No new dependencies,
...
- **STATUS: PASS** -- The new menu method
  `export_site_client_fingerprints_count()` stays under 25 lines, takes
  <=5 parameters (`self`, `site_id`, `distinct`, `time_window`,
  `result_limit`), and contains <=5 logical blocks (prompt collection ->
  SDK call -> summary flatten -> bucket flatten -> DataExporter call).
  Hierarchy is unchanged: one new method on an existing class. No new
  packages, modules, or top-level constants are introduced. The two
  output flatteners are inlined as single comprehension blocks; if either
...
- **STATUS: PASS** -- The new behavior is added as a method on the
  existing site-insights export class in `MistHelper.py` (the same class
  that owns adjacent site insights exports). No standalone wrapper
  function is introduced. The menu dispatch in the main loop references
  the class method directly. Variable names use full words
  (`fingerprint_bucket`, `summary_row`) -- no single-letter iterators.

### Principle III: Safety-First (NON-NEGOTIABLE)

- **STATUS: PASS** -- All user input is collected through `safe_input()`
  with explicit `context=` strings
  (`"site_client_fingerprints_count:site_id"`,
  `"site_client_fingerprints_count:distinct"`,
  `"site_client_fingerprints_count:duration"`,
  `"site_client_fingerprints_count:limit"`) so SSH / container EOF exits
  ...
  required. `site_id` is validated against the Mist UUID shape before
  the API call; on validation failure the method logs a warning and
  returns early. API token comes from `.env` via the existing
...
- **STATUS: PASS** -- After implementation the standard pipeline applies
  ...
  `version YY.MM.DD.HH.MM - add menu 79 countOrgClientFingerprints` ->
...
- **STATUS: PASS** -- All log calls use ASCII text and `%s` style
  formatting. `INFO` is emitted before the API call ("Fetching client
  fingerprint count for site %s distinct=%s duration=%s"); `DEBUG` after
  the call with summary counts ("Fingerprint count: total=%d buckets=%d
  limit=%d"); `WARNING` on 404 / empty payload; `ERROR` on unexpected
  exception with full traceback via `logging.exception`. No secrets,
...
- **STATUS: PASS** -- Every executable line in the new method, the new
  ...
  are exempt per the constitution. Any uncommented adjacent lines in the
  touched site-insights block get comments added in the same PR.
...
- **STATUS: PASS** -- The new method follows the documented pattern:
  ...
  The `DataExporter` call already emits its own per-backend log lines;
...
```text
specs/511-mist-count-org-client-fingerprints/
|-- plan.md              # This file
|-- research.md          # Phase 0 - SDK signature, PK strategy, naming, menu placement
|-- data-model.md        # Phase 1 - response entities + DDL + PK registration
|-- quickstart.md        # Phase 1 - local run + .env + quality gates
|-- contracts/
|   `-- count_org_client_fingerprints.md   # Phase 1 - HTTP + SDK contract
...
```text
MistHelper.py            # New method on the site-insights export class +
                         # PK strategy + menu 79 registration. No new
...
as a new public method on the existing site-insights export class in
`MistHelper.py` (the same class that owns the adjacent site insight
exports). The menu number proposal is **79**, chosen because operations
73-79 are the Insights cluster (per the menu category table in
`.github/copilot-instructions.md`) and 79 is the next available slot
before the Stats cluster begins at 80. The full menu list is re-verified
at task generation time; if 79 collides with an in-flight feature branch,
the next free integer in the same cluster (or, failing that, an entry in
the next safe-interactive band) is used.
...
- **Principle I (Five-Item Rule)**: PASS -- The detailed method outline
  ...
  insert (existing structure), so no level-5 hierarchy explosion.
- **Principle II (Class-Based)**: PASS -- All work lives on the existing
  site-insights export class. No wrappers introduced. Flattening
  helpers, if extracted, are added as private methods on the same class.
- **Principle III (Safety-First)**: PASS -- Phase 1 contract confirms the
  endpoint is GET only, with no destructive side effect. `safe_input()`
...
- **Principle VI (Inline Comments)**: PASS -- Phase 1 quickstart shows
  ...
  PK strategy entry and menu registration line.
