# Feature Specification: Mist API Read Operation -- countOrgClientFingerprints

**Feature Branch**: `511-mist-count-org-client-fingerprints`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `countOrgClientFingerprints` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `countOrgClientFingerprints`
- **Method**: `GET`
...
- **mistapi SDK status**: The installed SDK does not define `countOrgClientFingerprints`.
- **Nearest SDK operation**: `countSiteClientFingerprints` in `mistapi.api.v1.sites.insights`.
...
(org / site / device as applicable), and receives the JSON payload exposed by `countOrgClientFingerprints` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.insights.fingerprints.count.countOrgClientFingerprints()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.insights.fingerprints.count.countOrgClientFingerprints()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `countOrgClientFingerprints` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
