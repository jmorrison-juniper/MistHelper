# Feature Specification: Mist API Read Operation -- getOrgMxTunnel

**Feature Branch**: `620-mist-get-org-mx-tunnel`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getOrgMxTunnel` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs MxTunnels`
- **mistapi SDK module**: `mistapi.api.v1.orgs.mxtunnels`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getOrgMxTunnel` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.mxtunnels.getOrgMxTunnel()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.mxtunnels.getOrgMxTunnel()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getOrgMxTunnel` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
