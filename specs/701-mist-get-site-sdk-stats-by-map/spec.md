# Feature Specification: Mist API Read Operation -- getSiteSdkStatsByMap

**Feature Branch**: `701-mist-get-site-sdk-stats-by-map`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getSiteSdkStatsByMap` and add it as a new MistHelper menu item."
...
- **mistapi SDK module**: `mistapi.api.v1.sites.stats.maps.sdkclients`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getSiteSdkStatsByMap` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.stats.maps.sdkclients.getSiteSdkStatsByMap()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.stats.maps.sdkclients.getSiteSdkStatsByMap()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getSiteSdkStatsByMap` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
