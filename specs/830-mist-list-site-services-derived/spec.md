# Feature Specification: Mist API Read Operation -- listSiteServicesDerived

**Feature Branch**: `830-mist-list-site-services-derived`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listSiteServicesDerived` and add it as a new MistHelper menu item."
...
- **Tag**: `Sites Services`
- **mistapi SDK module**: `mistapi.api.v1.sites.services.derived`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listSiteServicesDerived` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.services.derived.listSiteServicesDerived()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.services.derived.listSiteServicesDerived()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listSiteServicesDerived` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
