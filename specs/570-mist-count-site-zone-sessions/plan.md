# Implementation Plan: countSiteZoneSessions Menu Item

**Branch**: `570-mist-count-site-zone-sessions` | **Date**: 2026-06-29 | **Spec**: [spec.md](./spec.md)
**Input**: Feature specification from `specs/570-mist-count-site-zone-sessions/spec.md`

## Summary

Add a new read-only MistHelper menu item that invokes the Mist API endpoint
`GET /api/v1/sites/{site_id}/{zone_type}/count` (operationId `countSiteZoneSessions`)
to retrieve aggregated counts of site zone visit / RSSI zone session records grouped by a
user-selected distinct attribute (for example `zone_id`, `map_id`, or `user`). The menu
item prompts the user via `safe_input()` for a `site_id`, a `zone_type` (one of
`zones` or `rssizones`), and an optional `distinct` grouping attribute plus an optional
`duration` window (default `1d`). It then invokes the `mistapi` SDK function
`mistapi.api.v1.sites.count.countSiteZoneSessions()` exactly once, flattens the
`results[]` array into one row per distinct value (each carrying the `count` integer plus
the original distinct attribute as a column), and persists the rows through
...
current maximum of 194 (Clone Device Config to Gateway Template), sitting logically in
a new Safe Site Exports extension band that begins after the existing destructive 154-194
range.
...
using a known `MIST_ORG_ID` and `MIST_SITE_ID` from `.env`. Local quality gates:
`python -m py_compile MistHelper.py`, `python -m ruff check MistHelper.py`,
`python -m black --check MistHelper.py`. The default test sweep skip list (14, 18, 63-65,
90-100) does not affect menu 195; menu 195 will be added to the sweep range
explicitly in the test runner once it ships.
...
**Performance Goals**: Single GET request completes in <=5 seconds for typical zone
session counts (the endpoint returns an aggregated `count_result[]` capped by `limit`,
...
the `zone_type` path parameter is validated against the closed set `{zones, rssizones}`
before any API call to avoid 404 round-trips.
**Scale/Scope**: One new public menu method (~22 lines) on the existing `SiteExportUtils`
class (the same class that owns the related `zones()` listing at line 13219), one new
entry in `ENDPOINT_PRIMARY_KEY_STRATEGIES`, one new CSV/SQLite table
(`site_zone_session_counts`), one menu registration entry, one README operation-count
...
- **STATUS: PASS** -- The new menu method `count_site_zone_sessions()` stays under 25
  lines, takes <=5 parameters (`self`, `site_id`, `zone_type`, `distinct`, `duration`),
  and contains <=5 logical blocks (validate zone_type -> prompt for optional filters ->
  API call -> flatten results into rows -> DataExporter call). Hierarchy is unchanged:
  one new method on an existing class. The flattener is a single comprehension; if it
...
- **STATUS: PASS** -- The new behavior is added as a method on the existing
  `SiteExportUtils` class (the same class that owns the related `zones()` site-zones
  export at MistHelper.py line 13219). No standalone wrapper function is introduced. The
  menu dispatch in the main loop references the class method directly. Variable names use
  full words (`zone_type`, `distinct_attribute`, `count_row`) -- no single-letter
  iterators.

### Principle III: Safety-First (NON-NEGOTIABLE)

- **STATUS: PASS** -- All user input is collected through `safe_input()` with explicit
  `context=` strings (`"count_site_zone_sessions:site_id"`,
  `"count_site_zone_sessions:zone_type"`,
  `"count_site_zone_sessions:distinct"`,
  `"count_site_zone_sessions:duration"`) so SSH / container EOF exits cleanly with code 0
  ...
  UUID shape and `zone_type` against the closed enum `{zones, rssizones}` before the API
...
- **STATUS: PASS** -- After implementation the standard pipeline applies without
  ...
  -> commit with `version YY.MM.DD.HH.MM - add menu 195 countSiteZoneSessions`
...
- **STATUS: PASS** -- All log calls use ASCII text and `%s` style formatting. `INFO` is
  emitted before the API call ("Counting zone sessions for site %s zone_type=%s
  distinct=%s"); `DEBUG` after the call with summary counts ("Zone session count
  response: total=%d returned=%d"); `WARNING` on 404 / empty payload; `ERROR` on
...
- **STATUS: PASS** -- Every executable line in the new method, the new PK strategy
  ...
  lines in the touched block (the existing `SiteExportUtils.zones()` neighborhood) get
...
```text
specs/570-mist-count-site-zone-sessions/
|-- plan.md              # This file
|-- research.md          # Phase 0 - SDK signature, PK strategy, naming, menu placement
|-- data-model.md        # Phase 1 - response entities + DDL + PK registration
|-- quickstart.md        # Phase 1 - local run + .env + quality gates
|-- contracts/
|   `-- count_site_zone_sessions.md   # Phase 1 - HTTP + SDK contract
...
```text
MistHelper.py            # New method on SiteExportUtils class + PK strategy + menu 195
                         # registration. No new modules; same single-file monolith.
README.md                # Operation count bump (194 -> 195) and new row in the menu
                         # table for op 195.
...
data/                    # Runtime output target (existing dir, no schema migration
                         ...
                         # DataExporter from the PK strategy entry).
```
...
owns the existing `zones()` site-zones listing at line 13219). The menu number proposal
is **195**, chosen because operations 1-194 are already assigned (current maximum at
MistHelper.py menu registry tail is 194 -- Clone Device Config to Gateway Template).
Menu 195 opens a new Safe Site Exports continuation band above the existing destructive
154-194 cluster; subsequent read-only site / zone catalogue endpoints should fall into
196-199 before reopening a new theme cluster. The full menu list will be re-verified at
task generation time; if 195 collides with an in-flight feature branch, the next free
integer is used.
...
- **Principle III (Safety-First)**: PASS -- Phase 1 contract confirms the endpoint is
  ...
  path. UUID and zone_type enum validation happen before the SDK call.
- **Principle IV (Pipeline)**: PASS -- No deviation from the standard pipeline.
- **Principle V (Observability)**: PASS -- Log statements in the design are ASCII-only
  with `%s` formatting and never include the API token or full request URLs.
