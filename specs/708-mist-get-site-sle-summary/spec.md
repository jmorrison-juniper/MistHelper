# Feature Specification: Mist API Read Operation -- getSiteSleSummary

**Feature Branch**: `708-mist-get-site-sle-summary`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getSiteSleSummary` and add it as a new MistHelper menu item."
...
This API Endpoint is deprecated and replaced by [Get Site SLE Summary Trend](../../documentation/api/sites/GET_sites_site_id_sle_scope_scope_id_metric_metric_summary-trend.md)
...
(org / site / device as applicable), and receives the JSON payload exposed by `getSiteSleSummary` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.sle.metric.summary.getSiteSleSummary()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.sle.metric.summary.getSiteSleSummary()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getSiteSleSummary` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
