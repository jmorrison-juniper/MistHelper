...
| `2447-stale-bulk-run-controls` | Stale and bulk run controls | Show stale age, reconcile state, preview a bulk action, cancel or retry selected runs, and isolate concurrent operators. | `test_run_controls/test_stale.py` covers stale display. Multi-operator tests cover site-lock isolation. | **Fail** | `test_bulk.py` and `test_isolation.py` contain no workflow tests. The 137 implementation tasks remain open. Issue #2447 tracks this work. |
...
- The complete isolated Playwright suite passed: 188 tests passed and 4 tests skipped.
...
- The complete non-browser portal suite passed: 3,897 tests passed.
...
1. Add browser journeys for bulk preview, bulk cancel, bulk retry, reconciliation, and operator isolation.
2. Add a browser-token test that proves a Mist request uses the token from the browser session.
3. Add a complete browser retry journey from a stopped or failed run to a fresh pre-check.
4. Build the digest repair into the local container.
5. Run a new live read-only Tier 3 capture and require the `verified` state.
6. Confirm that history never reports `complete` for an unverified capture.
7. Classify each conditional browser skip. Do not count a skip as proof.
