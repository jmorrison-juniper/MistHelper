# Feature Specification: Mist API Read Operation -- getMspInventoryByMac

**Feature Branch**: `583-mist-get-msp-inventory-by-mac`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getMspInventoryByMac` and add it as a new MistHelper menu item."
...
- **Tag**: `MSPs Inventory`
- **mistapi SDK module**: `mistapi.api.v1.msps.inventory`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getMspInventoryByMac` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.msps.inventory.getMspInventoryByMac()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.msps.inventory.getMspInventoryByMac()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getMspInventoryByMac` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
