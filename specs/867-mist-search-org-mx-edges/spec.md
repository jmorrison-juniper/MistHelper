# Feature Specification: Mist API Read Operation -- searchOrgMxEdges

**Feature Branch**: `867-mist-search-org-mx-edges`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `searchOrgMxEdges` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs MxEdges`
- **mistapi SDK module**: `mistapi.api.v1.orgs.mxedges.search`
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchOrgMxEdges` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.mxedges.search.searchOrgMxEdges()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.mxedges.search.searchOrgMxEdges()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `searchOrgMxEdges` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
