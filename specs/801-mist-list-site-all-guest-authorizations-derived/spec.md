# Feature Specification: Mist API Read Operation -- listSiteAllGuestAuthorizationsDerived

**Feature Branch**: `801-mist-list-site-all-guest-authorizations-derived`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listSiteAllGuestAuthorizationsDerived` and add it as a new MistHelper menu item."
...
- **Tag**: `Sites Guests`
- **mistapi SDK module**: `mistapi.api.v1.sites.guests.derived`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listSiteAllGuestAuthorizationsDerived` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.guests.derived.listSiteAllGuestAuthorizationsDerived()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.guests.derived.listSiteAllGuestAuthorizationsDerived()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listSiteAllGuestAuthorizationsDerived` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
