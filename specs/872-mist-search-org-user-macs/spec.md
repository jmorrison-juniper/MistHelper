# Feature Specification: Mist API Read Operation -- searchOrgUserMacs

**Feature Branch**: `872-mist-search-org-user-macs`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `searchOrgUserMacs` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs User MACs`
- **mistapi SDK module**: `mistapi.api.v1.orgs.usermacs.search`
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchOrgUserMacs` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.usermacs.search.searchOrgUserMacs()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.usermacs.search.searchOrgUserMacs()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `searchOrgUserMacs` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
