# Feature Specification: Mist API Read Operation -- getSiteChannelScores

**Feature Branch**: `922-mist-get-site-channel-scores`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `getSiteChannelScores` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `getSiteChannelScores`
- **Method**: `GET`
- **Path**: `/api/v1/sites/{site_id}/rrm/channel_scores/band/{band}`
- **Tag**: `Sites RRM`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getSiteChannelScores` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.rrm.getSiteChannelScores()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.rrm.getSiteChannelScores()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `getSiteChannelScores` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
