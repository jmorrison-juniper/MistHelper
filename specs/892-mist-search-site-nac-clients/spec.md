# Feature Specification: Mist API Read Operation -- searchSiteNacClients

**Feature Branch**: `892-mist-search-site-nac-clients`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `searchSiteNacClients` and add it as a new MistHelper menu item."
...
- **mistapi SDK module**: `mistapi.api.v1.sites.nac_clients.search`
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchSiteNacClients` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.nac_clients.search.searchSiteNacClients()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.nac_clients.search.searchSiteNacClients()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `searchSiteNacClients` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
