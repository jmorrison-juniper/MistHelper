# Feature Specification: Mist API Read Operation -- listOrgNacTags

**Feature Branch**: `936-mist-list-org-nac-tags`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `listOrgNacTags` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs NAC Tags`
- **mistapi SDK module**: `mistapi.api.v1.orgs.nactags`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listOrgNacTags` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.nactags.listOrgNacTags()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.nactags.listOrgNacTags()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `listOrgNacTags` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
