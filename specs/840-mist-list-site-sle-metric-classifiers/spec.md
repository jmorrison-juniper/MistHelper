# Feature Specification: Mist API Read Operation -- listSiteSleMetricClassifiers

**Feature Branch**: `840-mist-list-site-sle-metric-classifiers`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listSiteSleMetricClassifiers` and add it as a new MistHelper menu item."
...
- **Tag**: `Sites SLEs`
- **mistapi SDK module**: `mistapi.api.v1.sites.sle.metric.classifiers`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listSiteSleMetricClassifiers` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.sle.metric.classifiers.listSiteSleMetricClassifiers()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.sle.metric.classifiers.listSiteSleMetricClassifiers()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listSiteSleMetricClassifiers` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
