# Feature Specification: Mist API Read Operation -- searchOrgOspfStats

**Feature Branch**: `957-mist-search-org-ospf-stats`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `searchOrgOspfStats` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `searchOrgOspfStats`
- **Method**: `GET`
- **Path**: `/api/v1/orgs/{org_id}/stats/ospf_peers/search`
- **Tag**: `Orgs Stats - Ospf`
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchOrgOspfStats` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.stats.searchOrgOspfStats()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.stats.searchOrgOspfStats()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `searchOrgOspfStats` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
