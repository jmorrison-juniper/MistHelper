# Feature Specification: Mist API Read Operation -- countSiteWanClientEvents

**Feature Branch**: `562-mist-count-site-wan-client-events`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `countSiteWanClientEvents` and add it as a new MistHelper menu item."
...
- **mistapi SDK module**: `mistapi.api.v1.sites.wan_client.events.count`
...
(org / site / device as applicable), and receives the JSON payload exposed by `countSiteWanClientEvents` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.wan_client.events.count.countSiteWanClientEvents()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.wan_client.events.count.countSiteWanClientEvents()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `countSiteWanClientEvents` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
