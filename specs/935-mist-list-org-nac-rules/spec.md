# Feature Specification: Mist API Read Operation -- listOrgNacRules

**Feature Branch**: `935-mist-list-org-nac-rules`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `listOrgNacRules` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs NAC Rules`
- **mistapi SDK module**: `mistapi.api.v1.orgs.nacrules`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listOrgNacRules` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.nacrules.listOrgNacRules()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.nacrules.listOrgNacRules()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `listOrgNacRules` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
