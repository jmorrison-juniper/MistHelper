...
- [X] T023 [P] Create the browser fixtures in `tests/e2e/upgrade_portal/conftest.py`. Reuse the shape of the `gunicorn_server` fixture that `tests/e2e/conftest.py` held at the time of writing, and bind it to port 8056. Issue #1998: that fixture is gone. Its own file records that no test ever used it, and that Gunicorn cannot import `fcntl` on Windows.
...
- [X] T219 [P] Add the performance test for the 90-second Tier 2 capture target and the 3-second comparison render target to `tests/unit/upgrade_portal/test_performance.py`. Issue #1998: both numbers are performance goals of `plan.md`. This task once named them SC-002 and SC-005, and neither label is right. SC-002 is a 30-second read of the finished comparison, and SC-005 is the two-operator criterion.
...
- [ ] T232 Run every scenario in `quickstart.md` and record the result
  ...
  - Run of 2026-09-02: the container stack ran, and the store half now verifies. `quickstart-results.md` section 0 holds the record. Scenario A wrote capture `cap-94b9e2fb486c47eea7c77a0506d2d4d6-01`, which reached `verified` with 9 devices, 40 wired clients, and 20 wireless clients. Scenario B compared two stored captures 42 hours apart, named every added and every missing client, and exported the full comparison as CSV. Scenario E took and released the site lock. Scenario F listed 8 stored captures with readable UTC moments. The run found one defect: one device of the type `router` made the inventory page answer 500. Issue #2211 records it and pull request #2212 repaired it. Scenario C and scenario D stay open, because both write firmware to production hardware and a person must decide to run them.
  - Run of 2026-09-04: the confirm page ran against the live cloud and the live stack, and `specs/1823-upgrade-capture-portal/live-run-2026-09-04.md` holds the record. A browser walked the whole read path of the Morrison House Site: the service check, the token sign-in, the organization list, the site list, the inventory page, the site lock, a tier 2 capture that reached `verified` with 9 devices and 18,989 bytes, the run create, the options save, and the confirm page. The walk chose the no-reboot option, and the confirm page then drew all four plan warnings, including the repair of issue #2003 with the real count of 6 access points and the repair of issue #2007 for the switch that may reboot with the control off. The begin button stayed disabled with no word typed. The walk cancelled the staged run and released the site lock, so it wrote no firmware. The inventory page also showed the repair of issue #2211 for the `router` row and the repair of issue #2006 for the gateway, which draws the running version beside the configured target. Scenario C and scenario D stay open for the reason above.
