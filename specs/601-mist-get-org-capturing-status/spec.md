# Feature Specification: Mist API Read Operation -- getOrgCapturingStatus

**Feature Branch**: `601-mist-get-org-capturing-status`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getOrgCapturingStatus` and add it as a new MistHelper menu item."
...
- **Tag**: `Utilities PCAPs`
- **mistapi SDK module**: `mistapi.api.v1.orgs.pcaps.capture`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getOrgCapturingStatus` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.pcaps.capture.getOrgCapturingStatus()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.pcaps.capture.getOrgCapturingStatus()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getOrgCapturingStatus` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
