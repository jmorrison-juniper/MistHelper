# Feature Specification: Mist API Read Operation -- countOrgAuditLogs

**Feature Branch**: `509-mist-count-org-audit-logs`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `countOrgAuditLogs` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs Logs`
- **mistapi SDK module**: `mistapi.api.v1.orgs.logs.count`
...
(org / site / device as applicable), and receives the JSON payload exposed by `countOrgAuditLogs` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.logs.count.countOrgAuditLogs()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.logs.count.countOrgAuditLogs()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `countOrgAuditLogs` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
