# Feature Specification: Mist API Read Operation -- countSiteGuestAuthorizations

**Feature Branch**: `551-mist-count-site-guest-authorizations`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `countSiteGuestAuthorizations` and add it as a new MistHelper menu item."
...
- **Tag**: `Sites Guests`
- **mistapi SDK module**: `mistapi.api.v1.sites.guests.count`
...
(org / site / device as applicable), and receives the JSON payload exposed by `countSiteGuestAuthorizations` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.guests.count.countSiteGuestAuthorizations()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.guests.count.countSiteGuestAuthorizations()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `countSiteGuestAuthorizations` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
