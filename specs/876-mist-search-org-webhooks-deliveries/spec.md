# Feature Specification: Mist API Read Operation -- searchOrgWebhooksDeliveries

**Feature Branch**: `876-mist-search-org-webhooks-deliveries`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `searchOrgWebhooksDeliveries` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs Webhooks`
- **mistapi SDK module**: `mistapi.api.v1.orgs.webhooks.events.search`
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchOrgWebhooksDeliveries` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.webhooks.events.search.searchOrgWebhooksDeliveries()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.webhooks.events.search.searchOrgWebhooksDeliveries()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `searchOrgWebhooksDeliveries` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
