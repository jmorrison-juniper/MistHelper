# Feature Specification: Mist API Read Operation -- searchMspOrgGroup

**Feature Branch**: `859-mist-search-msp-org-group`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `searchMspOrgGroup` and add it as a new MistHelper menu item."
...
- **Tag**: `MSPs`
- **mistapi SDK module**: `mistapi.api.v1.msps.search`
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchMspOrgGroup` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.msps.search.searchMspOrgGroup()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.msps.search.searchMspOrgGroup()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `searchMspOrgGroup` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
