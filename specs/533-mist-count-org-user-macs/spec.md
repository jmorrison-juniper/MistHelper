# Feature Specification: Mist API Read Operation -- countOrgUserMacs

**Feature Branch**: `533-mist-count-org-user-macs`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `countOrgUserMacs` and add it as a new MistHelper menu item."
...
(org / site / device as applicable), and receives the JSON payload exposed by `countOrgUserMacs` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.usermacs.count.countOrgUserMacs()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.usermacs.count.countOrgUserMacs()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `countOrgUserMacs` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
