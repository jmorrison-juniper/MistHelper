# Feature Specification: Mist API Read Operation -- listOrgNacPortals

**Feature Branch**: `934-mist-list-org-nac-portals`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `listOrgNacPortals` and add it as a new MistHelper menu item."
...
(org / site / device as applicable), and receives the JSON payload exposed by `listOrgNacPortals` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.nacportals.listOrgNacPortals()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.nacportals.listOrgNacPortals()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `listOrgNacPortals` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
