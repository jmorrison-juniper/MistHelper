# Feature Specification: Mist API Read Operation -- searchSiteSkyatpEvents

**Feature Branch**: `897-mist-search-site-skyatp-events`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `searchSiteSkyatpEvents` and add it as a new MistHelper menu item."
...
- **Tag**: `Sites Skyatp`
- **mistapi SDK module**: `mistapi.api.v1.sites.skyatp.events.search`
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchSiteSkyatpEvents` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.skyatp.events.search.searchSiteSkyatpEvents()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.skyatp.events.search.searchSiteSkyatpEvents()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `searchSiteSkyatpEvents` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
