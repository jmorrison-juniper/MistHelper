# Feature Specification: Mist API Read Operation -- searchSiteDeviceLastConfigs

**Feature Branch**: `885-mist-search-site-device-last-configs`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `searchSiteDeviceLastConfigs` and add it as a new MistHelper menu item."
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchSiteDeviceLastConfigs` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.devices.last_config.search.searchSiteDeviceLastConfigs()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.devices.last_config.search.searchSiteDeviceLastConfigs()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `searchSiteDeviceLastConfigs` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
