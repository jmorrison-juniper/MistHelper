# Feature Specification: Mist API Read Operation -- listOrgAAMWProfiles

**Feature Branch**: `764-mist-list-org-a-a-m-w-profiles`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `listOrgAAMWProfiles` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `listOrgAAMWProfiles`
- **Method**: `GET`
- **Path**: `/api/v1/orgs/{org_id}/aamwprofiles`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listOrgAAMWProfiles` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.aamwprofiles.listOrgAAMWProfiles()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.aamwprofiles.listOrgAAMWProfiles()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `listOrgAAMWProfiles` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
