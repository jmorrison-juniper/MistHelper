# Feature Specification: Mist API Read Operation -- listSelfAuditLogs

**Feature Branch**: `944-mist-list-self-audit-logs`
**Created**: 2026-09-15
**Status**: Draft
**Input**: User description:
"Catalog the missing Mist API GET endpoint `listSelfAuditLogs` and add it as a new MistHelper menu item."
...
- **Tag**: `Self Audit Logs`
- **mistapi SDK module**: `mistapi.api.v1.self.logs`
...
(org / site / device as applicable), and receives the JSON payload exposed by `listSelfAuditLogs` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.self.logs.listSelfAuditLogs()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.self.logs.listSelfAuditLogs()` via the `mistapi` SDK when available.
...
**FR-005**: Register the operationId `listSelfAuditLogs` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
