# Feature Specification: Mist API Read Operation -- searchOrgOtherDeviceEvents
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchOrgOtherDeviceEvents` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.otherdevices.events.search.searchOrgOtherDeviceEvents()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.otherdevices.events.search.searchOrgOtherDeviceEvents()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `searchOrgOtherDeviceEvents` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
