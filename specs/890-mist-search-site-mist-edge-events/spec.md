# Feature Specification: Mist API Read Operation -- searchSiteMistEdgeEvents

**Feature Branch**: `890-mist-search-site-mist-edge-events`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `searchSiteMistEdgeEvents` and add it as a new MistHelper menu item."
...
- **Tag**: `Sites MxEdges`
- **mistapi SDK module**: `mistapi.api.v1.sites.mxedges.events.search`
...
(org / site / device as applicable), and receives the JSON payload exposed by `searchSiteMistEdgeEvents` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.mxedges.events.search.searchSiteMistEdgeEvents()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.mxedges.events.search.searchSiteMistEdgeEvents()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `searchSiteMistEdgeEvents` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
