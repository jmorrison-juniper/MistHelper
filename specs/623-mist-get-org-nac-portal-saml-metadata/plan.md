# Implementation Plan: GetOrgNacPortalSamlMetadata Menu Item

**Branch**: `623-mist-get-org-nac-portal-saml-metadata` | **Date**: 2026-06-30 | **Spec**: [spec.md](./spec.md)
**Input**: Feature specification from `specs/623-mist-get-org-nac-portal-saml-metadata/spec.md`

## Summary

Add a new read-only MistHelper menu item that invokes the Mist API endpoint
`GET /api/v1/orgs/{org_id}/nacportals/{nacportal_id}/saml_metadata` (operationId
`getOrgNacPortalSamlMetadata`) to retrieve the SAML Service-Provider metadata for
a single NAC portal so a NOC engineer can hand it to an IdP administrator when
configuring SSO / SAML federation. The menu item collects two identifiers via
`safe_input()` (`org_id` and `nacportal_id`), invokes the `mistapi` SDK, flattens
the flat JSON object into a single row, and persists the result through
...
new operation is proposed as menu number **89**, the next unused slot inside the
Interactive Safe cluster (60-96) that already hosts the other per-entity NAC
portal viewers.
...
The `metadata` field of the response holds an embedded XML document that is
persisted as a single opaque TEXT column (no XML shredding).
**Testing**: `python MistHelper.py --test` exercises the menu item in
non-interactive mode using a known org + NAC portal from `.env`. Local quality
...
63-65, 90-100) is unaffected -- new item 89 sits inside the default test sweep
range.
...
NAC portals. The endpoint is non-paginated and returns one small JSON object
(the `metadata` XML string is typically a few KB). Adaptive delay metrics in
...
(`os.path.join` / `pathlib.Path`). The XML `metadata` payload is never logged in
full (only its length is emitted at DEBUG) to keep log files readable.
**Scale/Scope**: One new public menu method (~20 lines) added to the existing
`OrgExportUtils` class (which already owns `_nac_portals` for the bulk portal
list), one new entry in `ENDPOINT_PRIMARY_KEY_STRATEGIES`, one new CSV/SQLite
table (`org_nac_portal_saml_metadata`), one menu registration entry, one README
...
- **STATUS: PASS** -- The new menu method
  ...
  blocks (prompt for org -> prompt for portal -> API call -> flatten single
  row -> DataExporter call). Hierarchy is unchanged: one new method on an
  existing class. No new packages, modules, or top-level constants are
  introduced. The single flattener is a one-expression dict comprehension; if
  it grows past 5 lines during implementation, it is extracted to a private
  helper on the same class.

### Principle II: Class-Based Architecture (No Wrappers)

- **STATUS: PASS** -- The new behavior is added as a method on the existing
  `OrgExportUtils` class -- the same class that owns `_nac_portals`,
  `_nac_tags`, `_nac_rules`, and `_nac_events`. No standalone wrapper function
  is introduced. The menu dispatch in the main loop references the class
  method directly. Variable names use full words (`portal_row`,
  `saml_metadata_xml`) -- no single-letter iterators.

### Principle III: Safety-First (NON-NEGOTIABLE)

- **STATUS: PASS** -- All user input is collected through `safe_input()` with
  explicit `context=` strings (`"org_nac_portal_saml:org_id"`,
  `"org_nac_portal_saml:nacportal_id"`) so SSH / container EOF exits cleanly
  ...
  `WARNING` and returns early. The API token comes from `.env` via the
  existing `mistapi.APISession` and is never logged. The embedded XML
  `metadata` field is never echoed in full to logs.
...
- **STATUS: PASS** -- After implementation the standard pipeline applies
  ...
  -> `black --check` -> commit with `version YY.MM.DD.HH.MM - add menu 89
  GetOrgNacPortalSamlMetadata` -> `git push origin main` ->
...
- **STATUS: PASS** -- All log calls use ASCII text and `%s` style formatting.
  `INFO` is emitted before the API call ("Fetching SAML metadata for NAC
  portal %s in org %s"); `DEBUG` after the call with a size summary
  ("SAML metadata: acs_url=%s entity_id=%s metadata_len=%d");
  `WARNING` on 404 / empty payload; `ERROR` on unexpected exception with full
  traceback via `logging.exception`. No secrets, tokens, or the XML
  `metadata` payload itself are logged.
...
- **STATUS: PASS** -- Every executable line in the new method, the new
  ...
  in the touched block (the existing NAC-portal cluster around line 12611)
...
- **STATUS: PASS** -- The new method follows the documented pattern:
  ...
  `logging.debug(...)` after flatten (row count = 0 or 1), `logging.info(...)`
  before write. The DataExporter call already emits its own per-backend log
...
```text
specs/623-mist-get-org-nac-portal-saml-metadata/
├── plan.md              # This file
├── research.md          # Phase 0 - SDK signature, PK strategy, naming, menu placement
├── data-model.md        # Phase 1 - response entity + DDL + PK registration
├── quickstart.md        # Phase 1 - local run + .env + quality gates
├── contracts/
│   └── get_org_nac_portal_saml_metadata.md   # Phase 1 - HTTP + SDK contract
...
MistHelper.py            # New method on OrgExportUtils class + PK strategy entry
                         # + menu 89 registration. No new modules; same single-
                         # file monolith.
...
data/                    # Runtime output target (existing dir, no schema migration
                         # needed beyond the new SQLite table created on first write
                         # by DataExporter)
```
...
(the same class that owns `_nac_portals`, `_nac_tags`, `_nac_rules`, and
`_nac_events`). Extending this class -- rather than introducing a new
`NacPortalUtils` class -- respects Principle II (no wrappers, prefer existing
homes) and keeps all NAC-portal-related exports in one grep-able location. The
menu number proposal is **89**, chosen because operations 60-96 are the
Interactive Safe cluster and 89 is a currently-unallocated integer between the
existing NAC-portal viewers and the resource-intensive block at 97-101. The
full menu list will be re-verified at task generation time; if 89 collides
with an in-flight feature branch, the next free integer in the same cluster
is used.
...
- **Principle II (Class-Based)**: PASS -- All work lives on `OrgExportUtils`.
  No wrappers introduced. The optional flattening helper, if needed, is added
  as a private method on the same class.
...
- **Principle V (Observability)**: PASS -- Log statements in the design are
  ...
  echo the raw XML metadata payload.
