# Feature Specification: Mist API Read Operation -- getSiteSleSummaryTrend

**Feature Branch**: `709-mist-get-site-sle-summary-trend`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getSiteSleSummaryTrend` and add it as a new MistHelper menu item."

## Source Endpoint

- **operationId**: `getSiteSleSummaryTrend`
- **Method**: `GET`
- **Path**: `/api/v1/sites/{site_id}/sle/{scope}/{scope_id}/metric/{metric}/summary-trend`
- **Tag**: `Sites SLEs`
- **mistapi SDK module**: `mistapi.api.v1.sites.sle.metric.summary_trend`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getSiteSleSummaryTrend` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.sites.sle.metric.summary_trend.getSiteSleSummaryTrend()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.sites.sle.metric.summary_trend.getSiteSleSummaryTrend()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getSiteSleSummaryTrend` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
