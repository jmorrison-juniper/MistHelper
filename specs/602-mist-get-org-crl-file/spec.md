# Feature Specification: Mist API Read Operation -- getOrgCrlFile

**Feature Branch**: `602-mist-get-org-crl-file`
**Created**: 2026-06-29
**Status**: Draft
**Input**: User description: "Catalog the missing Mist API GET endpoint `getOrgCrlFile` and add it as a new MistHelper menu item."
...
- **Tag**: `Orgs CRL`
- **mistapi SDK module**: `mistapi.api.v1.orgs.crl`
...
(org / site / device as applicable), and receives the JSON payload exposed by `getOrgCrlFile` -- exported to the
...
1. **Given** valid credentials and org context, **When** the user selects the new menu item, **Then**
   MistHelper invokes `mistapi.api.v1.orgs.crl.getOrgCrlFile()` exactly once per required scope and persists results.
...
**FR-001**: Provide a new menu item that invokes `mistapi.api.v1.orgs.crl.getOrgCrlFile()` via the `mistapi` SDK.
...
**FR-005**: Register the operationId `getOrgCrlFile` in `ENDPOINT_PRIMARY_KEY_STRATEGIES` with the correct PK strategy (natural / composite / auto-increment).
